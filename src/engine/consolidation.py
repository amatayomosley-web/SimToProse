...
Contracts:
  ...
  record-contract.md      — event_type catalog shape {appraisal_map, world_map,
                            visibility}.
  state.py _DIM_TO_PATH — the known appraisal dimension vocabulary.
...
# ---- Known appraisal dimensions (state.py _DIM_TO_PATH vocabulary) ----
# DERIVED from state._DIM_TO_PATH, which is the single source of truth for what an appraisal
...
from .state import _DIM_TO_PATH as _DIMS_SOURCE            # noqa: E402
...
CATALOG = {
    ...
    "affront": {
        ...
        # The actor-taggable home for the social_violation dimension that status and insult scenes are built on
    ...
    "correction": {
        # Compensating event (consolidation-loop.md open-q 3, BUILT 2026-09-19): append-only [loop-oq3]
        "appraisal_map":   ["social_violation", "relief"],
        # `world_map: none` is load-bearing and stays: a correction moves no field of its OWN. Its
        # effect is that `fold.fold` SKIPS every event id in its `supersedes` list, which is the
        # inverse delta for a from-zero fold (fold.superseded_ids). Writer: `critic.correct_run`.
        "world_map":       "none",
...
def validate_tags(tags, percepts, skills):
    ...
    if row is not None:
        ...
        # The actor is asked for `tags.subject` (prompt.py) — never `tags.target`. Both
        # drivers merge the RESOLVED target onto `applied` — a SEPARATE dict — AFTER
        # this function returns (scripts/scene.py and scripts/direct.py both do
        # `applied = dict(applied, target=target)` post-validate_tags), which is why
        # arc.assess's later read of `tags.get("target")` is live while this containment
        # check's read of it never was.
        target = tags.get("subject")
        if target:
            from . import concepts as _concepts     # local: consolidation is imported early by many
            if _concepts.looks_like_concept(target):
                # A CONCEPT subject (gate three, 2026-09-11) is validated by identity against the
                # closed registry, never against the PerceptSet — sickness is recognised, not seen.
                if not _concepts.is_concept(target):
                    soft_flags.append(_flag("TAG_CONCEPT_UNKNOWN",
                                            "subject %r is not in the concept registry" % target))
            elif not _is_perceived(target, percepts):
