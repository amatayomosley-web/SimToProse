...
def edges(con, run_id, perceiver, target, as_of):
    """Relationship trajectory perceiver -> target as of a turn: every delta with
    the event that caused it, plus the folded net per axis."""
    as_of = _int_as_of(as_of)
    res = ReadResult([], ["edges: relationship_deltas %s->%s WHERE turn<=%d"
                          % (perceiver, target, as_of)],
                     as_of=as_of, perspective="char:%s" % perceiver)
    rows = _rows(con.execute(
        "SELECT delta_id, turn, axis, delta, cause_event FROM relationship_deltas "
        "WHERE run_id=? AND perceiver=? AND target=? AND turn<=? ORDER BY turn, delta_id",
        (run_id, perceiver, target, as_of)))
    net = {}
    for r in rows:
        net[r["axis"]] = round(net.get(r["axis"], 0.0) + (r["delta"] or 0.0), 6)
    res.rows = rows
    res.step("net by axis: %s" % (net if net else "(none)"))
    if not rows:
