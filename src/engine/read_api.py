...
def established(con, run_id, subjects, as_of):
    """What is ESTABLISHED about each subject as of a turn — the fence the actor is handed.

    THE LORE LICENCE (2026-09-11; owner: actors may make up lore where nothing is established).
    Per subject, two sources, both BINDING: the authored line (`bible_entities.what`, any kind,
    pinned with the run's bible) and every live utterance about it whose folded tier is authored
    or established (`claims.about` over `claims.BINDING` — the complete join, unranked, because a
    dropped fact is a contradiction the actor could not have avoided). A subject with neither is
    OPEN: the packet says so, and that is the ground the actor may fill. Superposed claims are
    deliberately NOT here — they bind nothing, and handing them to the next actor as facts would
    make every offhand line canon by repetition.

    -> rows [{subject, authored, kept: [verbatim sayings], open}], one per subject, in input order.
    """
    from . import claims as _claims
    _known_run(con, run_id)
    _require(isinstance(subjects, (list, tuple)) and all(isinstance(x, str) for x in subjects),
             "READ_SUBJECTS_NOT_STRINGS", "subjects must be a list of strings, got %r" % (subjects,))
    as_of = _int_as_of(as_of)
    res = ReadResult([], ["established: %d subject(s) as of turn %d" % (len(subjects), as_of)], as_of=as_of)
    utts = _claims.for_run(con, run_id, as_of)
    resolutions = _claims.resolutions_for(con, run_id, as_of)
    seen = set()
    for subject in subjects:
        ...
        row = con.execute(
            ...
            "WHERE be.entity_id=? AND be.fingerprint = json_extract(r.config, '$.bible_fingerprint') "
            "AND r.run_id=? AND be.what <> '' LIMIT 1", (sid, run_id)).fetchone()
        authored = str(row["what"]) if row and row["what"] else ""
        kept = [u["text"] for u in _claims.about(utts, sid, resolutions)
                if _claims.tier_of(u, resolutions) in _claims.BINDING]
        res.rows.append({"subject": sid, "authored": authored, "kept": kept,
                         "open": not authored and not kept})
        res.step("%s: %s authored, %d kept saying(s)%s"
                 % (sid, "an" if authored else "no", len(kept), "" if (authored or kept) else " — OPEN"))
    return res
