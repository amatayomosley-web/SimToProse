...
import math
import os
from .records import PATHS, RecordError   # rule 6's bad-input type; see codes.py PROFILE_*
...
VALID_BASELINE_FIELDS = {
    ...
    # The eight PATHS are NOT here (2026-09-10). Until then a formative diff moved a path's
    # resting MEAN (baseline.temperament.<PATH>.mean); the owner ruled temperament is a
    # character DESIGN question authored as a rest WORD beside the voice, so no pass writes
    # there. What a formative profile used to say about resting fear is kept on the profile as
    # a `rest_note` for the author who now chooses the word. `validate_profile` refuses a path
    # field by name (PROFILE_DIFF_FIELD_UNKNOWN).
...
def validate_profile(profile):
    ...
    for r in profile.get("catalog_rows", []):
        ...
        if "lever" not in r or r["lever"] not in PATHS:
            raise RecordError("PROFILE_CATALOG_ROW_LEVER_UNKNOWN", "catalog_row lever must be one of %s, got %r" % (PATHS, r.get("lever")))
        op = r.get("op", "x")
        if op not in ("x", "+"):
...
#   (baseline.temperament is NOT a target since 2026-09-10 — the rest word is authored, see above)
...
def path_for(field):
    ...
    if field in PATHS:
        raise RecordError("PROFILE_FIELD_PATH_UNKNOWN",
                          "profiles.path_for: %r is a PATH — where a character rests is authored as a "
                          "rest word at baseline.temperament.%s.rest beside their voice (2026-09-10); "
                          "no formative diff writes there. Put what the profile says about it in its "
                          "rest_note" % (field, field))
    if field in _HEXACO:
