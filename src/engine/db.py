...
def _migrate(con):
    ...
    if v < SCHEMA_VERSION:
        with open(_SCHEMA_PATH, encoding="utf-8") as fh:
            _v32_scene_clock(con, fh.read())            # runs the schema script, rebuilding scene_clock around it
...
_OLD_AT_CHECK = re.compile(r"\bat_minutes\s*>=\s*0")


def _v32_scene_clock(con, schema):
    """Run the schema script, rebuilding a scene_clock that still carries v25's CHECK (at_minutes >= 0) around it
    (v32, gate own-timelines: a scene may be set before day 1). SQLite cannot drop a CHECK, so the old table is
    renamed aside with its append-only triggers dropped, the script creates the new one and its triggers, and the
    rows are copied across as they are before the old one goes. Each step is idempotent on its own condition, so an
    open interrupted part-way finishes the rebuild on the next."""
    old = con.execute("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'scene_clock'").fetchone()
    # A WORD BOUNDARY, NOT A SUBSTRING: the new table's `beat_minutes >= 0` contains "at_minutes >= 0", so a
    # substring test found the old CHECK in every scene_clock there is.
    if old is not None and _OLD_AT_CHECK.search(old[0]):
        con.executescript("BEGIN; DROP TRIGGER IF EXISTS scene_clock_no_update; DROP TRIGGER IF EXISTS "
                          "scene_clock_no_delete; ALTER TABLE scene_clock RENAME TO scene_clock_v31; COMMIT;")
    con.executescript(schema)
    if con.execute("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'scene_clock_v31'").fetchone():
        con.executescript("BEGIN; INSERT INTO scene_clock (clock_id, run_id, turn, at_minutes, lasts_minutes, "
                          "beat_minutes) SELECT clock_id, run_id, turn, at_minutes, lasts_minutes, beat_minutes FROM "
                          "scene_clock_v31; DROP TABLE scene_clock_v31; COMMIT;")
