...
from .records import PATHS, WOUND_DELTA_KINDS, RecordError   # noqa: F401  (the first two re-exported for callers of this tier)
...
def effective(current, rows=()):
    ...
    mult = {p: 1.0 for p in PATHS}
    add  = {p: 0.0 for p in PATHS}
...
def _row_active(when, ctx):
    ...
    for key in ("present_edge", "target_edge"):
        ...
        if key == "target_edge":
            # TARGET_EDGE IS NOT PRESENT_EDGE. What you feel toward the person this beat is ABOUT
            # is a fact about you, not about the room, so it must survive their being elsewhere.
            # `ctx["edges"]` holds only parties PRESENT; once a merely-spoken-of person stopped
            # taking an edge, a row keyed to the subject went dead with nothing raised —
            # `_row_active` returns False and rows never report non-firing. `subject_edge` is the
            # assembler's edge for that absent subject; `present_edge` below is untouched and must
            # never see them.
            subject = ctx.get("target")
            row = edges.get(subject) if subject in edges else ctx.get("subject_edge")
            edges = {subject: row} if (subject and row) else {}
        who = edge_req.get("id")
