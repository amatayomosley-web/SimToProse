...
def divergence(led, run_id):
    ...
    THE TAIL REPLAY APPLIES THE CORRECTION SKIP RULE (2026-09-19), because the from-zero fold it is
    compared against does. `fold.fold` drops every event id an arrived `correction` supersedes; a
    replay that did not would say "she is in the ward" where the fold says None, and this function
    would report a divergence that is its own arithmetic rather than a stale cache. Measured: a
    snapshot cached at turn 1, a bad move at 2, a park at 3 and a correction at 4 diverged on every
    resume — the turn-1 cache SURVIVES the drop and carries the move forward.

    WHY SKIPPING IS EXACT HERE AND NOT A GUESS, which is the whole reason a tail replay can do it
    at all: a tail cannot un-apply what the cached snapshot already folded. It does not have to.
    `world_events._reaches_back_to` drops the cache from the earliest SUPERSEDED event's
    `effective_at`, so any snapshot that survives is cached BELOW every superseded id — every one
    of them is therefore in the tail, and so is the correction naming them (it is later still).
    That invariant is what makes `superseded_ids(tail)` the same set `fold` computes over the whole
    log. Weaken the drop in `world_events` and this becomes wrong silently, so they move together.
    """
    from . import fold as _fold                        # local: `fold` imports KINDS from this module
    ...
    if cached is None:
        ...
        tail = led._events_between(run_id, as_of, t)
        dead = _fold.superseded_ids(tail)              # collected over the WHOLE tail, before any of it projects
        for ev in tail:
            if ev["event_id"] not in dead:
