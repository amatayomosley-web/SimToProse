...
def parse(token):
    ...
    if kind not in _RESOLVERS and kind not in _UNBACKED:
        raise CitationError(
            "unknown citation namespace %r (known: %s)"
            % (kind, ", ".join(sorted(set(_RESOLVERS) | set(_UNBACKED))))
        )
...
_UNBACKED = {
    "law": "no lore store yet (orchestrator-design.md §7.1)",
    "entity": "no lore store yet (orchestrator-design.md §7.1)",
    "chronicle": "no lore store yet (orchestrator-design.md §7.1)",
}
