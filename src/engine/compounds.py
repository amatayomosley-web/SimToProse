...
def compose(name, intensity=1.0, targets=None):
    ...
    if name not in COMPOUNDS:
        raise CompoundError("compose: unknown compound %r (known: %s)"
                            % (name, ", ".join(sorted(COMPOUNDS))))
    if not isinstance(intensity, (int, float)) or not (0.0 <= float(intensity) <= 1.0):
        raise CompoundError("compose: intensity must be a number in [0,1], got %r" % (intensity,))
...
def recognise(vector, top=3, floor=0.0):
    ...
    if not isinstance(vector, dict):
        raise CompoundError("recognise: vector must be a dict, got %r" % type(vector).__name__)
...
def recipe_sum(name, intensity=1.0):
    ...
    """
    if name not in COMPOUNDS:
        raise CompoundError("recipe_sum: unknown compound %r" % (name,))
...
def blend(name, baseline, intensity=1.0, targets=None):
    ...
    if not isinstance(baseline, dict):
        raise CompoundError("blend: baseline must be a dict, got %r" % type(baseline).__name__)
