...
def with_actor_summary(seat_tags, actor_tags):
    """The event seat's tags, carrying the ACTOR's one-line summary of what it did -> dict.

    The seat RATES the act and never writes a summary; the actor's reply contract asks for one on
    every beat (`prompt.py`, tags.summary), and `assess` / `witness_belief` build a lived belief from
    it. When the seat's tags replaced the actor's wholesale, the summary went with them, and a
    character's memory could grow only under --stub or a seat refusal: measured, 14 of 14 actor
    replies in a recorded scene carried a summary and 0 acquisitions were written over 33 beats.

    The summary is the actor's own account, not a rating, so it rides alongside the seat's verdict
    and never overrides anything the seat said. A seat reply that somehow carries its own summary
    keeps it. Returns a new dict; neither input is mutated.
    """
    out = dict(seat_tags or {})
    if not str(out.get("summary") or "").strip():
        summary = str((actor_tags or {}).get("summary") or "").strip() if isinstance(actor_tags, dict) else ""
        if summary:
            out["summary"] = summary
    return out
