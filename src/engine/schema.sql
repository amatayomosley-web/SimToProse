...
CREATE TABLE IF NOT EXISTS relationship_deltas (-- every appraisal that moves a relationship edge
    ...
    -- v27 (bond-arithmetic.md s5): WHAT THE ACT WAS ABOUT — the seat's object — so a reader can see
    -- WHY an edge moved ("because of what he did to the boy / to the ship") without re-deriving it.
    -- '' for a row written before the seat named objects, or an act about nothing in particular.
    object      TEXT    NOT NULL DEFAULT '',
    -- v29 (bond-arithmetic.md s6, 2026-09-18): for a debt row, WHAT changed hands — the seat's transfer
    -- `what`, quoted from the action — so an account movement can be read back as the thing it was
...
-- THE SCENE CLOCK (schema v25, 2026-09-10). A scene's own reading: when it opened, in minutes
-- from the book's day 1 00:00, and how long it ran if the author said. The gap before the NEXT
...
CREATE TRIGGER IF NOT EXISTS scene_clock_no_update
BEFORE UPDATE ON scene_clock BEGIN
    SELECT RAISE(ABORT, 'scene_clock is append-only (CLAUDE.md hard rule 2): UPDATE refused.');
END;
CREATE TRIGGER IF NOT EXISTS scene_clock_no_delete
BEFORE DELETE ON scene_clock BEGIN
    SELECT RAISE(ABORT, 'scene_clock is append-only (CLAUDE.md hard rule 2): DELETE refused.');
END;
...
-- ---- where an edge RESTS (schema v28, docs/bond-arithmetic.md section 6) ----
-- Drift relaxes each relationship axis toward its OWN rest, never toward _NEUTRAL: seeded from the
-- authored edge (`authored`, at run creation or at a pre-v28 run's first resume), lowered by a trust
-- cliff (`cliff`, riding the causing turn's commit). An edge with no row rests where a stranger does
-- (bond_rest.stranger_rest: default_trust on trust, _NEUTRAL elsewhere) — the pre-v28 behaviour, so
-- an old log replays to the numbers it held. Append-only: a moved rest is a NEW row (the v12 lesson:
-- a mutable value with no log row cannot be rebuilt).
CREATE TABLE IF NOT EXISTS rest_declared (
    rest_id   INTEGER PRIMARY KEY AUTOINCREMENT,
    run_id    TEXT    NOT NULL CHECK (run_id <> ''),
    turn      INTEGER NOT NULL,                     -- takes effect BEFORE this turn's time declaration
    perceiver TEXT    NOT NULL CHECK (perceiver <> ''),
    target    TEXT    NOT NULL CHECK (target <> ''),
    axis      TEXT    NOT NULL CHECK (axis <> ''),   -- trust | affinity | respect | debt
    rest      REAL    NOT NULL CHECK (rest >= 0 AND rest <= 1),
    source    TEXT    NOT NULL CHECK (source IN ('authored', 'cliff'))
);
CREATE TRIGGER IF NOT EXISTS rest_declared_no_update
BEFORE UPDATE ON rest_declared BEGIN
    SELECT RAISE(ABORT, 'rest_declared is append-only (CLAUDE.md hard rule 2): UPDATE refused. A moved rest is a NEW row.');
END;
CREATE TRIGGER IF NOT EXISTS rest_declared_no_delete
BEFORE DELETE ON rest_declared BEGIN
    SELECT RAISE(ABORT, 'rest_declared is append-only (CLAUDE.md hard rule 2): DELETE refused. A moved rest is a NEW row.');
END;

-- ---- what a person HOLDS that is not a person (schema v30, docs/bond-arithmetic.md section 3) ----
-- The sheet's `current.attachments` block is seeded here as `authored` rows at run creation (or a
-- late join, or a pre-v30 run's first resume); the director's scene-cfg declarations land as
-- `director` rows at the scene's first turn; the keeper's rubric (deferred) will write `keeper`.
-- bond_rest.rehydrate folds them in log order beside the rest rows (slot: after rest, before the
-- turn's time declaration) onto the sheet block; connection.held_map reads the folded block every
-- beat; bonds.stake_of / act_from_tags read the same map. Append-only: a moved hold is a NEW row,
-- a dropped hold is a row at 0 — never a rewrite (the v12 lesson, again).
CREATE TABLE IF NOT EXISTS attachment_declared (
    attachment_id INTEGER PRIMARY KEY AUTOINCREMENT,
    run_id   TEXT    NOT NULL CHECK (run_id <> ''),
    turn     INTEGER NOT NULL,                        -- takes effect after this turn's rest rows, before its time declaration
    char_id  TEXT    NOT NULL CHECK (char_id <> ''),  -- whose hold
    entity   TEXT    NOT NULL CHECK (entity <> ''),   -- loc.<world.locations id> | grp.<people[].groups tag>
    hold     REAL    NOT NULL CHECK (hold >= 0 AND hold <= 1),
    sign     TEXT    NOT NULL CHECK (sign IN ('+', '-')),
    source   TEXT    NOT NULL CHECK (source IN ('authored', 'director', 'keeper'))
);
CREATE TRIGGER IF NOT EXISTS attachment_declared_no_update
BEFORE UPDATE ON attachment_declared BEGIN
    SELECT RAISE(ABORT, 'attachment_declared is append-only (CLAUDE.md hard rule 2): UPDATE refused. A moved hold is a NEW row.');
END;
CREATE TRIGGER IF NOT EXISTS attachment_declared_no_delete
BEFORE DELETE ON attachment_declared BEGIN
    SELECT RAISE(ABORT, 'attachment_declared is append-only (CLAUDE.md hard rule 2): DELETE refused. A dropped hold is a NEW row at 0 (relation none), never an erased one.');
END;

-- ---- readings: what the appraiser saw, per beat (docs/emotion-arithmetic.md section 1) ----
-- THE CAUSE, not the snapshot. `turns.affect` records where a character ENDED a beat; these record
-- WHY. Section 5 step 8 is explicit about the stakes: "Without it `f` and `R` are not re-derivable
-- and hard rule 2 is false for emotion." Same discovery as v23 (aboutness died at process exit) and
-- v12 (drift mutated memory and was recorded nowhere) -- each found by asking what a resume could
-- not reconstruct.
--
-- THE RUNG IS A NAME, NOT AN INDEX. Section 1: "No number leaves the appraiser." A name also
-- survives a re-band; `vault[N]` positional references already cost this repo once (gate.py:249).
--
-- ONE ROW PER READING, and NO UNIQUE across (turn, actor, path). Two readings on one path in one
-- beat are two separate additions -- section 3 step 2 adds each -- so collapsing them would lose
-- one. That is the opposite of `target_binds`, which is last-write-wins and keys accordingly.
CREATE TABLE IF NOT EXISTS readings (
    reading_id INTEGER PRIMARY KEY AUTOINCREMENT,
    run_id     TEXT    NOT NULL REFERENCES runs(run_id) CHECK (run_id <> ''),
    turn       INTEGER NOT NULL,
    actor      TEXT    NOT NULL CHECK (actor <> ''),   -- whose feeling moved
    path       TEXT    NOT NULL CHECK (path <> ''),    -- one of records.PATHS
    rung       TEXT    NOT NULL CHECK (rung <> ''),    -- a rung NAME on that path's ladder
    about      TEXT    NOT NULL,                       -- entity id; '' IS UNBOUND, not an error
    confidence TEXT    NOT NULL CHECK (confidence <> '')  -- a WORD; never enters arithmetic
);

CREATE TRIGGER IF NOT EXISTS readings_no_update
BEFORE UPDATE ON readings BEGIN
    SELECT RAISE(ABORT, 'readings is append-only (CLAUDE.md hard rule 2): UPDATE refused. A correction is a NEW reading, never a rewritten one.');
END;

CREATE TRIGGER IF NOT EXISTS readings_no_delete
BEFORE DELETE ON readings BEGIN
    SELECT RAISE(ABORT, 'readings is append-only (CLAUDE.md hard rule 2): DELETE refused. Deleting a reading makes the affect it caused underivable.');
END;
...
CREATE TABLE IF NOT EXISTS lands_on (
    run_id  TEXT    NOT NULL CHECK (run_id <> ''),
    turn    INTEGER NOT NULL,
    actor   TEXT    NOT NULL CHECK (actor <> ''),    -- whose beat this is
    char_id TEXT    NOT NULL CHECK (char_id <> ''),  -- a listener the beat reached
    ord     INTEGER NOT NULL,                        -- row order, the seat's own list order
    PRIMARY KEY (run_id, turn, char_id)
);

CREATE TRIGGER IF NOT EXISTS lands_on_no_update
BEFORE UPDATE ON lands_on BEGIN
    SELECT RAISE(ABORT, 'lands_on is append-only (CLAUDE.md hard rule 2): UPDATE refused. A correction is a NEW turn''s row, never a rewritten one.');
END;

CREATE TRIGGER IF NOT EXISTS lands_on_no_delete
BEFORE DELETE ON lands_on BEGIN
    SELECT RAISE(ABORT, 'lands_on is append-only (CLAUDE.md hard rule 2): DELETE refused. Deleting a lands_on row makes a pruned listener''s salience underivable.');
END;

-- ---- aboutness: what each feeling is ABOUT, per beat ----
-- The one accumulating tier that had no log. `targets.retarget` has computed per-primitive
-- aboutness every beat since 2026-08-22 and both drivers wrote it straight onto
-- `char["current"]["targets"]` in memory, so it died at process exit: measured 2026-09-06, neither
-- driver's rehydrate block replayed it while all five of its siblings (arc, vault, edges, toward,
-- wounds) did. A character enraged at a man all scene resumed the next scene enraged at nobody.
--
-- LAST WRITE WINS, NOT A SUM, and that is why this table is not shaped like toward_deltas. Those
-- carry SIGNED deltas that accumulate; a target is REPLACED (targets.py: "it is replaced, or it is
-- released"). `targets.replay` therefore assigns in turn order rather than summing.
--
-- A RELEASE IS A ROW. `retarget` rule 3 drops a bind the DIRECTEDNESS registry does not admit, and
-- rule 5 clears one whose feeling has decayed back to rest. Logging only positive binds would make
-- un-binding underivable and the replay would diverge from the live run — so `target` is '' for a
-- release and the column is NOT NULL: a release is a fact, not an absence.
CREATE TABLE IF NOT EXISTS target_binds (
...
CREATE TRIGGER IF NOT EXISTS target_binds_no_update
BEFORE UPDATE ON target_binds BEGIN
    SELECT RAISE(ABORT, 'target_binds is append-only (CLAUDE.md hard rule 2): UPDATE refused. A rebind is a NEW row, never a rewritten one.');
END;
CREATE TRIGGER IF NOT EXISTS target_binds_no_delete
BEFORE DELETE ON target_binds BEGIN
    SELECT RAISE(ABORT, 'target_binds is append-only (CLAUDE.md hard rule 2): DELETE refused. A release is a NEW row with target = '''', never a deleted one.');
END;

-- v26 (2026-09-11, gate three): WOUNDS ARE ENGINE STATE. A wound used to be authored prose on the
-- sheet (baseline.drives.fears_wounds) plus a hand-authored catalog row; the owner moved it out of
-- the sheet and onto the paths. A wound is keyed (concept, path) and comes into being either from
-- the formative library at creation (on the sheet, source profile:<id>) or MID-STORY from a durable
-- beat that leaves a path bound to a registry concept in its top rungs (wound.mint). The mid-story
-- birth is a fact the log must show or hard rule 2 is false for scars: resume folds these rows,
-- then wound_deltas, onto baseline.wounds. `triggers` are the words the character perceived on the
-- beat, kept for the actor's rendering; the LINK to later beats is the concept, by identity.
CREATE TABLE IF NOT EXISTS wound_minted (
    mint_id   INTEGER PRIMARY KEY AUTOINCREMENT,
    run_id    TEXT    NOT NULL CHECK (run_id <> ''),
    turn      INTEGER NOT NULL,
    char_id   TEXT    NOT NULL CHECK (char_id <> ''),
    wound_id  TEXT    NOT NULL CHECK (wound_id <> ''),   -- "<concept>@<PATH>"
    concept   TEXT    NOT NULL CHECK (concept <> ''),    -- a src/engine/concepts.py id
    path      TEXT    NOT NULL CHECK (path <> ''),       -- one of records.PATHS
...
BEFORE UPDATE ON wound_minted BEGIN
    SELECT RAISE(ABORT, 'wound_minted is append-only (CLAUDE.md hard rule 2): UPDATE refused. A wound moves by a wound_deltas row, never by rewriting its birth.');
END;
CREATE TRIGGER IF NOT EXISTS wound_minted_no_delete
