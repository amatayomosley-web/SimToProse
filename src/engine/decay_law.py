...
MEASURED 2026-09-03, THE ENGINE HELD SIX SPELLINGS OF IT:
    ...
    bond_rest.drift          rest[axis] + (edge - rest[axis]) * (_RETENTION[axis] ** elapsed)
...
THE LIST ABOVE WAS WRITTEN FROM WHAT THE CODE DID, NOT FROM WHAT IT IMPORTED, and one row was
false for six days. `state.decay` is named here as a caller and imported nothing from this
module until 2026-09-09: six spellings were unified and the SEVENTH sat inside a module this
header claims, agreeing by arithmetic rather than by wiring, green throughout. It calls
`relax` now, and it took the `elapsed` argument it was the only decay tier lacking. A header
that enumerates callers is a hand-maintained duplicate like any other -- `tests/test_state.py`
now asserts the call rather than trusting this paragraph.
...
def relax(value, rest, retention, elapsed):
    ...
    disagree about whether to clamp before or after — `bond_rest.drift` clamps per axis, `arc.erode`
