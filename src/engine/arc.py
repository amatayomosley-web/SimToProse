...
from .state import build_profile, price_for
...
# RESILIENCE LOST ITS SELF-CONTROL TERM on 2026-09-10. It used to be
#     0.45 * effortful_control + 0.30 * (1 - load) + 0.25 * attachment
# and `effortful_control` was cut with the primitive-era genotype (owner: "start with the
# foundation, we will add to it for what we need"). The two surviving terms are RENORMALISED to
# the same 30:25 proportion so the range stays [0.05, 0.95] and the PTG fork at 0.70 stays
# reachable — a constant standing in for the cut term (the first draft) squeezed every character
# into [0.34, 0.75] and put the fork one rounding error from unreachable, which `tests/test_arc.py`
# caught. When a self-control axis returns to the genotype, it takes a share of these weights back;
# the split below is what it takes it from. Recorded in the gate's OMISSIONS.
_W_CONDITION, _W_ATTACHMENT = 0.30 / 0.55, 0.25 / 0.55
...
def derive_resilience(char, condition, excluded=None):
    """DERIVED, never stored (arc-engine.md): self-control × condition × attachment-security.
    (The 4th design field — meaning-frame availability — is a documented TODO; defaulted out here.
    The self-control term is `_EC_TYPICAL` for everyone until a genotype axis for it returns.)
    Returns [0.05, 0.95].
    ...
    _her.entry(char.get("fixed", {}).get("genotype", {}), PATHS[0])   # refuses a retired-shape genotype loudly
    # High load depletes resilience. An ABSENT load reads 0.0, the value every other reader of an absent
    # condition takes (gate._energy_budget, direction.direct_condition): a book that runs no condition
    # system must not be rested for memory and loaded for the arc at once. It read 0.3 until gate
    # systems-registry (2026-09-22) - a copy of the attachment prior below, never documented for load.
    ...
    r = _W_CONDITION * (1.0 - load) + _W_ATTACHMENT * attachment
...
def assess(tags, impact, char, condition, heights=None):
    """If this turn's event is DURABLE, return a sparse baseline diff; else None.

    `heights` (Phase 3, 2026-09-11): {path: the height of this beat's reading on that path}. A
    reading at or above the durable height makes the beat a candidate exactly as a severe
    dimension does (docs/emotion-arithmetic.md section 5 step 7); `impact` is then the beat's
    total addition from `state.receive`.
    ...
    candidate = (tags.get("durability") == "durable"
                 or any(float(v) >= _DURABLE_DIM for v in dims.values())
                 or any(float(h) >= _DURABLE_DIM for h in (heights or {}).values()))
    if not candidate:
        return None
    ...
    #
    # STILL DIMS ONLY, and that is now half the story (2026-09-19, gate heights-price-arc, owner
    # decision D3). `state._DIM_TO_PATH` carries no row for RECEPTIVITY or SELF-REGARD — nothing in
    # the event vocabulary means "something good reached you" or "your standing moved" — so this
    # table can never price either path, by design, not by omission. D3 is READINGS-ONLY for the
    # two of them: no new `_DIM_TO_PATH` rows. The receipt just below prices every path whose
    # READING reached the durable height this beat, dims or no dims, through the identical damage
    # step — where a path has both a priced dim and a durable reading, it collects both receipts.
    ...
    # THE READINGS RECEIPT (see above): what AROSE, priced from the height that named it rather
    # than from a dimension — the only route RECEPTIVITY/SELF-REGARD ever get, and open to every
    # other path too. Any path whose reading reached `_DURABLE_DIM` this beat qualifies, whether or
    # not it is the reading that made this beat a candidate, and whether or not the dims loop above
    # already priced it — when both fire the terms SUM, the same accumulation the dims loop uses.
    for path, height in (heights or {}).items():
        h = float(height)
        if h >= _DURABLE_DIM:
            v = h * grw * (1.0 - resilience)
            if v:
                diff["temperament"][path] = diff["temperament"].get(path, 0.0) + v
...
def apply(char, diff):
    ...
    # SEED FIRST (gate resume-and-parity, 2026-09-22). A path authored with only a REST WORD has no
    # `mean` until `heritable.ensure_temperament` seeds one, and assembly does that on its first line —
    # AFTER both drivers' resume paths have already replayed the arc through here. Unseeded, the
    # `.get("mean", 0.5)` below stamped the MIDPOINT as the authored value and landed a moved path at
    # 0.5 plus its change instead of its rest plus its change: every registered sheet on the live run
    # carries rest words only. Idempotent, so the live path (already seeded) is untouched.
    _her.ensure_temperament(new)
    temp = new.setdefault("baseline", {}).setdefault("temperament", {})
    for p, d in diff.get("temperament", {}).items():
        if p in PATHS and isinstance(temp.get(p), dict):
            ...
            # target, the retired renderer's rising/settling language — is untouched. The authored copy
