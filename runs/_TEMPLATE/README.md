...
Copy this directory to `runs/<book-slug>/` to start a book. This is step A of `docs/new-book-manifest.md`, which lists everything else a book needs. The **showrunner** (`.claude/agents/showrunner.md`, designed in `docs/orchestration.md`) maintains these files as it runs the production. They are the production's **process memory** — not canon: together with the run DB (`runs/<book>.db` — the canonical world-truth ledger, `docs/world-state-ledger.md`), they fully reconstruct where the story is and what has been done at any pause, so a fresh showrunner — or a human — can resume mid-book.
