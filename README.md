...
*Mimesis by construction — simulate characters faithfully, then cut the novel from what they actually did.*

Most AI writing tools ask a model to produce a story. This does the opposite: it **simulates people**, records everything they think and do, and then *edits* a novel out of that record.

A deterministic Python engine computes every value — emotional state, skill checks, perception, memory salience, relationship deltas. The LLM is invoked only where no script can perform the act: being the person and choosing. Numbers live in the database; the prompt receives *"gripped by fear, barely holding together,"* never `fear: 0.8`.

You direct by **changing the world, never the character**. If no honest circumstance can motivate a beat, the character refuses — and that refusal is treated as the plot being wrong, not as a failure to push harder.
...
**By the numbers:** 10,641 lines of stdlib-only Python — a 3,673-line engine across 18 modules, 7 CLI tools, 24 test suites, 48 design documents, and an optional agent layer of 9 roles with 9 craft toolboxes.

### The parts that carry the weight
- **`ledger.py` + `schema.sql`** — an append-only event log with *two clocks* (`caused_at`, `effective_at`), so a delayed consequence is written now and folds at the tick it lands. Turn-commits are atomic; `resume` replays the log and **asserts** it equals the from-zero fold, aborting loudly on divergence.
- **`state.py`** — three-tier state (temperament → current → effective), with appraisal and decay.
- **`scene.py` + `gate.py`** — deterministic perception and recall: what a character can see, and what this moment brings to mind.
- **`consolidation.py`** — the keystone. The actor labels its own deed; the engine validates those tags against schema, containment, and capability. Nobody records anyone else's mind.
- **`arc.py`** — durable change: the same event that spikes transient state can permanently diff the baseline.
- **`citation.py` + `read_api.py`** — the grounding core. Every read is *as-of* a turn and carries a trace, so an empty answer is attributable rather than mysterious.

### Three properties worth the attention
1. **Faithfulness by construction.** Prose is cut from the record, not generated beside it — so the book cannot contain what didn't happen.
2. **POV is a hard boundary.** The narrator is walled to one character's knowledge, which makes dramatic irony a *computable* property rather than an authorial trick.
3. **Guards ship with negative controls.** `coherence_probe.py --corrupt` **must** fail; the citation gate's corrupt control **must** deny. A guard without a proven negative control is indistinguishable from one that cannot fire — a lesson this repo learned the hard way.
...
- [`docs/knowledge-model.md`](docs/knowledge-model.md) — the vault / injection model: knowledge is *acquired* (provenance, time-sliced, false belief); knowing gated, saying free; prior art (Writer's Desk, Scribe)
