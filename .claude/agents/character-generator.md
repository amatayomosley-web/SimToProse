...
Every number you write must trace to *why* it has that value — to the position, the formative environment, or the personal history that set it. **A value you cannot trace to their life is the arbitrary insert this design rejects** (`docs/baseline-generation.md`). You never "set traits to taste." You read off what *this* position and *this* history would produce, and you record the number *with its provenance* (`FEAR-temperament high → the flood, age 9`). Provenance does double duty: it also seeds the vault — they remember the flood, and the sim's `{thought}` will draw on it.
