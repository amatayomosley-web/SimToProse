...
**Premise:** *"a valley where every debt is carved on the debtor's door."*
```
LAW:      Debt marks itself — the moment a debt is struck, its tally cuts itself into the
          debtor's door, and it fades only when the debt is paid.
          { Limit: the mark CANNOT be cut away, burned off or left behind — a new door carries it
            by morning; it records the sum, never the creditor; it cannot collect anything }
          { Epistemic: FORKS THE WHOLE WORLD — if known-true, the valley lives under a ledger no
            one can falsify; if sincere false belief (the reeve's men cut the marks by night and
            everyone believes the doors cut themselves), the same premise is a tragedy of a people
            governed by a trick. The believer is simulated identically; only the ground truth
            differs. Pick one — it is load-bearing. }
COMMUNITY: standalone.
```
The Limit is what lets the world **refuse** a lever — a character cannot just burn the door and walk away from what they owe, because the mark is back by morning. That single line keeps the probe honest. The Epistemic fork is what makes the *same* premise two different worlds. One premise → answers in law, each carrying its teeth and its truth-status: that cascade *is* the work. Everything downstream — a valley whose carpenters never want for work, a history the marks bent, an economy of credit and a reeve's office built around them — inherits it and may not violate it.
