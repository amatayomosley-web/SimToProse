...
WORLD = {"world": "w",
         "people": [{"id": "pru", "what": "the stall holder"}, {"id": "ned", "what": "the cake judge"}],
         "locations": [{"id": "the-marquee", "what": "the fete marquee"}],
         "laws": [{"act": "sell_before_judging", "modality": "forbidden"}]}
CHARS = {"pru": {}, "ned": {}}
...
def _cfg(**over):
    ...
    # doing its job.
    base = {"name": "t", "at": {"day": 1, "time": "09:00"}, "situation": "Rain is coming down hard and the marquee roof has started to sag over the cake tables.",
            "subject": (None, None),
            "props": ["a bucket", "a broom", "a folding ladder"],
...
def test_each_error_fires_on_its_own_violation():
    ...
    ok, ctx = _fires(_cfg(cast=[{"id": "ghost", "drive": "a"}, {"id": "ned", "drive": "b"}]),
                     "not a character in this book")
    check("unknown-cast-id", ok, ctx)

    ok, ctx = _fires(_cfg(cast=[{"id": "pru", "drive": "a"}, {"id": "pru", "drive": "b"}]),
                     "more than once")
    ...
    sit = "Rain is coming down hard and the marquee roof has started to sag over the cake tables."
    ok, ctx = _fires(_cfg(cast=[{"id": "pru", "drive": sit}, {"id": "ned", "drive": "b"}]),
                     "copy of the situation")
    check("drive-copied-from-situation", ok, ctx)


def test_the_directors_holds_are_checked():
    """GATE 5 (bond-arithmetic.md s3): a cfg's `attachments` declarations name a cast member, a
    registered place or group, and a relation word (`none` legal). Each wrong part is its own error."""
    print("")
    print("[D] THE DIRECTOR'S HOLDS")
    good = _cfg(attachments=[{"char": "pru", "entity": "loc.the-marquee", "relation": "post"}])
    e, _w, _u = lint_scene.lint_cfg(good, WORLD, CHARS)
    check("a-good-declaration-is-clean", not [x for x in e if x.startswith("attachments")], e)
    for name, decl, needle in (("off-the-cast", {"char": "nobody", "entity": "loc.the-marquee", "relation": "post"}, "not in this scene's cast"),
                               ("unregistered-entity", {"char": "pru", "entity": "loc.tower", "relation": "post"}, "not a place or group the world registers"),
                               ("off-table-relation", {"char": "pru", "entity": "loc.the-marquee", "relation": "fond"}, "relation 'fond' is not one of")):
        fired, detail = _fires(_cfg(attachments=[decl]), needle)
        check("error-%s" % name, fired, detail)
    fired, _d = _fires(_cfg(attachments=[{"char": "pru", "entity": "loc.the-marquee", "relation": "none"}]), "attachments")
    check("none-is-a-legal-relation", not fired)
...
def test_heuristics_warn_and_never_block():
    ...
    scripted = _cfg(situation="The stall holder describes her sister's wedding to the cake judge at length.")
    ...
    meta = _cfg(cast=[{"id": "pru", "drive": "introduce the world lore to the reader"},
                      {"id": "ned", "drive": "keep the judging on time"}])
    ...
    aligned = _cfg(cast=[{"id": "pru", "drive": "same want"}, {"id": "ned", "drive": "same want"}])
...
def test_rule_5_is_enforced_now_that_props_reach_something():
    ...
    e2, _w, _u = lint_scene.lint_cfg(_cfg(props=["a bucket", "a tray"]), WORLD, CHARS)
    ...
    e4, w4, _u = lint_scene.lint_cfg(
        _cfg(props=["a tea urn", "the prize rosettes", "a cracked jam jar"]), WORLD, CHARS)
    ...
    slice_ = {"event": {"text": "x", "kind": "mundane"}, "props": ["a tea urn"]}
...
def main():
    ...
    for t in (test_a_clean_cfg_is_clean, test_each_error_fires_on_its_own_violation, test_the_directors_holds_are_checked,
