...
CLAIMS = {
    ...
    "direction_changes is read": (
        DEFERRED,
        "src/engine/records.py sets `direction_changes` on 8 primitives and it was consumed by\n         direction._phrase_for, which chose the REFLEXIVE variant when a primitive was aimed at\n         the character themselves. RETIRED 2026-09-08 with the band-phrase renderer: no rung\n         block knows who the state is aimed at, so the flag is set and read by nothing",
        "the composer target slot docs/rungs/STIRRING.md names, or a rung layer that can carry\n         an object",
        lambda: _calls("src/engine/direction.py", "direction_changes"),
    ),
    ...
    "the slope is rendered": (
        DEFERRED,
        "docs/emotion-dynamics.md — a state that LEAPS and one that climbs are different facts; "
        "RETIRED 2026-09-08 with the band-phrase renderer, whose `_slope_marker` was the only "
        "thing that rendered it. No rung block encodes movement (measured, 0 of 83), and "
        "`ledger.previous_affect` still records the data, so only the RENDERING is gone",
        "a rung path that can express movement, or a marker line composed beside the block",
