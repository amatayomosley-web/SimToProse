...
from src.engine import bonds, bond_rest, wound, arc, toward         # noqa: E402
...
def test_bonds_drift_is_UNCHANGED_by_the_lift():
    ...
    Since bond gate 4 the caller is `bond_rest.drift(edge, rest, elapsed)`; toward a stranger's rest
    with no priors the rest IS the old `_NEUTRAL`, so the equivalence holds byte for byte."""
    bad = []
    for e in _TIMES:
        edge = {"trust": 0.8, "affinity": 0.2, "respect": 0.65, "debt": 0.4}
        got = bond_rest.drift(dict(edge), bond_rest.stranger_rest({}), elapsed=e)
        for axis, r0 in bonds._NEUTRAL.items():
            want = max(0.0, min(1.0, r0 + (edge[axis] - r0) * (bond_rest._RETENTION[axis] ** e)))
            if abs(got[axis] - want) > 1e-12:
                bad.append((e, axis, got[axis], want))
    check("bond_rest.drift-matches-the-pre-lift-arithmetic", not bad, "%r" % (bad[:3],))
