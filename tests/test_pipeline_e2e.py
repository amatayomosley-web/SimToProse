...
def _run_scene(led, run_id, world, chars, cfg, start_turn):
    ...
    """
    # A --stub run has NO emotion seat, so the floor must see lands_on=None and never prune a
    # listener (gate lands-on-to-floor, 2026-09-19: the first build passed [] here and pruned everyone).
    import contextlib, io as _io
    _out = _io.StringIO()
    with contextlib.redirect_stdout(_out):
        nt = scene.run_scene(world, chars, cfg, led, run_id, start_turn, "stub", True, 3, think=False)
    check("a-stub-run-never-prunes-a-listener", " ~unreached" not in _out.getvalue(), _out.getvalue()[-300:])
...
def main():
    ...
    cfg1 = {"name": "fireside", "pov": "maren", "at": {"day": 1, "time": "09:00"}, "situation": "Maren and Edda rest by the fire.",
