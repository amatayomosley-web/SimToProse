...
def test_admit_rejects_out_of_bounds_lever_multiplier():
    bad = {
        ...
        "baseline_diffs": {"security": 0.10},
        "catalog_rows": [{"when": {"percept": ["fire"]}, "lever": "WARINESS", "op": "x", "magnitude": 4.0}]
    }
    admitted, reason = P.admit(bad)
...
def test_admit_accepts_valid_orthogonal_profile():
    valid = {
        ...
        "catalog_rows": [
            {"when": {"percept": ["blizzard", "whiteout", "frostbite"]}, "lever": "WARINESS", "op": "x", "magnitude": 1.7, "source": "wound:blizzard"}
        ],
        "vault_belief_seeds": [
...
def test_compose_single_profile():
    # WARINESS in the prior passes through UNTOUCHED: a path is not a composable field since
    # 2026-09-10 (the rest word is authored beside the voice); fire_survival_acute's old +0.10 on
    # it is now the profile's rest_note.
    prior = {"WARINESS": 0.25, "security": 0.55, "emotionality": 0.50}
    picks = [{"profile": "fire_survival_acute", "weight": 1.0}]
    composed = P.compose(prior, picks)
    assert abs(composed["WARINESS"] - 0.25) < 1e-4, composed["WARINESS"]
    assert abs(composed["security"] - 0.70) < 1e-4
    assert abs(composed["emotionality"] - 0.58) < 1e-4
    assert "WARINESS" in P.get("fire_survival_acute").get("rest_note", ""), "the old temperament effect must survive as a note"


def test_compose_enforces_max_stacked_cap():
    prior = {"WARINESS": 0.25, "security": 0.55}
    picks = [
...
def test_no_library_profile_writes_a_path():
    """The library carries no path diff (69 of 94 did until 2026-09-10); every profile that did
    keeps the effect as a rest_note, and validate_profile refuses a path field by name."""
    from src.engine.records import PATHS
    offenders = [pid for pid, p in P.LIBRARY.items() if any(k in PATHS for k in p.get("baseline_diffs", {}))]
    assert not offenders, offenders
    noted = [pid for pid, p in P.LIBRARY.items() if p.get("rest_note")]
    assert len(noted) >= 60, "expected the moved diffs to survive as rest_notes, found %d" % len(noted)
    try:
        P.validate_profile({"id": "x", "name": "x", "category": "c", "baseline_diffs": {"WARINESS": 0.1}})
        raise AssertionError("a path field must be refused")
    except Exception as exc:
        assert "PROFILE_DIFF_FIELD_UNKNOWN" in str(exc), str(exc)
    try:
        P.path_for("WARINESS")
        raise AssertionError("path_for must refuse a PATH")
    except Exception as exc:
        assert "PROFILE_FIELD_PATH_UNKNOWN" in str(exc) and "rest_note" in str(exc), str(exc)
