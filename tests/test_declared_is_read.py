...
#
# RETIRED 2026-09-19 (gate correction-events): `visibility`. Its entry read "never copied onto a
# produced Event — every producer takes records.py's 'public' default". `scripts/critic.py`
# `correct_run` now copies `CATALOG["correction"]["visibility"]` onto the correction Event it
# appends, which is the first producer anywhere to take a type's DECLARED visibility instead of the
# dataclass default, so the exemption is retired rather than re-worded — the two-way rule above.
# THE RESIDUE, recorded here because retiring an exemption is where a half-fix hides: exactly ONE
# of the seventeen rows reaches a producer this way. The other sixteen still hand every event
# records.py's "public", and this guard is a REFERENCE check by its own docstring, so its green is
# not a claim that per-type visibility is wired end to end. That remains a knowledge-model decision.
_KNOWN_UNREAD = {}
...
def test_the_guard_can_actually_fail():
    ...
    #
    # RE-AIMED 2026-09-19: this used to assert `_readers("visibility") == []`, which held only
    # while the column had no subscript reader anywhere — so wiring ONE (critic.py, the correction
    # row) turned the control red for the opposite of the reason it exists. The claim was never
    # "nothing reads visibility"; it is "an ATTRIBUTE reference does not count as reading the
    # column", and that is now asserted directly against the three modules that touch
    # `.visibility` and never subscript it. Stronger than the old form, and it cannot rot back:
    # a `_readers` that fell back to the bare word would name all three.
    attribute_only = ["src/engine/ledger.py", "src/engine/records.py", "src/engine/world_events.py"]
    vis = _readers("visibility")
    check("same-named-attribute-is-not-a-reader",
          not [p for p in attribute_only if p in vis],
          "ev.visibility must not count as reading CATALOG[t]['visibility']: %s" % vis)
    check("...and-the-attribute-users-really-do-name-it",
          all(".visibility" in open(os.path.join(REPO, p), encoding="utf-8").read()
              for p in attribute_only),
          "the control's specimen files stopped using the attribute — re-pick them")
