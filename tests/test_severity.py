...
def test_the_words_land_on_the_thresholds_ALREADY_calibrated_against_this_scale():
    ...
    from src.engine.bonds import _OVERT_SEVERITY, _CLIFF_FLOOR
    from src.engine.consolidation import _MISMATCH_THRESHOLD
    from src.engine.severity import act_value_of, ACT_WORDS
    ...
    # THE CLIFF LEFT THIS SCALE (bond gate 4): it reads the ACT ladder's floor word, not a strength
    # word — `treacherous` cliffs at `slight`, and `extreme` on its own cliffs nothing
    assert act_value_of("trust", ACT_WORDS["trust"][0]) <= _CLIFF_FLOOR < act_value_of("trust", ACT_WORDS["trust"][1]), (
        "only the floor word of the act ladder may reach the cliff")
...
def test_the_full_ladder_survives_the_engine_and_stays_ordered():
    ...
    for w in WORDS:
        ...
        impacts.append(sum(abs(out[p] - base[p]) for p in PATHS))
...
def test_the_rubric_anchors_the_two_boundaries_the_ENGINE_acts_on():
    ...
    assert "no version of this event that could be worse" in text, (
        "extreme is the worst of its kind — the rubric must give it a test a reader can actually run")
...
def test_a_scene_CFG_may_write_the_word_the_docs_tell_it_to_write():
    ...
    cfg = {"name": "t", "at": {"day": 1, "time": "09:00"}, "situation": "s", "cast": [{"id": "a", "drive": "to be believed"}],
