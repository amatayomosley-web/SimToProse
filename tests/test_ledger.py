...
def test_the_correction_readers_round_trip(tmp):
    """`corrections_for` / `superseded_events` — the reader half of consolidation-loop.md open-q 3.

    Thin delegates to `fold.py`, so what is pinned here is the SEAM: the rows come back, the ids
    come back as a set of ints (not the strings a JSON payload could hand over), `as_of` gates on
    `effective_at` the way the fold does, and a run with no correction answers empty rather than
    raising — every consumer (`cut.py`, `doctor.py`) asks unconditionally."""
    led = fresh(tmp, "corrections")
    led.append_turn(commit("r1", 0, events=[Event(type="move", payload={"to": "hills"}, actor="maren")]))
    moved = led.con.execute("SELECT event_id FROM events WHERE run_id='r1' AND type='move'").fetchone()[0]
    assert led.corrections_for("r1") == [], "a clean run reported a correction"
    assert led.superseded_events("r1") == set(), "a clean run reported a superseded id"

    from src.engine import world_events
    world_events.append(led, "r1", 3, [Event(
        type="correction", actor="maren", visibility="private-to-actor",
        payload={"supersedes": [moved], "turn": 0, "issue": "she never left", "source": "critic"})])

    rows = led.corrections_for("r1")
    assert len(rows) == 1 and rows[0]["type"] == "correction", rows
    assert json.loads(rows[0]["payload"])["issue"] == "she never left", rows[0]["payload"]
    assert led.superseded_events("r1") == {moved}, led.superseded_events("r1")
    assert all(isinstance(i, int) for i in led.superseded_events("r1")), "ids came back as strings"
    assert led.superseded_events("r1", as_of=2) == set(), "a correction applied before its own tick"
    assert led.superseded_events("r1", as_of=3) == {moved}, "a correction did not apply at its tick"
    # ...and the fold agrees with the reader, which is the only reason the reader is trustworthy.
    assert led.fold("r1", 3)["agents"]["maren"]["location"] is None, led.fold("r1", 3)["agents"]
    assert led.fold("r1", 2)["agents"]["maren"]["location"] == "hills", led.fold("r1", 2)["agents"]
...
def test_lands_on_rides_the_turn(tmp):
    """gate lands-on-to-floor (2026-09-19): the emotion seat's lands_on rides the SAME transaction
    as the readings it came with -- append_turn writes it (readings.write_lands_on), and
    Ledger.lands_on_for reads it back in the order the seat listed it."""
    led = fresh(tmp, "lands_on")
    led.append_turn(commit("r1", 0, lands_on=["b", "c"]))
    assert led.lands_on_for("r1", 0) == ["b", "c"], led.lands_on_for("r1", 0)
    n = led.con.execute("SELECT COUNT(*) c FROM lands_on WHERE run_id='r1' AND turn=0").fetchone()["c"]
    assert n == 2, "two ids in lands_on should persist as two rows (%d)" % n

    # AN EMPTY LIST PERSISTS NONE -- the seat naming nobody is not the same as no seat (that
    # distinction is `landed=None` vs `landed=False` in floor.py); either way, nothing to write.
    led.append_turn(commit("r1", 1, lands_on=[]))
    assert led.lands_on_for("r1", 1) == [], led.lands_on_for("r1", 1)
    n1 = led.con.execute("SELECT COUNT(*) c FROM lands_on WHERE run_id='r1' AND turn=1").fetchone()["c"]
    assert n1 == 0, "an empty lands_on should persist no rows (%d)" % n1

    # APPEND-ONLY, THE SAME TRIGGER PATTERN AS readings (CLAUDE.md hard rule 2).
    import sqlite3
    try:
        with led.con:
            led.con.execute("UPDATE lands_on SET char_id='z' WHERE run_id='r1' AND turn=0")
        raise AssertionError("an UPDATE on lands_on was accepted")
    except sqlite3.IntegrityError:
        pass
    try:
        with led.con:
            led.con.execute("DELETE FROM lands_on WHERE run_id='r1' AND turn=0")
        raise AssertionError("a DELETE on lands_on was accepted")
    except sqlite3.IntegrityError:
        pass


def test_bystander_rows_ride_the_turn(tmp):
    """gate non-speaker-decay (2026-09-22): every OTHER present character's mood after the beat's
    minutes lands in current_state in the turn's own transaction; a refused or rolled-back commit
    writes none of it; a bystander naming the actor is refused by name."""
    led = fresh(tmp, "bystanders")
    calm = flat_affect(0.3)
    led.append_turn(commit("r1", 0, bystanders={"edda": {"affect": calm, "condition": {"energy": 0.4}},
                                                 "joss": {"affect": flat_affect(0.2)}}))
    rows = {r["char_id"]: (json.loads(r["affect"]), json.loads(r["condition"])) for r in led.con.execute(
        "SELECT char_id, affect, condition FROM current_state WHERE run_id='r1' AND turn=0")}
    assert set(rows) == {"maren", "edda", "joss"}, rows
    assert rows["edda"] == (calm, {"energy": 0.4}) and rows["joss"][1] == {}, rows
    assert led.latest_affect("r1", "edda")["affect"] == calm, "resume must read the bystander's row"

    for bad, code in (({"maren": {"affect": calm}}, "RECORD_BYSTANDER_IS_ACTOR"),
                      ({"edda": {"affect": {"WARINESS": 0.2}}}, "RECORD_AFFECT_MISSING_PRIMARIES")):
        try:
            led.append_turn(commit("r1", 1, bystanders=bad))
            raise AssertionError("a bad bystander row was accepted: %r" % (bad,))
        except RecordError as e:
            assert e.code == code, (e.code, code)
    # MID-TRANSACTION: the bystander row collides on (run, char, turn) after the turn row inserted
    with led.con:
        led.con.execute("INSERT INTO current_state (run_id, char_id, turn, affect) VALUES ('r1', 'edda', 1, '{}')")
    try:
        led.append_turn(commit("r1", 1, bystanders={"edda": {"affect": calm}}))
        raise AssertionError("a colliding bystander row was accepted")
    except LedgerError as e:
        assert e.code == "LEDGER_TURN_COMMIT_ROLLED_BACK", e.code
    assert led.con.execute("SELECT COUNT(*) c FROM turns WHERE run_id='r1' AND turn=1").fetchone()["c"] == 0, (
        "a bystander row that failed must take its turn with it")
    assert led.con.execute("SELECT COUNT(*) c FROM current_state WHERE run_id='r1' AND turn=1 AND char_id='maren'"
                           ).fetchone()["c"] == 0, "the actor's row survived a rolled-back commit"


def test_lands_on_migration(tmp):
    """gate lands-on-to-floor (2026-09-19): a v30 db (pre-dating this table) gains lands_on and
    both triggers on open, and user_version stamps the engine's CURRENT version -- derived from
    `_db.SCHEMA_VERSION`, never a literal, so the next schema bump cannot re-break this the way a
    hardcoded number would (the exact fragility `tests/test_integrity.py`'s own
    "the-migration-stamped-the-CURRENT-version" check was written to retire)."""
    import sqlite3
    from src.engine import db as _db
    schema = io.open(os.path.join(REPO, "src", "engine", "schema.sql"), encoding="utf-8").read()
    path = os.path.join(tmp, "v30.db")
    con = sqlite3.connect(path)
    con.executescript(schema)
    con.execute("DROP TABLE lands_on")
    con.execute("PRAGMA user_version = 30")
    con.commit()
    con.close()

    con = _db.connect(path)
    names = {r[0] for r in con.execute("SELECT name FROM sqlite_master WHERE name LIKE 'lands_on%'")}
    ver = con.execute("PRAGMA user_version").fetchone()[0]
    integrity_errors = con.execute("PRAGMA integrity_check").fetchone()[0]
    con.close()
    assert names >= {"lands_on", "lands_on_no_update", "lands_on_no_delete"}, names
    assert ver == _db.SCHEMA_VERSION, "user_version=%d, engine is at %d" % (ver, _db.SCHEMA_VERSION)
    assert integrity_errors == "ok", integrity_errors
...
def test_a_LOST_RACE_refuses_with_the_SAME_code(tmp):
    ...
    led.register_character("r1", "quentin", {}, {})
    ...
    for expected, call in (("LEDGER_SCENE_EXISTS",
                           ...
                           ("LEDGER_CHARACTER_EXISTS",
                            lambda: led.register_character("r1", "quentin", {}, {})),
