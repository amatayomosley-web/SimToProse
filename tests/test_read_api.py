...
def _fixture(tmp):
    ...
    for t, fear in ((10, 0.2), (20, 0.8)):
        con.execute("INSERT INTO current_state(run_id, char_id, turn, affect, condition) VALUES(?,?,?,?,?)",
                    (RUN, "nessa", t, json.dumps({"WARINESS": fear}), json.dumps({"energy": 1.0})))
...
def test_state_is_as_of_not_latest(con):
    ...
    assert r.rows[0]["affect"]["WARINESS"] == 0.2, r.rows[0]
    r20 = R.state(con, RUN, "nessa", 20)
    assert r20.rows[0]["affect"]["WARINESS"] == 0.8, r20.rows[0]
...
def test_a_MISS_is_still_not_an_ERROR(con):
    ...
    ALLOWED = {
        ...
        "READ_SUBJECTS_NOT_STRINGS",      # a malformed QUESTION (2026-09-11, established(): subjects must be strings)
