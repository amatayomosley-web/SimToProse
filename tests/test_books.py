...
def test_slug_and_db_path_are_one_definition(tmp):
    _make_books(tmp, ["The Long Test Book"])
    d = os.path.join(tmp, "The Long Test Book")
    assert books.slug(d) == "the-long-test-book", books.slug(d)
    assert books.db_path(d).endswith(os.path.join("runs", "the-long-test-book.db")), books.db_path(d)
