...
AFF = {p: 0.5 for p in ("SEEKING", "FEAR", "RAGE", "LUST", "CARE", "PANIC_GRIEF", "PLAY")}
FAILS = []
...
def main():
    ...
    led.register_character("r", "brakk", {"name": "Brakk"}, {})
    led.register_character("r", "orven", {"name": "Orven"}, {})

    _commit(led, "r", 0, "brakk", "passes the bread", {"type": "mundane", "dimensions": {"mastery": 0.1}, "durability": "transient"})
    _commit(led, "r", 1, "brakk", "refuses, his voice gone hard", {"type": "affront", "dimensions": {"social_violation": 0.8}, "durability": "durable"})
    _commit(led, "r", 2, "orven", "a quiet remark over the soup", {"type": "care", "dimensions": {"care_relevant": 0.3}, "durability": "transient"})
    led.append_arc_diff("r", "brakk", 1, {"temperament": {"RAGE": 0.05}, "_meta": {"dominant": "RAGE", "impact": 0.42}})
    led.append_acquisition("r", "brakk", 1, {"claim": "Father will not bend on the worker", "provenance": "lived"})
    led.append_scene("r", 0, "dinner", "brakk", 0, 2)
    ...
    check("arc-hinge-brakk", "brakk" in wc and wc["brakk"][0]["dominant"] == "RAGE" and wc["brakk"][0]["turn"] == 1, str(wc))

    # acquisitions: what was learned
    av = cut.acquisitions_view(led, "r")
    check("acquisition-listed", "brakk" in av and av["brakk"][0]["claim"].startswith("Father will not bend"), str(av))
    ...
    check("scene-cast-and-length", si[0]["cast"] == ["brakk", "orven"] and si[0]["n_turns"] == 3, str(si[0]))
    check("scene-pov", si[0]["pov"] == "brakk")
