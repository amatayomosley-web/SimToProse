...
def _flat_affect(v=0.5):
    return {k: v for k in ("SEEKING", "FEAR", "RAGE", "LUST", "CARE", "PANIC_GRIEF", "PLAY")}
...
def test_name_hygiene():
    ...
    t = "Kestra threw herself under the palfrey; tend Kestra."
    masked = scope_names(t, {"kestra": {"known_as": "the damaged worker"}})
    check("unacquired-name-masked", "Kestra" not in masked, masked)
    check("masked-uses-actor-term", "the damaged worker" in masked, masked)
    check("acquainted-name-kept", scope_names(t, {"kestra": {"trust": 0.2}}) == t,
          "name masked despite no known_as — default must be common knowledge")
    fam = "Brakk and Wynn sat together."
    check("family-not-overmasked", scope_names(fam, {"brakk_wintercrest": {"trust": 0.5}}) == fam,
          "family name masked when it should be known")
    check("display-name-single", _display_name("kestra") == "Kestra")
    check("display-name-compound", _display_name("wynn_wintercrest") == "Wynn Wintercrest")
...
def test_acquaintance_recognition():
    ...
    world  = {"people": [{"id": "kestra", "what": "the worker"}]}
    skills = {"perception": 0.9, "insight": 0.1}        # low insight — would FAIL the identity check
    cond   = {"energy": 1.0, "allostatic_load": 0.0}
    ss     = {"event": {"text": "Kestra stands at the door.", "kind": "mundane"}, "recent": [], "location": None}
    stranger = [p for p in perception_scope(ss, world, skills, cond, {}) if p.get("ref") == "entity.kestra"]
    check("stranger-low-insight-unrecognized", bool(stranger) and "recognized_as" not in stranger[0], str(stranger))
    known = [p for p in perception_scope(ss, world, skills, cond, {"kestra": {"trust": 0.2}}) if p.get("ref") == "entity.kestra"]
    check("acquaintance-recognized-low-insight", bool(known) and known[0].get("recognized_as") == "Kestra", str(known))
