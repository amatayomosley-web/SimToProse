...
def _fixture():
    ...
    # THE PHOBIA AS A LEVER, for THIS tier's tests. Since gate three (2026-09-11) Ren's spider
    # scar is an engine WOUND at baseline.wounds that multiplies the RECEIPT through
    # connection.for_about, and the catalog row that doubled it on the float is gone from the
    # fixture. These tests measure the levers tier — a row on a standing fact — so they put a
    # row back here, in the test, where its double-count cannot reach a run.
    ch["baseline"].setdefault("catalog", {}).setdefault("rows", []).insert(0, {
        "when": {"percept": ["spider", "spiders", "web", "webbing"]},
        "lever": "WARINESS", "op": "x", "magnitude": 3.4, "source": "test: the phobia as a lever"})
    return ch, w
...
def test_formula():
    ...
    cur = flat(0.20)
    cur["WARINESS"] = 0.40
    e = effective(cur, [{"lever": "WARINESS", "op": "x", "magnitude": 2.0, "source": "a"}])
    check("multiplier", abs(e["WARINESS"] - 0.80) < 1e-9, e["WARINESS"])
    e = effective(cur, [{"lever": "WARINESS", "op": "+", "magnitude": 0.25, "source": "a"}])
    check("additive", abs(e["WARINESS"] - 0.65) < 1e-9, e["WARINESS"])
    e = effective(cur, [{"lever": "WARINESS", "op": "+", "magnitude": -0.30, "source": "a"}])
    check("additive-negative-is-a-debuff", abs(e["WARINESS"] - 0.10) < 1e-9, e["WARINESS"])
    # multipliers before adds, so authoring order within a kind cannot change the result
    rows = [{"lever": "WARINESS", "op": "+", "magnitude": 0.10, "source": "a"},
            {"lever": "WARINESS", "op": "x", "magnitude": 2.0, "source": "b"}]
    check("mult-applies-before-add",
          abs(effective(cur, rows)["WARINESS"] - 0.90) < 1e-9, effective(cur, rows)["WARINESS"])
    check("order-within-kind-irrelevant",
          effective(cur, rows) == effective(cur, list(reversed(rows))))
    e = effective(cur, [{"lever": "WARINESS", "op": "x", "magnitude": 9.0, "source": "a"}])
    check("clamped-high", e["WARINESS"] == 1.0, e["WARINESS"])
    e = effective(cur, [{"lever": "WARINESS", "op": "+", "magnitude": -9.0, "source": "a"}])
    check("clamped-low", e["WARINESS"] == 0.0, e["WARINESS"])
    check("other-levers-untouched", e["GOODWILL"] == 0.20)
...
def test_fail_loud():
    ...
    bad = [
        ...
        ("bad-op",            {"lever": "WARINESS", "op": "^", "magnitude": 2.0}),
        ("non-numeric",       {"lever": "WARINESS", "op": "x", "magnitude": "a lot"}),
        ("negative-multiple", {"lever": "WARINESS", "op": "x", "magnitude": -2.0}),
...
def test_conditions():
    ...
    R = lambda when: [{"when": when, "lever": "WARINESS", "op": "x", "magnitude": 2.0, "source": "s"}]
    ...
    ctx_a = dict(ctx, affect=dict(flat(), DISPLEASURE=0.80))
    check("affect-hit", len(active_rows(R({"affect_at_least": {"DISPLEASURE": 0.7}}), ctx_a)) == 1)
    check("affect-miss", len(active_rows(R({"affect_at_least": {"DISPLEASURE": 0.9}}), ctx_a)) == 0)
    ...
    check("no-when-is-always-active",
          len(active_rows([{"lever": "WARINESS", "op": "x", "magnitude": 2.0, "source": "s"}], ctx)) == 1)
...
def test_ceiling_is_broken():
    ...
    # `direction._BANDS` was retired with the band-phrase affect renderer on 2026-09-08. This
    # test needs a COARSE quantiser to show that tier 3 moves a value across bands, not the
    # retired prompt vocabulary, so it carries its own edges rather than borrowing
    # `_EDGE_BANDS`, which quantises RELATIONSHIP axes and would couple two unrelated scales.
    from src.engine.direction import _band
    _BANDS = (0.25, 0.55, 0.80)
    ...
    for _ in range(400):
        a = decay(appraise(a, {"dimensions": {"threat": 1.0}}, prof), temp, prof, elapsed=30.0)   # a TEST beat, minutes (see test_state._TEST_BEAT_MINUTES)
    tier1_max = a["WARINESS"]
    ...
    eff = pk["volatile"]["state"]["effective"]["WARINESS"]
    rawf = pk["volatile"]["state"]["affect"]["WARINESS"]
    ...
    rest_eff = pk0["volatile"]["state"]["effective"]["WARINESS"]
    rest_raw = ch["current"]["affect"]["WARINESS"]
    ...
    check("the-genotype-still-says-brave",
          str(ch["fixed"]["genotype"]["WARINESS"]["hit"]).startswith("low"))


def _direction_text(affect):
    """The actor-facing emotion direction for an affect vector, through the PATH form.

    Until 2026-09-08 this test read `direction.direct_affect`. That renderer is retired; the emotion
    language is now the selected rung blocks, so the assertion that different company produces a
    different DIRECTION is measured where the direction actually lives.
    """
    import sys as _sys, os as _os
    _sys.path.insert(0, _os.path.join(REPO, "scripts"))
    import composer as _c
    rows = _c.selectable(affect)
    if not rows:
        return ""
    return _c.direction_for(rows, _c.select_deterministic(rows))
...
def test_spider_A_vs_B():
    ...
    for name, ev in (("A", A), ("B", B)):
        ...
        out[name] = (st["effective"], _direction_text(st["effective"]),
                     len(pk["volatile"]["levers"]))
    (eA, dA, nA), (eB, dB, nB) = out["A"], out["B"]
    print("     A: %d rows  FEAR %.3f  CARE %.3f" % (nA, eA["WARINESS"], eA["GOODWILL"]))
    print("     B: %d rows  FEAR %.3f  CARE %.3f" % (nB, eB["WARINESS"], eB["GOODWILL"]))
    check("company-fires-more-rows", nB > nA, "%d vs %d" % (nB, nA))
    check("fear-lower-with-someone-to-protect", eB["WARINESS"] < eA["WARINESS"],
          "%.3f vs %.3f" % (eB["WARINESS"], eA["WARINESS"]))
    check("care-higher-with-someone-to-protect", eB["GOODWILL"] > eA["GOODWILL"],
          "%.3f vs %.3f" % (eB["GOODWILL"], eA["GOODWILL"]))
    ...
    check("contest-reverses",
          (eA["WARINESS"] > eA["GOODWILL"]) and (eB["GOODWILL"] > eB["WARINESS"]),
          "A fear %.3f/care %.3f ; B fear %.3f/care %.3f"
          % (eA["WARINESS"], eA["GOODWILL"], eB["WARINESS"], eB["GOODWILL"]))
...
def test_no_action_selection():
    ...
    check("every-lever-is-a-bounded-primary",
          all(r["lever"] in PATHS for r in rows),
          [r["lever"] for r in rows if r["lever"] not in PATHS])
...
def test_antagonistic_edges():
    ...
    hate = [{"when": {"target_edge": {"affinity_at_most": 0.25}},
             "lever": "DISPLEASURE", "op": "x", "magnitude": 2.4, "source": "him specifically"}]
    near = [{"when": {"present_edge": {"affinity_at_most": 0.25}},
             "lever": "STIRRING", "op": "x", "magnitude": 0.3, "source": "not while he is here"}]
    ...
    # THE SUBJECT MAY NOT BE IN THE ROOM. Once a person merely SPOKEN OF stopped taking an edge,
    # `ctx["edges"]` no longer held them and every `target_edge` row keyed to them stopped firing —
    # silently, because `_row_active` returns False and rows never report non-firing. What you feel
    # toward the man being talked about is a fact about you, not about the room.
    absent_ctx = {"text": "words", "edges": {"friend": edges["friend"]}, "affect": cur,
                  "condition": {}, "target": "foe",
                  "subject_edge": {"target": "foe", "trust": 0.10, "affinity": 0.05}}
    check("target_edge-fires-on-an-ABSENT-subject", len(active_rows(hate, absent_ctx)) == 1,
          "the foe is elsewhere; the row is about him, not about the room")
    check("present_edge-never-sees-the-absent-subject", len(active_rows(near, absent_ctx)) == 0,
          "presence rows must not fire on someone who is not here")
    # NEGATIVE CONTROL: with no subject_edge supplied the row cannot fire — proves the pass above
    # comes from the new branch and not from the foe leaking back into `edges`.
    check("no-subject-edge-no-fire",
          len(active_rows(hate, dict(absent_ctx, subject_edge=None))) == 0)

    e_foe = effective(cur, active_rows(hate, ctx("foe")))
    e_fr = effective(cur, active_rows(hate, ctx("friend")))
    check("hate-pulls-the-vector-toward-it", e_foe["DISPLEASURE"] > e_fr["DISPLEASURE"],
          "%.3f vs %.3f" % (e_foe["DISPLEASURE"], e_fr["DISPLEASURE"]))
    # at-least still means at-least: every row authored before this change is unaffected
    ally = [{"when": {"present_edge": {"affinity": 0.70}},
             "lever": "WARINESS", "op": "x", "magnitude": 0.6, "source": "someone at my back"}]
    check("bare-axis-still-means-at-least", len(active_rows(ally, ctx("foe"))) == 1)
    try:
        active_rows([{"when": {"present_edge": {"warmth": 0.5}}, "lever": "DISPLEASURE",
...
def test_a_wound_scales_the_row_it_names():
    ...
    # A SYNTHETIC PAIR since gate three (2026-09-11): the fixture's wound is engine state at
    # baseline.wounds and no fixture row names it (the receipt carries it). The wire itself is
    # unchanged: a row that names a wound id is scaled by what remains of that wound.
    wounds = [{"id": "predator@WARINESS", "concept": "predator", "path": "WARINESS", "intensity": 0.95,
               "source": "profile:t", "text": "the spider"}]
    rows = [{"wound": "predator@WARINESS", "when": {"percept": ["spider"]}, "lever": "WARINESS", "op": "x", "magnitude": 3.4, "source": "t"},
            {"wound": "predator@WARINESS", "when": {"percept": ["spider"]}, "lever": "STIRRING", "op": "x", "magnitude": 0.5, "source": "t"}]
    ...
    check("at-authored-intensity-nothing-moves",
          full["WARINESS"] == 3.4 and full["STIRRING"] == 0.5, str(full))
    ...
    check("a-healed-wound-hits-softer", 1.0 < part["WARINESS"] < 3.4, str(part))
    ...
    check("a-debuff-decays-upward", 0.5 < part["STIRRING"] < 1.0, str(part))
    ...
    check("a-spent-wound-is-the-identity", zero["WARINESS"] == 1.0 and zero["STIRRING"] == 1.0, str(zero))

    # and it must reach the VECTOR, not just the row
    base = {p: 0.3 for p in PATHS}
    check("the-scaling-reaches-effective",
          effective(base, scale_to_wounds(rows, healed))["WARINESS"]
          < effective(base, scale_to_wounds(rows, wounds))["WARINESS"])
...
def test_an_unlinked_row_is_untouched():
    ...
    from src.engine.levers import scale_to_wounds
    rows = [{"lever": "WARINESS", "op": "x", "magnitude": 3.4, "source": "s"},
            {"lever": "GOODWILL", "op": "+", "magnitude": 0.22, "source": "s"}]
...
def test_the_authored_magnitude_survives_for_the_trace():
    ...
    from src.engine.levers import scale_to_wounds
    rows = [{"wound": "w", "lever": "WARINESS", "op": "x", "magnitude": 3.4, "source": "s"}]
...
def test_a_deepened_wound_does_not_kill_the_beat():
    ...
    rows = [{"wound": "w", "lever": "STIRRING", "op": "x", "magnitude": 0.5, "source": "s"}]
    ...
    check("suppressor-floors-at-zero-not-negative", out[0]["magnitude"] == 0.0,
          str(out[0]["magnitude"]))
    try:
        effective({p: 0.3 for p in PATHS}, out)
    ...
    boost = scale_to_wounds([{"wound": "w", "lever": "WARINESS", "op": "x", "magnitude": 3.4,
    ...
    check("unmoved-is-still-byte-exact",
          scale_to_wounds([{"wound": "w", "lever": "WARINESS", "op": "x", "magnitude": 3.4,
...
def test_the_bookkeeping_key_never_reaches_the_actor():
    ...
    # the wound is engine state since gate three (2026-09-11): it rides the prefix as `wounds`
    # (scene._wounds_for_prefix) and renders as "what has marked you"
    from src.engine.scene import _wounds_for_prefix
    wounds = replay_wound_deltas([{"id": "predator@WARINESS", "concept": "predator", "path": "WARINESS",
                                   "text": "the spider", "intensity": 0.95, "source": "profile:t"}],
                                 [("predator@WARINESS", -0.55, "event")])
    rendered = json.dumps(direct_identity({"wounds": _wounds_for_prefix(_strip_notes({"wounds": wounds})["wounds"])}))
