...
CHAR = {
    "fixed": {"name": "Vesk", "genotype": {
        "STIRRING": {"hit": "elevated"}, "GOODWILL": {"hit": "low"}, "DEFLATION": {"hit": "low"},
        "DISPLEASURE": {"hit": "elevated"}}},
    "baseline": {
        "temperament": {p: {"mean": m, "variability": 0.1} for p, m in
                        zip(PATHS, (0.6, 0.45, 0.5, 0.2, 0.3, 0.35, 0.3, 0.15, 0.1))},
    ...
    "current": {
        "affect": {p: v for p, v in zip(PATHS, (0.6, 0.45, 0.5, 0.2, 0.3, 0.35, 0.3, 0.25, 0.1))},
...
def test_alien_full_loop():
    ...
    try:
        ...
        for i, ev in enumerate(EVENTS_ALIEN):
            ...
            affect = decay(appraise(affect, tags, profile), temperament, profile, elapsed=30.0)   # a TEST beat, minutes (see test_state._TEST_BEAT_MINUTES)
...
def _private_tokens():
    """The private-content tokens, taken from test_no_private_content.py — ONE list, not two.

    These two guards prove different properties (architecture vs IP) and must stay separate tests,
    but they were each carrying a hand-maintained token tuple, and the two drifted apart exactly as
    you would expect: on 2026-08-21 a private book's SURNAME sat in src/engine/gate.py while THIS
    sweep was green, because this tuple lists only fixture names and the IP list had first names
    without the family name. A guard whose token list is maintained in two places guards neither.
    Import failure is non-fatal: the fixture sweep below still runs (fail-open on the import, never
    on a hit).
    """
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location(
            "_pc", os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_no_private_content.py"))
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        return tuple(t for t in mod._BANNED if "/" not in t and ":" not in t)
    except Exception:
        return ()
...
def test_no_fixture_tokens_in_engine():
    ...
    Covers BOTH the public fixture cast (a machine that names its own fixtures is not portable)
    and the private book casts (hard rule 1 — src/engine carries zero book content).
    """
    print("\n[2] TOKEN SWEEP over src/engine")
    # TWO match modes on purpose. The fixture tokens are swept as SUBSTRINGS (their original
    # semantics — "healer" must also catch "healer_note"). The imported private tokens are swept on
    # WORD BOUNDARIES, because that is the semantics they were chosen under: a substring sweep over
    # them fires on ordinary prose forever and gets switched off inside a week. Measured, not
    # assumed — importing them as substrings immediately produced three hits in this very repo, each
    # a short private name buried inside an ordinary English word. test_no_private_content.py
    # documents the same trap. NOTE: do not name those tokens here to illustrate the point — the IP
    # sweep reads this file too, and spelling them out makes this comment itself a hit. Nor name the
    # words that hide them: a host word points a reader at its token as surely as spelling it out.
    # That is the guard working, not a reason to exempt the file.
    fixture_tokens = ("maren", "ashford", "joss", "edda", "bryn", "suil", "tobin", "healer",
                      "vesk", "carrowport", "marlo", "countinghouse")
    private_rx = [(t, re.compile(r"(?<![a-z0-9])%s(?![a-z0-9])" % re.escape(t))) for t in _private_tokens()]
    hits = []
