...
CHAR = {
    ...
    "baseline": {
        "temperament": {p: {"mean": m, "variability": 0.1} for p, m in
                        zip(PRIMARIES, (0.6, 0.45, 0.5, 0.2, 0.3, 0.35, 0.3, 0.15))},
    ...
    "current": {
        "affect": {p: v for p, v in zip(PRIMARIES, (0.6, 0.45, 0.5, 0.2, 0.3, 0.35, 0.3, 0.25))},
...
def test_no_fixture_tokens_in_engine():
    ...
    # a short private name buried inside an ordinary English word (two of them inside "comparison"
    # and "relevance"). test_no_private_content.py documents the same trap. NOTE: do not name those
    # tokens here to illustrate the point — the IP sweep reads this file too, and spelling them out
    # makes this comment itself a hit. That is the guard working, not a reason to exempt the file.
