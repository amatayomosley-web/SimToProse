...
REL = {"odile": {"known_as": "the woman with the lilies"}}   # the florist knows her only by a descriptor
...
def test_the_act_seam(tmp=None):
    ...
    clean = faithfulness.check_name_leaks("She tells the woman with the lilies what happened.", REL)
    leaked = faithfulness.check_name_leaks("She tells Odile what happened.", REL)
...
def test_the_wall_is_FACT_shaped_too(tmp=None):
    """The wall masked names and nothing else. It would catch "Leopold" where the character
    knows only "the man from the post office", and miss "the man from the post office is here"
    when the character should not know he works there at all. Not every secret is a name.
...
def test_faithful_turn_REGENERATES_on_a_fact_leak(tmp=None):
    ...
    try:
        ...
        def leak_then_clean(packet, event_text, temperament, model, stub, think=True, seed=None,
                            relationships=None, corrections=None, acts=(), **_kw):
...
def main():
    ...
    try:
        ...
        seq = ["I will ring Odile about the late delivery.", "I will ring the woman with the lilies about it."]
        calls = {"n": 0}
        ...
        check("regenerates-to-clean", not leaks and "Odile" not in turn["action"],
        ...
        def always_leak(packet, event_text, temperament, model, stub, think=True, seed=None,
                        relationships=None, corrections=None, acts=(), **_kw):
            calls2["n"] += 1
            return _turn("Odile is here, at the shop counter.")

        direct.llm_turn = always_leak
        turn2, leaks2 = direct.faithful_turn(PACKET, "ev", "t", "m", False, relationships=REL, max_retries=2)
        check("persistent-leak-returned", bool(leaks2) and leaks2[0][0] == "odile", str(leaks2))
        ...
        def clean_first(packet, event_text, temperament, model, stub, think=True, seed=None,
                        relationships=None, corrections=None, acts=(), **_kw):
            calls3["n"] += 1
            return _turn("I will wrap the bouquet for the woman with the lilies.")
