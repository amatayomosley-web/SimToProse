...
from src.engine.claims import (AUTHORED, BINDING, ESTABLISHED, FICTION,  # noqa: E402
                               ...
                               extracts_of, live, normalise, tested, tier_of, spoken)
...
def test_spoken_recovers_only_what_was_said():
    """THE UTTERANCE UNIT. An action is narration and dialogue mixed and only the dialogue is a
    claim anyone made; filing the narration would record the engine's prose as the character's.
    RED before 2026-09-06: no driver recorded an utterance at all, so `contradictions` ran over an
    empty table for every run.
    """
    q, ld, rd = '"', '“', '”'
    action = ("Ren set the axle down, and did not look up. "
              + q + "Cobb. Stop." + q + " A pause, level. "
              + q + "The wright at Fallow is two days off." + q)
    assert spoken(action) == ["Cobb. Stop.", "The wright at Fallow is two days off."], spoken(action)

    # CURLY QUOTES are the same act; a book's prose uses them and a straight-only reader is blind.
    assert spoken("He shrugged. " + ld + "Nine winters." + rd) == ["Nine winters."]

    # NARRATION ALONE ASSERTS NOTHING. A silent beat must record no utterance, or every crouch
    # becomes a claim the keeper has to read.
    assert spoken("He crouched by the cart and said nothing.") == []

    # CONTRACTIONS AND POSSESSIVES are why single quotes are not speech here.
    assert spoken("It was the men's own fault, and he didn't say so.") == []

    # ORDER IS KEPT, because a keeper reading a turn reads it in the order it was spoken.
    two = spoken(q + "First." + q + " He waited. " + q + "Second." + q)
    assert two == ["First.", "Second."], two

    # EMPTY SPANS ARE NOT UTTERANCES: `claims.record` refuses text that says nothing, and a caller
    # looping over this must not be handed something the writer will reject.
    assert spoken(q + q + " he managed.") == []
