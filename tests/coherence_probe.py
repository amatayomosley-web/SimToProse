...
# IMPORTED, not copied. This was a hand-written list of seven, and when DISGUST joined the basis on
# 2026-08-22 the probe kept building 7-key affect vectors and `levers.effective` refused them. A
# second copy of the basis is exactly how a basis change drifts silently — the probe is the
# permanent regression, so it must read the same tuple the engine does.
from src.engine.records import PATHS as _PRIMARIES    # noqa: E402
PATHS = list(_PRIMARIES)
...
from src.engine.consolidation import validate_tags, CATALOG, TagError, tag_refusal    # noqa: E402
...
def detectors(history, show=True):
    ...
    RECALIBRATED 2026-09-10 (decay-first order): the history now records each turn AFTER its receipt, not
    after a day's decay, so a startle a day apart is a full jump from rest and back — legit WARINESS reads
    0.18 (was 0.02), corrupt 0.23-0.30 on five paths (and still fails on saturation). OSC 0.18 -> 0.20."""
    SAT_FRAC, OSC, DRIFT = 0.15, 0.20, 0.55
    temps = history["temperament"]
    series = {p: [snap[p] for snap in history["affect"]] for p in PATHS}
    flags = []
    if show:
        print("  metric      " + "  ".join("%-5s" % p[:5] for p in PATHS))
    rows = {"sat": [], "osc": [], "rev": [], "drift": []}
    for p in PATHS:
        s = series[p]
        if any(v < -1e-9 or v > 1.0 + 1e-9 for v in s):
            flags.append("BOUNDS: %s left [0,1]" % p)
        # PINNED means stuck at a bound AWAY FROM REST. The ceiling always counts. The floor counts
        # only for a path that rests above its bottom rung: a path resting IN rung one (LEVITY at
        # `quiet`, 2026-09-11) that a threat-heavy scene holds at the floor has not left its rest
        # rung - the design says play is the first thing to go - and a float in the rest rung
        # carries exactly the information rest does. Before the ninth path every push was
        # positive and a floor pin could only mean a dead path; it can now mean collapse.
        from src.engine.heritable import rung_of_mean as _rung_of_mean
        rests_above_floor = _rung_of_mean(p, float(temps[p]["mean"])) > 1
        sat = sum(1 for v in s if v >= 0.97 or (v <= 0.03 and rests_above_floor)) / len(s)
...
def run_probe(mode):
    ...
    affect = {p: char["current"]["affect"][p] for p in PATHS}
    ...
    print("turn  " + "  ".join("%-5s" % p[:5] for p in PATHS) + "   event")
    for i, event in enumerate(EVENTS):
        ...
        if not validation["ok"]:
            # FAIL-FAST (2026-08-30). This was the THIRD byte-identical copy of the discard,
            # and it lives in the file CLAUDE.md designates the permanent real-model
            # regression — which run_all.py excludes via _PROBES. The silent discard would
            # have survived precisely in the tool used to verify it was gone, against a live
            # model: the context most likely to emit a genuinely invalid tag.
            raise TagError(*tag_refusal(validation, "maren", i))
        ...
        # DECAY FIRST, then the receipt — the drivers' order since 2026-09-10 (spec section 8). A TEST
        # beat of a DAY: the probe's events are separate life episodes (a raid, a death, a rescue).
        affect = appraise(decay(affect, temperament, profile, elapsed=1440.0), tags, profile)
        if mode == "corrupt":                      # positive control: corrupt the state row
            affect = {p: clamp(affect[p] + rng()) for p in PATHS}
        ...
        print("%4d  " % i + "  ".join("%.2f " % affect[p] for p in PATHS) +
              ("  | %s" % event["text"][:48]))
