...
with an unset act every law bears and most of them deny, so a blanket call would refuse every scene.
...
def _fixture():
    ...
    world = {
        ...
        "locations": [{"id": "reading_room", "what": "the reading room"}],
        "laws": [
            ...
            {"id": "no-calls-in-the-reading-room", "domain": "custom", "modality": "FORBIDS",
             "epistemic": "known-true", "act": "take-a-call",
             "teeth": "The librarian keeps the phone at the desk until closing.",
             "statement": "No call is taken in the reading room."},
...
def test_forbids_allows_with_teeth():
    ...
    con, fp = _con(*_fixture())
    v = bible.verdict_for(con, fp, act="take-a-call")
    check("allowed", v["allowed"] is True, v)
    check("recorded-as-a-violation", v["violations"] == ["no-calls-in-the-reading-room"], v["violations"])
    check("teeth-surface", v["teeth"] and "librarian" in v["teeth"][0], v["teeth"])


def test_does_not_over_refuse():
    print("\n[3] An act no law bears on is ALLOWED")
    con, fp = _con(*_fixture())
    v = bible.verdict_for(con, fp, act="shelve-a-book")
    check("allowed", v["allowed"] is True, v)
...
def test_blanket_call_is_the_wrong_call():
    ...
    authored = {"stone-does-not-float", "no-calls-in-the-reading-room", "the-old-road"}
...
def test_post_action_teeth():
    ...
    ev = sc._law_events(led, "r1", world, chars, {"act": "take-a-call"}, "someone")
    check("forbids-produces-an-event", len(ev) == 1, len(ev))
    check("teeth-are-carried", "librarian" in str(ev[0].payload.get("teeth")), ev[0].payload)
    ...
    for empty in ("", None, "shelve-a-book"):
