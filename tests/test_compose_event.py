...
was fixed: unconditional old-fixture text on an empty log, a 300-character truncation that fed
...
def test_compose_event_returns_the_situation_UNTOUCHED_on_an_empty_log():
    """The bug this function is scarred by: on an empty log it used to append a fixed line of the
    old default fixture's scene-setting — its time of day and its table — unconditionally, so the
    FIRST BEAT OF EVERY SCENE IN EVERY BOOK was told that one fixture's hour and furniture. Measured
    on a real book's opening scene. The situation is the director's, and it comes back as written."""
    sit = "First light at the mill. The wheel is still."
    ...
    check("no-fixture-furniture-leaks-in",
          "table" not in compose_event(sit, "").lower(), "the old fixture's furniture reappeared")
