...
def _char(rels):
    return {"fixed": {"genotype": {"GOODWILL": {"hit": "elevated"}, "DEFLATION": {"hit": "elevated"}}},
            "baseline": {"temperament": {p: {"mean": 0.30, "variability": 0.1} for p in PATHS},
...
def test_the_beloved_and_the_stranger_now_differ():
    ...
    aff = {p: 0.30 for p in PATHS}
    L = lambda t: {"type": "loss", "dimensions": {"loss": 0.85}, "target": t}
    beloved = appraise(aff, L("sister"), prof)["DEFLATION"]
    stranger = appraise(aff, L("a_stranger"), prof)["DEFLATION"]
...
def test_the_ceiling_keeps_the_alleles_apart():
    ...
    for allele in ("low", "typical", "elevated", "high"):
        ...
        ch["fixed"]["genotype"]["DEFLATION"]["hit"] = allele
        landed.append(appraise(aff, L, build_profile(ch))["DEFLATION"])
...
def test_connection_slows_a_real_decay():
    ...
    # WAS {"PLAY": 0.12, "RAGE": 0.11, "DISGUST": 0.07}. PLAY has no successor path, so the
    # warmth in this fixture is GOODWILL (was CARE) -- the surviving positive feeling one
    # person can hold toward another.
    start = {"current": {"toward": {"j": {"GOODWILL": 0.12, "DISPLEASURE": 0.11, "DISTASTE": 0.07}}}}
    # RE-BASED FROM DAYS TO MINUTES (2026-09-19, gate `attitude-staircase`): `erode`'s second
    # argument is the one declared clock now, so 10 days is 10 x 1440 and 60 days is 60 x 1440. The
    # spans are unchanged; only the unit they are spelled in is. What the assertions read off them
    # is unchanged too — at 1440 x 60 the warmth still falls past the contempt.
    far = copy.deepcopy(start); toward.erode(far, 10 * 1440.0)                   # an acquaintance
    near = copy.deepcopy(start); toward.erode(near, 10 * 1440.0, {"j": 0.9})     # someone close
    for prim in ("GOODWILL", "DISPLEASURE", "DISTASTE"):
    ...
    long = copy.deepcopy(start); toward.erode(long, 60 * 1440.0)
    v = long["current"]["toward"]["j"]
    check("contempt-outlasts-the-warmth", v["DISTASTE"] > v["GOODWILL"],
          "GOODWILL %.5f vs DISTASTE %.5f" % (v["GOODWILL"], v["DISTASTE"]))
...
def test_temperament_returns_toward_the_authored_self():
    ...
    ch = {"baseline": {"temperament": {
        "GOODWILL": {"mean": 0.85, "_authored_mean": 0.40, "variability": 0.1},
        "DISPLEASURE": {"mean": 0.20, "_authored_mean": 0.55, "variability": 0.1},
        "WARINESS": {"mean": 0.30, "variability": 0.1}}}}          # never moved: no stamp
    arc.erode(ch, 50)
    t = ch["baseline"]["temperament"]
    check("a-raised-mean-comes-down", t["GOODWILL"]["mean"] < 0.85, "%.4f" % t["GOODWILL"]["mean"])
    check("but-not-below-the-authored-value", t["GOODWILL"]["mean"] > 0.40, "%.4f" % t["GOODWILL"]["mean"])
    check("a-LOWERED-mean-comes-back-UP", t["DISPLEASURE"]["mean"] > 0.20, "%.4f" % t["DISPLEASURE"]["mean"])
    check("and-not-above-its-authored-value", t["DISPLEASURE"]["mean"] < 0.55, "%.4f" % t["DISPLEASURE"]["mean"])
    check("an-unmoved-primary-is-left-alone", t["WARINESS"]["mean"] == 0.30, str(t["WARINESS"]))
    check("the-authored-value-is-never-touched",
          t["GOODWILL"]["_authored_mean"] == 0.40 and t["DISPLEASURE"]["_authored_mean"] == 0.55)
