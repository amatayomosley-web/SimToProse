...
def test_holds_render_when_in_scope():
    """A held entity reaches volatile.holds only when it is in scope THIS TURN -- a percept or the
    beat's own subject/target -- and manifest.holds names the same kept entities (gate
    attachments-to-actor)."""
    print("\n[10] HOLDS -- a present percept or the beat's own subject puts a hold in scope")
    ch, w = _char(), _world()
    af, cond = _flat_affect(), ch["current"]["condition"]

    # (a) the fixture's own loc.cottage hold (.85, life), with the cottage percept present
    ss_present = {"event": {"text": "A quiet morning in the cottage.", "kind": "mundane"},
                  "recent": [], "location": "cottage"}
    packet = assemble(ch, w, ss_present, af, cond)
    cottage_what = next(l["what"] for l in w["locations"] if l["id"] == "cottage")
    check("loc.cottage is in scope and priced life",
          packet["volatile"]["holds"] == [{"entity": "loc.cottage", "name": cottage_what, "word": "life"}],
          packet["volatile"]["holds"])
    check("manifest.holds names it", packet["manifest"]["holds"] == ["loc.cottage"], packet["manifest"]["holds"])

    # (b) the SAME attachments block, no cottage percept and a different (unheld) target -> empty
    ss_absent = {"event": {"text": "A traveler passes on the upland road.", "kind": "mundane",
                           "target": "ren_traveler"},
                 "recent": [], "location": "upland_road"}
    packet2 = assemble(ch, w, ss_absent, af, cond)
    check("out of scope -> volatile.holds is empty", packet2["volatile"]["holds"] == [], packet2["volatile"]["holds"])
    check("and manifest.holds agrees", packet2["manifest"]["holds"] == [], packet2["manifest"]["holds"])

    # (c) a "-" entry is skipped even when its percept is present
    ch_minus = copy.deepcopy(ch)
    ch_minus["current"]["attachments"] = {"loc.cottage": {"hold": .85, "sign": "-", "note": "no longer hers"}}
    packet3 = assemble(ch_minus, w, ss_present, af, cond)
    check("a '-' hold never enters scope", packet3["volatile"]["holds"] == [], packet3["volatile"]["holds"])

    # (d) a grp. key that is the beat's own target renders with the tail as its name
    ch_grp = copy.deepcopy(ch)
    ch_grp["current"]["attachments"] = {"grp.ashford_villagers": {"hold": .40, "sign": "+", "note": "member"}}
    ss_grp_target = {"event": {"text": "Edda speaks for the village.", "kind": "mundane",
                               "target": "grp.ashford_villagers"},
                     "recent": [], "location": None}
    packet4 = assemble(ch_grp, w, ss_grp_target, af, cond)
    check("a grp. key as the beat's target renders with the tail as its name",
          packet4["volatile"]["holds"] ==
          [{"entity": "grp.ashford_villagers", "name": "ashford_villagers", "word": "member"}],
          packet4["volatile"]["holds"])

    # a character with NO attachments renders the packet exactly as before, plus the new empty
    # list and the new manifest key (gate attachments-to-actor's byte-identical contract)
    ch_none = copy.deepcopy(ch)
    del ch_none["current"]["attachments"]
    packet5 = assemble(ch_none, w, ss_present, af, cond)
    check("no attachments -> volatile.holds == []", packet5["volatile"]["holds"] == [])
    check("no attachments -> manifest.holds == []", packet5["manifest"]["holds"] == [])
