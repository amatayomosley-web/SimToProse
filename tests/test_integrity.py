...
from src.engine import db as _db                                # noqa: E402  (the version is DERIVED)
...
def test_a_MIGRATED_database_keeps_its_original_columns(tmp):
    ...
    # DERIVED, NOT PINNED. This read `version == 22` and went red on the v23 bump — a test whose
    # own name says CURRENT cannot hardcode a number, or every schema addition costs a false
    # failure and the next person edits the literal without asking what it was asserting.
    check("the-migration-stamped-the-CURRENT-version", version == _db.SCHEMA_VERSION,
          "user_version=%d, engine is at %d" % (version, _db.SCHEMA_VERSION))
