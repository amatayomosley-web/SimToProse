...
# Since 2026-08-30 a flag is STRUCTURE, not prose: {"code", "detail"}. It used to be a sentence,
# and faults.reason_key recovered its meaning with four regexes plus a catch-all that dropped every
# "schema:" flag into a bucket _ACTIONABLE excluded — so a run in which every beat failed on a
# missing `durability` reported "(none - the vocabulary fit this run)".
def _dim_mismatch_flag(dim, val, tag_type, legit):
    return {"code": "TAG_DIMENSION_NOT_IN_APPRAISAL_MAP",
            "detail": "dimension %r (%.2f) not in appraisal_map for type %r (legitimate dims: %s)" % (
...
def _run(turns, actor="ilsa", run_id="r1"):
...
def test_reason_key():
    ...
    check("unknown-dim", faults.reason_key(
        {"code": "TAG_DIMENSION_UNKNOWN", "detail": "unknown dimension 'wibble' (dropped; not invalidating)"})
        == ("unknown-dim", "wibble"))
    check("unknown-type", faults.reason_key(
        {"code": "TAG_TYPE_UNKNOWN", "detail": "unknown type 'frobnicate' (not in CATALOG)"})
        == ("unknown-type", "frobnicate"))
    check("target-not-perceived", faults.reason_key(
        {"code": "TAG_TARGET_NOT_PERCEIVED", "detail": "target 'ghost' not perceived in PerceptSet"})
        == ("target-not-perceived", None))
    check("capability", faults.reason_key(
        {"code": "TAG_CAPABILITY_BELOW_REQ", "detail": "skill 'combat' = 0.10 < required 0.30 for type 'harm'"})
        == ("capability-miss", "combat"))
    # THE ONE THAT USED TO BE DROPPED. A missing durability was ("schema-other", None) and
    # _ACTIONABLE excluded it, so the auditor counted the reason that was killing every beat and
    # threw it away. It is a first-class, actionable kind now.
    check("durability-missing-is-actionable", faults.reason_key(
        {"code": "TAG_DURABILITY_MISSING", "detail": "durability is absent; it must be one of {transient, durable}."})
        == ("durability-missing", None))
    check("durability-missing-in-actionable", "durability-missing" in faults._ACTIONABLE)
    check("legacy-prose-flag-is-other", faults.reason_key("schema: durability None not in {transient, durable}") == ("other", None))
    check("non-dict-safe", faults.reason_key(None) == ("other", None))
...
def test_recurring_gap_becomes_fault():
    ...
    try:
        ...
        if gap:
            ...
            check("localizes-to-actor", gap[0]["actors"] == ["ilsa"], str(gap[0]["actors"]))
