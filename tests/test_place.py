...
def test_NOT_NULL_does_not_mean_PRESENT(tmp):
    ...
    EXEMPT = {
        ...
        # THE RELEASE. `target_binds.target` is the one column here whose EMPTY value is the
        # meaning: `targets.retarget` rules 3 and 5 drop a bind, and a log carrying only positive
        # binds cannot express that, so the replay would re-attach an aboutness the beat had
        # already released. NOT NULL and deliberately unguarded — see src/engine/targets.py.
        "target_binds.target": "'' IS the release; a dropped bind is a fact, not an absence",
        # THE SAME SHAPE, one table over. A reading with no `about` is UNBOUND, and
        # `docs/emotion-arithmetic.md` section 5 step 3 rule 4 makes that meaningful rather than
        # missing: an empty `about` LEAVES AN EXISTING BIND UNTOUCHED, where a named one rebinds.
        # Refusing '' here would force the appraiser to invent a party for every feeling that is
        # about nothing nameable, which is most of them.
        "readings.about": "'' IS unbound; a feeling about nothing nameable is a fact, not an absence",
        # ENGINE WOUNDS (gate three, 2026-09-11). `source` is run:<turn> and defaults to '' only
        # for a row written by a caller that did not say (the engine always says); `text` is the
        # beat, which a stub may leave empty; `triggers` is a JSON list, '[]' when nothing was
        # perceived. The identity columns (wound_id, concept, path) ARE guarded.
        "wound_minted.source": "run:<turn> — the engine always writes it; the default guards a stub",
        "wound_minted.text": "the beat's words for the actor; empty under a stub is a fact",
        "wound_minted.triggers": "JSON",
