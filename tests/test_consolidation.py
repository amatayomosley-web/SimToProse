...
def test_schema_unknown_type():
    ...
    assert any("unknown type" in f for f in r["flags"]), \
...
def test_schema_bad_dim_value():
    ...
    assert any("outside [0, 1]" in f for f in r["flags"]), \
...
def test_schema_bad_durability():
    ...
    assert any("durability" in f for f in r["flags"]), \
...
def test_schema_bad_confidence():
    ...
    assert any("confidence" in f for f in r["flags"]), \
...
def test_containment_target_perceived():
    ...
    containment_flags = [f for f in r["flags"] if "containment" in f and "target" in f]
...
def test_dim_type_mismatch_mundane_threat():
    ...
    mismatch_flags = [f for f in r["flags"] if "not in appraisal_map" in f]
    assert len(mismatch_flags) >= 1, \
        "expected appraisal_map mismatch flag for threat=0.9, got: %s" % r["flags"]
    assert "threat" in mismatch_flags[0], \
...
def test_confidence_composition_one_soft_fail():
    ...
    assert len([f for f in r["flags"] if "not in appraisal_map" in f]) == 1, \
...
def test_affront_hosts_social_violation():
    ...
    assert not any("social_violation" in f and "appraisal_map" in f for f in r["flags"]), \
    ...
    r2 = validate_tags(_clean("threat", dims={"social_violation": 0.65, "threat": 0.5, "mastery": 0.7}),
                       EMPTY_PERCEPTS, MAREN_SKILLS)
    assert any("social_violation" in f and "appraisal_map" in f for f in r2["flags"]), \
