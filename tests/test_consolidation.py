...
def _clean(tag_type="mundane", dims=None, durability="transient",
    ...
    """Build a clean (schema-valid) tag dict. `subject` is the LIVE containment key
    (validate_tags reads tags.subject only, gate actor-contract-cleanup 2026-09-19) --
    never `target`, which is a resolved id the drivers merge onto `applied` AFTER
    validate_tags runs, which this fixture never sees."""
    t = {"type": tag_type,
...
def test_containment_target_perceived():
    ...
    r = validate_tags(_clean("care", subject="bryn"), BRYN_PERCEPTS, MAREN_SKILLS)
...
def test_containment_target_not_perceived():
    ...
    r = validate_tags(_clean("threaten", subject="the king", confidence=0.8),
...
def test_containment_target_not_perceived_escalates():
    ...
    r = validate_tags(_clean("mundane", subject="the king"), EMPTY_PERCEPTS, MAREN_SKILLS)
    assert r["escalate"] is True, \
        "unperceived target with default confidence must escalate, confidence=%.4f" % r["confidence"]


# ---- validate_tags reads tags.subject ONLY (gate actor-contract-cleanup, 2026-09-19) --------
# Both drivers merge the RESOLVED `target` onto `applied` -- a SEPARATE dict -- AFTER
# validate_tags returns (scripts/scene.py, scripts/direct.py: `applied = dict(applied,
# target=target)`), which is why a tags dict carrying only the dead `target` key must be
# invisible to containment, and one carrying `subject` must still be checked.

def test_target_alone_is_not_contained():
    """"target-alone-is-not-contained": a tags dict with only `target` (no `subject`) is not
    treated as contained -- not flagged AND not passed, because the check never runs on it at
    all. `target` stands for what a caller might carry before validate_tags resolves it; this
    function has never read it live -- its own dead-fallback history, closed by this gate."""
    r = validate_tags(dict(_clean("threaten", confidence=0.8), target="the king"),
                      EMPTY_PERCEPTS, {"combat": 0.50})
    assert _of(r, "TAG_TARGET_NOT_PERCEIVED") == [], \
        "a target-only tag must not be containment-checked at all, got: %s" % r["flags"]
    assert abs(r["confidence"] - 0.8) < 1e-9, \
        "confidence must be untouched (no soft-fail ran), got %.4f" % r["confidence"]


def test_subject_is_contained():
    """"subject-is-contained": `subject` alone -- still unperceived -- DOES trigger
    containment, proving the check runs on the live key rather than being silent on
    everything."""
    r = validate_tags(_clean("threaten", subject="the king", confidence=0.8),
                      EMPTY_PERCEPTS, {"combat": 0.50})
    assert len(_of(r, "TAG_TARGET_NOT_PERCEIVED")) >= 1, \
        "an unperceived subject must still raise the containment flag, got: %s" % r["flags"]
    assert r["confidence"] < 0.8, \
        "confidence must be capped on a containment miss, got %.4f" % r["confidence"]
