...
def test_the_dimension_vocabulary_is_DERIVED_not_copied(tmp):
    ...
    check("the-chassis-derives-its-dimensions", WA.DIMENSIONS == frozenset(_DIM_TO_PATH),
          "%s vs %s" % (sorted(WA.DIMENSIONS), sorted(_DIM_TO_PATH)))
...
def test_a_threat_NOTHING_watches_is_refused_as_a_beat(tmp):
    ...
    from src.engine.records import TurnCommit, PATHS
...
def test_the_DECAY_has_a_reader(tmp):
    ...
    led.declare_time("r1", 1, 12.0 * 24 * 60, "director")   # twelve DAYS each, in minutes: the tension tier reads the clock in days (clock.elapsed_days_since)
    led.declare_time("r1", 2, 12.0 * 24 * 60, "director")
...
def test_the_clock_and_the_band_have_ONE_spelling_each(tmp):
    ...
    check("and-CALLS-the-one-that-exists", ("led.elapsed_since(" in body or "led.elapsed_days_since(" in body),   # the tension tier reads the minute clock in DAYS since 2026-09-10
