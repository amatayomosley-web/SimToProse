...
SEEN = ["a spider on the windowpane", "a kettle coming to the boil"]
...
def test_the_perception_wall():
    ...
    check("unperceived-cue-does-not-move-it",
          wound.trial(w, {"threat": 0.99}, 0.5, ["lamplight", "the attic stairs"]) == 0.0)
...
def test_path_class_is_derived_from_dim_to_path():
    """GATE heights-price-arc (2026-09-19), GREEN_CONDITION. `_PATH_CLASS` is now DERIVED from
    `state._DIM_TO_PATH` — for each path, the dimension with the largest POSITIVE push onto it —
    instead of hand-written. The hand-written dict this replaced invented a dimension name for
    STIRRING and another for RECEPTIVITY, and neither was ever a key of `_DIM_TO_PATH` (its seven
    keys are threat/loss/care_relevant/mastery/social_violation/relief/attraction), so `trial`'s
    dims fallback for those two paths was dead code. Pin two ordinary derived rows and the two
    paths no dimension prices at all (RECEPTIVITY, SELF-REGARD — state.py's own comment: "those two
    paths are structurally reachable and never move")."""
    print("\n[10] _PATH_CLASS is derived from state._DIM_TO_PATH")
    check("STIRRING-classes-as-attraction", wound._PATH_CLASS["STIRRING"] == "attraction",
          wound._PATH_CLASS["STIRRING"])
    check("DEFLATION-classes-as-loss", wound._PATH_CLASS["DEFLATION"] == "loss",
          wound._PATH_CLASS["DEFLATION"])
    check("RECEPTIVITY-classes-as-None", wound._PATH_CLASS["RECEPTIVITY"] is None,
          wound._PATH_CLASS["RECEPTIVITY"])
    check("SELF-REGARD-classes-as-None", wound._PATH_CLASS["SELF-REGARD"] is None,
          wound._PATH_CLASS["SELF-REGARD"])


def test_a_receptivity_wound_with_dims_only_returns_zero_honestly():
    """No dimension prices RECEPTIVITY, so `_class_of` is None for a wound on that path, and
    `trial`'s dims fallback — `(dims or {}).get(_class_of(wound))` — must return 0.0 without
    raising rather than KeyError or silently matching some other dimension. `fires` must still be
    satisfied first, or this would pass for the wrong reason (no trial at all)."""
    print("\n[11] a RECEPTIVITY wound, dims only (no heights), returns 0.0 honestly")
    w = {"id": "w", "path": "RECEPTIVITY", "intensity": 0.5, "trigger": ["a letter arrives"]}
    check("class-of-a-RECEPTIVITY-wound-is-None", wound._class_of(w) is None, wound._class_of(w))
    check("the-cue-does-fire", wound.fires(w, ["a letter arrives"]))
    got = wound.trial(w, {"threat": 0.9, "care_relevant": 0.9}, 0.5, ["a letter arrives"])
    check("dims-only-trial-returns-zero-not-raises", got == 0.0, got)


def test_no_dead_dimension_names_remain():
    """SOURCE-LEVEL. The two invented dimension names this gate retired (the old `_PATH_CLASS`
    mapped STIRRING and RECEPTIVITY to them) must not survive anywhere in the module, including in
    a comment — a leftover mention would still document a dimension `_DIM_TO_PATH` never had."""
    print("\n[12] no dead dimension name survives in the module source")
    import io
    src = io.open(os.path.join(REPO, "src", "engine", "wound.py"), encoding="utf-8").read()
    check("no-literal-novelty", "novelty" not in src)
    check("no-literal-play_cue", "play_cue" not in src)


class _R:
    """A reading as the seat produces it: path, rung, about."""
    def __init__(self, path, about, rung="unease"):
        self.path, self.about, self.rung = path, about, rung


def test_about_for_reads_the_wounds_own_concept_on_its_own_path():
    """The identity route's input, built from the readings (gate wounds-and-memory-inputs, 2026-09-22).
    Both drivers passed the beat's resolved SUBJECT, a person, so this route could never match live."""
    w = {"id": "thunder@WARINESS", "concept": "thunder", "path": "WARINESS"}
    check("concept-on-its-own-path-reaches-it",
          wound.about_for(w, [_R("WARINESS", "concept:thunder")]) == "concept:thunder")
    check("the-same-concept-on-another-path-does-not",
          wound.about_for(w, [_R("DEFLATION", "concept:thunder")]) is None)
    check("its-own-path-about-a-person-does-not",
          wound.about_for(w, [_R("WARINESS", "vell")]) is None)
    check("one-matching-reading-among-others-is-enough",
          wound.about_for(w, [_R("WARINESS", "vell"), _R("WARINESS", "concept:thunder")]) == "concept:thunder")
    check("no-readings-no-about", wound.about_for(w, []) is None)
    check("a-malformed-wound-has-no-about", wound.about_for({"id": "x"}, [_R("WARINESS", "concept:x")]) is None)
    check("the-returned-about-fires-the-identity-route",
          wound.fires(dict(w, trigger=[]), {"about": wound.about_for(w, [_R("WARINESS", "concept:thunder")]),
                                            "surfaces": []}))
