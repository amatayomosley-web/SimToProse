...
CHAR_ENGINE = {
    "fixed": {"name": "Mira", "genotype": {
        "STIRRING": {"hit": "elevated"}, "DISPLEASURE": {"hit": "low"}}},
    "baseline": {
        "temperament": {p: {"mean": m, "variability": 0.1} for p, m in
                        zip(PATHS, (0.6, 0.4, 0.2, 0.2, 0.5, 0.3, 0.35, 0.2))},
    ...
    "current": {
        # DERIVED over PATHS with per-primitive overrides. A zip against a fixed tuple
        ...
        "affect": dict({p: 0.30 for p in PATHS},
                       # LUST 0.2 folded into STIRRING (which keeps SEEKING's 0.6, the general
                       # wanting) and PLAY 0.35 dropped with the pool -- LEVITY is not built.
                       STIRRING=0.6, WARINESS=0.4, DISPLEASURE=0.2, GOODWILL=0.5),
