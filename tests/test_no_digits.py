...
WHAT IT BANS, EXACTLY. Decimals, and only in what the actor is TOLD.
  ...
  * AUTHORED PROSE is the engine's business only to REPORT, never to rewrite. A real book was found
    carrying design notes inside string values — multipliers written as decimals mid-sentence — and those
...
def _fixture():
    ...
    char = {
        "fixed": {"id": "wren", "name": "Wren", "role_tier": 2,
                  "genotype": {"WARINESS": {"rest": "raised", "hit": "high", "hold": "lasting"},
                               "DISPLEASURE": {"hit": "typical"}}},
        "baseline": {
            "temperament": {p: {"mean": 0.4, "variability": 0.12} for p in PATHS},
            ...
            "drives": {"goals": [{"goal": "bring the goats down before the storm", "urgency": 0.93},
                                 {"goal": "keep the shop open", "urgency": 0.35}],
                       "orientation": "she takes the practical option and regrets it later"},
            # ENGINE WOUNDS (gate three): intensity is a number the renderer must band, never leak
            "wounds": [{"id": "drowning@WARINESS", "concept": "drowning", "path": "WARINESS", "intensity": 0.88,
                        "source": "profile:t", "text": "the dark water", "trigger": ["water", "dark"]},
                       {"id": "shame@SELF-REGARD", "concept": "shame", "path": "SELF-REGARD", "intensity": 0.21,
                        "source": "profile:t", "text": "being laughed at"}],
            "voice": {"register": "plain, short sentences"},
            "relationship_priors": {"default_trust": 0.45},
            "provenance": {"temperament.FEAR": "the flood, age nine"}},
        "current": {"affect": {p: 0.45 for p in PATHS},
                    ...
                    "active_goals": [{"goal": "bring the goats down before the storm", "urgency": 0.93}]}}
...
_NON_ACTOR_SURFACES = {
    ...
    "scripts/composition_pass.py": (
        ...
        "build_attach_classify_prompt (gate 5) is shown the four relation words and NEVER the price "
        "table; tests/test_composition_phase_a.py asserts no digit in its rendered text."),
    "scripts/narrate.py": (
    ...
    "scripts/appraiser.py": (
        "build_emotion_messages / build_event_messages — the two APPRAISER seats read a beat and "
        "report what it was. Neither is playing anyone, and neither is TOLD anything about a "
        "character: the emotion seat sees the beat's action and thought plus the eight ladders, the "
        "event seat sees the observable act alone. Hard rule 5 binds what a character is told about "
        "THEMSELVES, and a sensor is told nothing about anyone. The prompts still carry no number, "
        "and tests/test_appraiser.py asserts that against the RENDERED text plus a signature check "
        "that the builders have nowhere to put stored state."),
    "scripts/critic.py": (
...
def test_the_information_survives():
    ...
    for name, needle in (("the-goal-is-still-named", "bring the goats down before the storm"),
    ...
    # THE LIST ABOVE SHRANK ON 2026-09-06 and the losses are the point, not an erosion of this test.
    # `trigger`, `orientation` and `provenance` no longer reach the actor: the first two are rate and
    # routing fields the appraisal already spends, and provenance is the authoring rationale for a
    # number. `scene._build_stable` cuts them; `tests/test_prompt_manner.py` asserts their absence.
    # What this test still guards is unchanged — that stripping the DIGITS did not strip the
    # CHARACTER, and biography, goal, voice and percept are all still here in words.

    # THE VALUE RANKING is exercised directly now, because the worth menu is a RATE and no longer
    # rides the prefix. The renderer stays and is still worth testing: `direct_identity` keeps its
    # phrase tables so a field nobody has written a renderer for is still refused rather than
    # silently dropped, and that guard is what these four checks protect.
    char_for_worth, _w = _fixture()
    ident = direct_identity({"model": char_for_worth["baseline"]["model"]})
...
def test_no_new_leak_creeps_back():
    ...
    # THE GUARD IS TESTED AT THE FUNCTION, NOT THROUGH THE PACKET — and that changed on 2026-09-06
    # when `scene._build_stable` stopped shipping the rate blocks. The renderer KEPT its tables on
    # purpose: it is the thing that refuses a number nobody has written a phrase for, and a guard
    # reached only by a path that no longer exists is a guard that has been deleted without anyone
    # saying so. So this hands `direct_identity` the same sheet the prefix used to hand it.
    out = direct_identity({"model": char["baseline"]["model"],
                           "drives": char["baseline"]["drives"],
                           "voice": char["baseline"]["voice"]})
    blob = json.dumps(out)
    ...
    for name, needle in (("priority-is-said", "nothing you want outranks this"),
                         ...
                         ("assertiveness-is-said", "you do not qualify anything you say"),
    ...
    try:
        direct_identity({"model": char["baseline"]["model"]})
