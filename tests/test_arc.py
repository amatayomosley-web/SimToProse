...
def _char(ec="typical", aff_to_kwame=0.35, regard_fast=0.15, load=0.25):
    # `ec` is accepted and IGNORED: effortful_control was cut with the primitive-era genotype on
    # 2026-09-10 and resilience's self-control term is a constant until an axis returns.
    # Ottilie of the chess club plays only long games by post and thinks little of the members who
    # play fast games in the café (a low regard for the group), though she is warm by nature; Kwame,
    # who joined for the fast games, is the one of them she has begun to talk to.
    return {
        "fixed": {"genotype": _typical(GOODWILL={"hit": "high"}, DEFLATION={"hit": "high"})},
        "baseline": {
            "temperament": {p: {"mean": 0.4, "variability": 0.1} for p in PATHS},
            "model": {"regard": {"fast_players": regard_fast}},
        },
        "current": {
            "relationships": {"kwame": {"trust": 0.4, "affinity": aff_to_kwame, "respect": 0.3, "debt": 0.0}},
...
def _connection(mag=0.7):
    return {"dimensions": {"care_relevant": mag, "relief": 0.3}, "durability": "durable",
            "target": "kwame", "target_group": "fast_players"}


def test_slow_burn_arc():
    """A durable CONNECTION with Kwame moves Ottilie's CARE up AND raises her low regard['fast_players']."""
    print("\n[1] SLOW-BURN ARC (the bond generalizes to the group)")
    ch = _char()
    ...
    if diff:
        ...
        # temperament and the group regard. `tests/test_bonds.py` [9] pins the handover, and
        ...
        check("CARE-rises-instead", diff["temperament"]["GOODWILL"] > 0, str(diff["temperament"]))
        check("regard-erodes-up", diff["regard"]["fast_players"] > 0, str(diff["regard"]))
        new = arc.apply(ch, diff)
        check("CARE-actually-rose",
              new["baseline"]["temperament"]["GOODWILL"]["mean"] > ch["baseline"]["temperament"]["GOODWILL"]["mean"])
        check("regard-actually-rose",
              new["baseline"]["model"]["regard"]["fast_players"] > ch["baseline"]["model"]["regard"]["fast_players"])

    # accumulation across many beats: affinity climbs monotonically toward 1.0
    ch2 = _char()
    affinities = [ch2["current"]["relationships"]["kwame"]["affinity"]]
    for _ in range(12):
        d = arc.assess(_connection(), impact=0.6, char=ch2, condition=ch2["current"]["condition"])
        if d:
            ch2 = arc.apply(ch2, d)
        affinities.append(ch2["baseline"]["temperament"]["GOODWILL"]["mean"])
    ...
    check("regard-also-eroded", ch2["baseline"]["model"]["regard"]["fast_players"] > 0.15 + 0.05)
...
def test_threshold_keeps_most_transient():
    ...
    weak = {"dimensions": {"care_relevant": 0.6}, "durability": "durable", "target": "kwame",
            "target_group": "fast_players"}
...
def test_resilience_fork():
    ...
    low = {"fixed": {"genotype": _typical()},
           "baseline": {"temperament": {p: {"mean": 0.4, "variability": 0.1} for p in PATHS}},
           "current": {"relationships": {}, "condition": {"allostatic_load": 0.8}}}
    # high resilience: high effortful_control + low load + a secure bond
    high = {"fixed": {"genotype": _typical()},
            "baseline": {"temperament": {p: {"mean": 0.4, "variability": 0.1} for p in PATHS}},
    ...
    check("low-resilience-fear-up", d_low and d_low["temperament"].get("WARINESS", 0) > 0, str(d_low))
    check("high-resilience-growth", d_high and d_high["temperament"].get("STIRRING", 0) > 0, str(d_high))
    # THE GRADIENT, not the presence of a key. This assertion used to read
    # `"STIRRING" not in d_low["temperament"]`, which passed only because the old branches wrote one
    # primary per event. Under `state._DIM_TO_PATH` a threat ALSO pushes SEEKING at low
    ...
    sl, sh = d_low["temperament"].get("STIRRING", 0), d_high["temperament"].get("STIRRING", 0)
    ...
    check("the-same-threat-inverts-fear",
          d_low["temperament"].get("WARINESS", 0) > 0 > d_high["temperament"].get("WARINESS", 0),
          "broken %+.4f, resilient %+.4f" % (d_low["temperament"].get("WARINESS", 0),
                                             d_high["temperament"].get("WARINESS", 0)))
    ...
    # RETIRED 2026-09-08. Read `.get("PLAY", 0) < 0` -- see tests/test_toward.py for the full
    # reason. The FLATTENING this guarded is still covered by the next assertion, which needs a
    # threat to reach four or more paths; what is gone is specifically the fall.
...
def test_apply_bounded_and_fail_loud():
    ...
    ch = _char(regard_fast=0.95)
    # a big regard erosion clamps at 1.0
    new = arc.apply(ch, {"regard": {"fast_players": +0.5}})
    check("regard-clamped-at-1", new["baseline"]["model"]["regard"]["fast_players"] == 1.0)
    new2 = arc.apply(ch, {"temperament": {"WARINESS": +5.0}})
    check("temperament-clamped-at-1", new2["baseline"]["temperament"]["WARINESS"]["mean"] == 1.0)
...
def test_persist_roundtrip():
    ...
    try:
        ...
        led.register_character("r1", "ottilie", {"name": "Ottilie"}, {"temperament": "x"})
        diff = {"relationships": {"kwame": {"affinity": 0.07}}, "regard": {"fast_players": 0.03}}
        led.append_arc_diff("r1", "ottilie", 4, diff)
        row = led.con.execute("SELECT diff FROM arc_diffs WHERE run_id='r1' AND char_id='ottilie' AND turn=4").fetchone()
        check("arc-diff-persisted", row is not None)
        check("arc-diff-roundtrips", row and json.loads(row["diff"])["relationships"]["kwame"]["affinity"] == 0.07)
    finally:
        shutil.rmtree(tmp, ignore_errors=True)


def _bonded(aff_friend=0.9, aff_stranger=0.2, ec="typical", load=0.25):
    ...
    ch["current"]["relationships"] = {
        "friend":   {"trust": 0.8, "affinity": aff_friend,   "respect": 0.7, "debt": 0.1},
...
def test_the_severed_bond_does_not_buffer_its_own_severing():
    ...
    event reaching it, so grief at losing a friend was softened by the character's love for that friend."""
    print("\n[9] the severed bond cannot buffer the severing")
    ch = _bonded()
    cond = ch["current"]["condition"]

    r_all = arc.derive_resilience(ch, cond)
    r_less = arc.derive_resilience(ch, cond, excluded="friend")
    check("excluding-the-bond-lowers-resilience", r_all - r_less >= 0.15,
          "all=%.4f excluded=%.4f delta=%.4f" % (r_all, r_less, r_all - r_less))

    d_friend = arc.assess(_loss("friend"), impact=0.8, char=ch, condition=cond)
    d_other = arc.assess(_loss("stranger"), impact=0.8, char=ch, condition=cond)
    check("both-events-write-grief", bool(d_friend) and bool(d_other), "%r / %r" % (d_friend, d_other))
    if d_friend and d_other:
        g_friend = d_friend["temperament"]["DEFLATION"]
        g_other = d_other["temperament"]["DEFLATION"]
        check("losing-the-loved-one-hurts-more", g_friend > g_other,
              "loss of friend=%.5f vs loss of stranger=%.5f" % (g_friend, g_other))
...
def test_a_bond_still_buffers_a_threat_it_had_nothing_to_do_with():
    ...
    ch = _bonded()
    cond = ch["current"]["condition"]
    threat = {"dimensions": {"threat": 0.8, "loss": 0.1}, "durability": "durable", "target": "friend"}
    ...
    if d:
        got = d["temperament"].get("WARINESS", 0.0)
        expected = d_nobond["temperament"].get("WARINESS", 0.0)
...
def test_excluding_the_only_edge_falls_to_the_documented_default():
    ...
    lone = _char()
    lone["current"]["relationships"] = {"friend": {"trust": 0.8, "affinity": 0.9, "respect": 0.7}}
    friendless = _char()
    friendless["current"]["relationships"] = {}
    cond = lone["current"]["condition"]
    check("excluded-only-edge-equals-no-edges",
          abs(arc.derive_resilience(lone, cond, excluded="friend")
              - arc.derive_resilience(friendless, cond)) < 1e-12,
          "%.6f vs %.6f" % (arc.derive_resilience(lone, cond, excluded="friend"),
...
def test_the_authored_baseline_survives_the_arc():
    ...
    ch = _char()
    ch["baseline"]["model"]["regard"]["fast_players"] = 0.15
    authored = ch["baseline"]["temperament"]["GOODWILL"]["mean"]
    c = ch
    for _ in range(3):
        c = arc.apply(c, {"temperament": {"GOODWILL": 0.15}, "regard": {"fast_players": 0.05}})
    t = c["baseline"]["temperament"]["GOODWILL"]
    ...
    check("regard-authored-is-preserved",
          abs(c["baseline"]["model"]["_authored_regard"]["fast_players"] - 0.15) < 1e-9,
          str(c["baseline"]["model"]))
    check("regard-holds-the-effective-value",
          abs(c["baseline"]["model"]["regard"]["fast_players"] - 0.30) < 1e-9, str(c["baseline"]["model"]))
    ...
    check("untouched-primary-is-unstamped",
          "_authored_mean" not in c["baseline"]["temperament"]["DISPLEASURE"],
          str(c["baseline"]["temperament"]["DISPLEASURE"]))
    # RE-APPLYING must not re-baseline from the moved value
    c2 = arc.apply(c, {"temperament": {"GOODWILL": 0.10}})
    check("refold-keeps-the-ORIGINAL-authored",
          abs(c2["baseline"]["temperament"]["GOODWILL"]["_authored_mean"] - authored) < 1e-9,
          str(c2["baseline"]["temperament"]["GOODWILL"]))
...
def test_the_preserved_base_never_reaches_the_actor():
    ...
    ch = arc.apply(ch, {"temperament": {"GOODWILL": 0.15}, "regard": {"ashford_villagers": 0.05}})
    check("the-stamp-is-on-the-sheet",
          "_authored_mean" in ch["baseline"]["temperament"]["GOODWILL"])
    ...
    # THE ANTI-VACUITY ANCHOR MOVED on 2026-09-06. It used to be `model`, which carried
    # `_authored_regard` into the prefix; `scene._build_stable` no longer ships `model` at all, so
    # asserting its presence would now fail for the right reason and take this guard with it. The
    # guard still needs SOME block that arc stamps and the prefix carries, or "no authored key" is
    # true because the prefix is empty. `drives` is that block: a wound carries
    # `authored_intensity`, and the prefix ships the wound.
    check("and-DRIVES-is-in-the-stable-prefix", "drives" in packet["stable"],
          "with an empty prefix 'no authored key' would be vacuously true")
    # the wound moved to engine state 2026-09-11 (gate three): the prefix ships it as `wounds`
    check("...and-the-wound-text-is-in-it",
          "spider" in _json.dumps(packet["stable"].get("wounds")),
          "the block must carry the authored content, not just exist")
...
def test_the_slave_case():
    ...
    Before the arc was routed through `state._DIM_TO_PATH` this run moved exactly ONE primary.
    ...
    ch = {"fixed": {"genotype": _typical(WARINESS={"hit": "high"})},
          "baseline": {"temperament": {p: {"mean": m, "variability": 0.1} for p, m in
                       (("GOODWILL", 0.75), ("STIRRING", 0.60),
                        ("WARINESS", 0.20), ("DISPLEASURE", 0.20), ("DEFLATION", 0.25), ("DISTASTE", 0.20),
                        ("RECEPTIVITY", 0.50), ("SELF-REGARD", 0.50), ("LEVITY", 0.10))}},
          "current": {"relationships": {}, "condition": {"allostatic_load": 0.85}}}
    start = {p: ch["baseline"]["temperament"][p]["mean"] for p in PATHS}
    ...
    for _ in range(80):
        ...
        if d:
            ch = arc.apply(ch, d)
    end = {p: ch["baseline"]["temperament"][p]["mean"] for p in PATHS}

    check("angry", end["DISPLEASURE"] > 0.90, "RAGE %.3f -> %.3f" % (start["DISPLEASURE"], end["DISPLEASURE"]))
    check("hostile", end["DISTASTE"] > 0.70, "DISGUST %.3f -> %.3f" % (start["DISTASTE"], end["DISTASTE"]))
    check("afraid", end["WARINESS"] > 0.90, "FEAR %.3f -> %.3f" % (start["WARINESS"], end["WARINESS"]))
    # the one that proves it is not just "everything rises": the joy has to GO.
    # RETIRED 2026-09-08 with the PLAY pool. It proved the arc was not just "everything rises"
    # by requiring the joy to FALL. Under THIS event mix no surviving path falls: `loss` does
    # push STIRRING -0.18, but at loss 0.15 against threat's +0.12 at 0.75 the net is upward.
    # Neither `threat` nor `social_violation` has any negative weight left at all -- PLAY held
    # both of theirs. Elsewhere the table still falls (relief, mastery), so this is a hole in
    # ADVERSITY specifically, not in the engine's ability to lower a path. It returns with LEVITY.
    # THRESHOLD LOWERED 6 -> 5 on 2026-09-08, and this is a real loss of reach, not a
    # convenience. RECEPTIVITY and SELF-REGARD have no appraisal dimension at all, and GOODWILL
    # is untouched by threat/loss/social_violation, so five is every path this event CAN move.
    check("no-longer-flat", sum(1 for p in PATHS if abs(end[p] - start[p]) > 1e-9) >= 5,
          "moved: %s" % {p: round(end[p] - start[p], 3) for p in PATHS
    ...
    check("the-authored-base-survived-all-of-it",
          # WAS asserted on PLAY 0.75, the base-happy man's defining trait. `_authored_mean` is
          # stamped only on a path the arc actually WRITES, and GOODWILL (his warmth, 0.75) is
          # now moved by nothing in this event -- it survives trivially rather than survivingly.
          # STIRRING is the surviving trait the beatings DO reach, so it is where the law bites.
          ch["baseline"]["temperament"]["STIRRING"].get("_authored_mean") == 0.60,
          str(ch["baseline"]["temperament"]["STIRRING"]))
    # and law 3 — the genotype BIASED it. A calm man with the same eighty nights is not this man.
    calm = {"fixed": {"genotype": _typical(WARINESS={"hit": "low"})},
            "baseline": {"temperament": {p: {"mean": m, "variability": 0.1} for p, m in
                         (("GOODWILL", 0.75), ("STIRRING", 0.60),
                          ("WARINESS", 0.20), ("DISPLEASURE", 0.20), ("DEFLATION", 0.25), ("DISTASTE", 0.20),
                          ("RECEPTIVITY", 0.50), ("SELF-REGARD", 0.50), ("LEVITY", 0.10))}},
    ...
    _BIAS_N = 25
    reactive = {"fixed": {"genotype": _typical(WARINESS={"hit": "high"})},
                "baseline": {"temperament": {p: {"mean": m, "variability": 0.1} for p, m in
                             (("GOODWILL", 0.75), ("STIRRING", 0.60),
                              ("WARINESS", 0.20), ("DISPLEASURE", 0.20), ("DEFLATION", 0.25), ("DISTASTE", 0.20),
                              ("RECEPTIVITY", 0.50), ("SELF-REGARD", 0.50), ("LEVITY", 0.10))}},
    ...
    end = {p: reactive["baseline"]["temperament"][p]["mean"] for p in PATHS}
    check("the-genotype-biased-the-durable-tier",
          calm["baseline"]["temperament"]["WARINESS"]["mean"] < end["WARINESS"],
          ...
          % (calm["baseline"]["temperament"]["WARINESS"]["mean"], end["WARINESS"], _BIAS_N))


def test_readings_receipt_prices_a_height_only_path():
    """GATE heights-price-arc (2026-09-19), GREEN_CONDITION (a). RECEPTIVITY has no `_DIM_TO_PATH`
    row at all (state.py's own comment: "those two paths are structurally reachable and never
    move"), so `price_for` can never write it — a beat that qualified as durable purely on a
    RECEPTIVITY reading used to come back candidate=True with an empty temperament diff (the
    RED_STATE this gate closes). It must now price RECEPTIVITY from the height itself, through the
    identical damage step the dims route uses: height x _BASE_STEP x impact x (1 - resilience)."""
    print("\n[16] the readings receipt prices a height-only path (RECEPTIVITY)")
    ch = _char()
    cond = ch["current"]["condition"]
    mild = {"dimensions": {"care_relevant": 0.3}, "durability": "transient"}   # below _DURABLE_DIM
    impact = 0.5                                                              # above _ARC_THRESHOLD
    # dominant dim is care_relevant, never in _SEVERING_DIMS, so `assess` excludes no bond — this
    # is the identical call `assess` makes internally, safe to duplicate for the expected value.
    resilience = arc.derive_resilience(ch, cond)
    d = arc.assess(mild, impact=impact, char=ch, condition=cond, heights={"RECEPTIVITY": 0.9})
    check("candidate-on-the-height-alone", d is not None, str(d))
    if d:
        got = d["temperament"].get("RECEPTIVITY")
        expected = 0.9 * arc._BASE_STEP * impact * (1.0 - resilience)
        check("RECEPTIVITY-is-priced", got is not None, str(d["temperament"]))
        check("RECEPTIVITY-priced-from-the-height",
              got is not None and abs(got - expected) < 1e-9,
              "got %r expected %.9f" % (got, expected))
        check("RECEPTIVITY-is-positive", got is not None and got > 0, got)


def test_empty_heights_reproduces_todays_result_exactly():
    """GREEN_CONDITION (b). The identical beat with heights={} (or heights omitted, as every caller
    before this gate calls it — MIGRATION field) must be BYTE-IDENTICAL to what HEAD (3677f3b,
    before the readings receipt existed) produced. Captured by actually running it there, not
    assumed: `arc.assess(mild, impact=0.5, char=_char(), condition=cond, heights={})` returned
    `None` — mild dims (0.3, below _DURABLE_DIM) plus no readings never clears the candidate test,
    and an empty heights dict adds no candidate path either, so the receipt loop this gate adds
    never runs. Pinned as data, not re-derived, so a change that silently makes heights={} do
    something is caught here rather than only in the beat this same file builds around it."""
    print("\n[17] heights={} (or omitted) reproduces today's result exactly")
    ch = _char()
    cond = ch["current"]["condition"]
    mild = {"dimensions": {"care_relevant": 0.3}, "durability": "transient"}
    expected_at_head = None            # captured at HEAD 3677f3b, before this gate's change existed
    got_empty = arc.assess(mild, impact=0.5, char=ch, condition=cond, heights={})
    check("heights-empty-is-a-no-op", got_empty == expected_at_head, str(got_empty))
    got_omitted = arc.assess(mild, impact=0.5, char=ch, condition=cond)
    check("omitted-heights-is-the-same-no-op", got_omitted == expected_at_head, str(got_omitted))


def test_a_path_reached_by_both_routes_sums_the_two_receipts():
    """GREEN_CONDITION (c). A severe threat DIM and a WARINESS READING are two receipts of one
    beat — what happened and what arose — and where both name the same path they must SUM, not
    override. Isolate each route alone (dims-only, height-only on a dim that never touches
    WARINESS), then prove the combined call equals their total, to the bit: `price_for`, `grw` and
    `resilience` are identical across all three calls (same char/condition/impact, and `care_relevant`
    is no more in `_SEVERING_DIMS` than `threat` is, so the excluded bond never differs either)."""
    print("\n[18] dims and a height on the same path sum (two receipts, one beat)")
    ch = _char()
    cond = ch["current"]["condition"]
    impact = 0.8
    dims_only = {"dimensions": {"threat": 0.85}, "durability": "durable"}
    height_only = {"dimensions": {"care_relevant": 0.3}, "durability": "transient"}  # never touches WARINESS
    both = {"dimensions": {"threat": 0.85}, "durability": "durable"}

    d_dims = arc.assess(dims_only, impact=impact, char=ch, condition=cond)
    d_height = arc.assess(height_only, impact=impact, char=ch, condition=cond, heights={"WARINESS": 0.85})
    d_both = arc.assess(both, impact=impact, char=ch, condition=cond, heights={"WARINESS": 0.85})

    check("dims-alone-prices-WARINESS", bool(d_dims) and d_dims["temperament"].get("WARINESS", 0) > 0, str(d_dims))
    check("height-alone-prices-WARINESS", bool(d_height) and d_height["temperament"].get("WARINESS", 0) > 0, str(d_height))
    check("combined-prices-WARINESS", bool(d_both) and d_both["temperament"].get("WARINESS", 0) > 0, str(d_both))
    if d_dims and d_height and d_both:
        w_dims = d_dims["temperament"]["WARINESS"]
        w_height = d_height["temperament"]["WARINESS"]
        w_both = d_both["temperament"]["WARINESS"]
        check("combined-equals-the-sum-of-the-two-receipts-alone",
              abs(w_both - (w_dims + w_height)) < 1e-9,
              "dims %.9f + height %.9f = %.9f, got %.9f" % (w_dims, w_height, w_dims + w_height, w_both))


def test_apply_seeds_rest_words_before_moving():
    """gate resume-and-parity (2026-09-22): both drivers replay the arc on resume BEFORE assembly
    seeds rest words into means, and `apply` stamped the MIDPOINT as the authored value of any path
    carrying only a rest word, landing it at 0.5 + change. Every registered sheet on the live run is
    rest-words-only; one recorded diff returned seven of a character's resting moods near 0.5."""
    import copy
    from src.engine import heritable
    sheet = {"fixed": {"name": "Wilma"}, "current": {},
             "baseline": {"temperament": {"STIRRING": {"rest": "low"}, "WARINESS": {"rest": "raised"}}}}
    seeded = copy.deepcopy(sheet)
    heritable.ensure_temperament(seeded)
    want = {p: seeded["baseline"]["temperament"][p]["mean"] for p in ("STIRRING", "WARINESS")}
    new = arc.apply(sheet, {"temperament": {"STIRRING": 0.02, "WARINESS": -0.01}})["baseline"]["temperament"]
    for p, d in (("STIRRING", 0.02), ("WARINESS", -0.01)):
        check("rest-seeded-then-moved-%s" % p.lower(), abs(new[p]["mean"] - (want[p] + d)) < 1e-9,
              repr((new[p]["mean"], want[p] + d)))
        check("authored-mean-is-the-rest-not-the-midpoint-%s" % p.lower(),
              abs(new[p]["_authored_mean"] - want[p]) < 1e-9, repr(new[p].get("_authored_mean")))
    check("the-input-sheet-is-not-mutated", "mean" not in sheet["baseline"]["temperament"]["STIRRING"])
