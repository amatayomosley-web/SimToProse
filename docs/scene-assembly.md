...
Perception is the **live edge** of the vault; the vault is the **stored body**; each turn the edge becomes body via an acquisition event. "How does a world fact become a character belief mid-sim?" — **it is perceived, and perceiving is acquiring.** ⚠ **ASPIRATIONAL, marked 2026-09-19 — the code does not meet this yet.** The sentence below describes what a *false* perception SHOULD do; what `src/engine/gate.py` actually does is produce ABSENCE on a failed perception check (no belief at all about the thing missed), never a wrong belief — pinned by `tests/test_gate.py` (`test_absence_on_failed_check` and the failed-perception-wall cases). This is CLAUDE.md's known gap, restated here: "a failed perception check yields NO belief about an act rather than a wrong one (the false-belief layer is unbuilt engine-wide)". The design intent stands (below); only its BUILT status changes:
> A *false* perception (a disguise that works, a misheard line) enters as a belief carrying witnessed-provenance that may be wrong — exactly the false-belief machinery the vault already stores (`knowledge-model.md` "vault stores BELIEFS, not facts"). No separate coupling mechanism is needed; **perception *is* the coupling.**
...
- **Persona, voice, goal TEXT and wound TEXT — and nothing else.** *(Narrowed 2026-09-06; this line
  read "Persona / disposition / values / drives / the Model" and shipped all of it.)* Layer-7 state
  is volatile by definition and lives only below. *(Fixed 2026-06-10 — audit B8: the earlier
  "1·3·5·7" listed state in both halves.)*

  **A RATE IS SPENT ONCE.** `state.build_profile` reads `fixed.genotype`, the trait means and
  `baseline.model` and turns them into gains, relevance and regard — how hard an event moves this
  person. Putting the same fields in the prefix rendered them to the actor as sentences too, so a
  character was damped once in the arithmetic and again in the prose. The path model gives every
  emotion one axis and a rung on it, and a rung has to mean the same thing for two people or the
  scale is not shared — let the sheet speak twice and rung 9 is assault for one character and mild
  irritation for another. So: **genotype, traits, model and provenance do not reach the actor**,
  and inside `drives` only the goal text and the wound text do. The Layer-3 and Layer-10 operands
  still exist and are still authored; they enter through `build_profile`, `levers` and `wound`.

  **The two prongs, in order.** (1) RATE — if the engine reads the field as a number, the actor
  never sees it; grep-checkable. (2) AXIS — of what is left, a line stays only if it makes no claim
  a path also makes, i.e. it holds at every one of the 92 rungs. Voice survives: a laconic man is
  laconic when furious and when bored. `effortful_control` (an allele cut 2026-09-10) did not: "you can set anything aside
  and mean it" is contradicted outright by DISPLEASURE 9. Pinned by `tests/test_prompt_manner.py`.
