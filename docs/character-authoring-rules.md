...
engine had never fired once, in a book whose belief *prose* was rich. Examples use invented
books so this file stays upstreamable to the template.
...
- **The Violation**: `- His mother drowned at the ford the spring it flooded, and he watched from the bank.`
  Beautiful, load-bearing, and invisible to the machine.
- **The Valid Form**: `- (1.0, he watched it) His mother drowned at the ford the spring it flooded,
  and he watched from the bank [[The_Ford]]`
...
- **The Violation**: `"formative": {"culture": "Brightwell touring circus (the tent up by dusk, every
  act rehearsed twice)"}`. Precise, evocative, and invisible. The actor playing this character
...
- **The Valid Form**: put it in `fixed.position.class` / `.place`, written as **self-sufficient
  ...
  `"class": "the circus rigger — first up the king pole at every new ground and the last word on whether
  an act goes on. The performers trust their necks to her knots, and every one of them knows it."`
...
- **The Violation, measured in a live book**: three supporting characters carried
  `role_tier: "principal"` while the **protagonist and their first scene partner carried no tier
...
- **The Valid Form**: Name the nouns your scenes will use — *the quiz*, *the picture round*, *the
  league*, *the tie-break*. If a belief is about losing the league, the word "league" or "tie-break" must be in it.
...
- **The Violation**: Two dance teachers with twenty years in the same studio and
...
- **The Valid Form**: Author the edge on **both** sides, keyed to the exact `world.people` id, with
  `debt` carrying real weight where one of them owes the other something.
...
- **The Violation**: Tuning `conscientiousness` from 0.6 to 0.7 expecting behaviour to move.
...
- **The Law**: `baseline.voice` and `orientation` are passed to the LLM **verbatim** in the stable
  prefix. The engine never parses them. (Wounds left this rule 2026-09-11: `drives.fears_wounds` is
  retired and refused by lint; a wound is engine state at `baseline.wounds` and reaches the actor
  RENDERED — the concept's gloss, what happened, the intensity as a sentence — never as your prose.)
...
- **The Violation**: A marathon runner whose `current.location` is the finish line, where the
  book ends, while chapter one opens at her kitchen table.
