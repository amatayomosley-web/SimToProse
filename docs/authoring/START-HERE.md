...
- **`baseline.model` weights** (schwartz / moral_foundations / needs) — only the *order* matters. A
  ...
  out.
- **`baseline.temperament` and `current.affect`** — nine paths each (LEVITY joined 2026-09-11), all required. Use the
...
**One warning is expected and is not your fault.** `lint_book.py` will always list the acts your
laws are keyed to (informational). (It used to warn that each wound's trigger had no catalog row;
since 2026-09-11 a wound is engine state — minted from a formative profile, matched by concept —
and the old `fears_wounds` block is refused outright, with the fix named.)
