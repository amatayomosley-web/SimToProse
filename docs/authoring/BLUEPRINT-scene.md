...
|---|---|
| "One potter explains to the other why the shared firing failed." | "The kiln is open on a shelf of cracked bowls with her mark scratched under most of them, and the potter she shares it with is already loading the next firing without looking up." |
...
## 6b. What changes hands for good — the director's holds (optional)

**Write in the cfg as:** `attachments` — a list of `{"char": <cast id>, "entity": "loc.<id>" | "grp.<tag>", "relation": "life" | "post" | "member" | "acquainted" | "none", "note": "<why>"}`

Only when THIS scene changes what someone holds: dismissed from the post (`none`), the ship sinks
(`none`), the oath taken (`post`). A relation WORD, never a number — the engine prices it and writes an
append-only row at the scene's first turn (`bond-arithmetic.md` s3). Most scenes have none.

## 6c. How worn someone arrives — the director's condition (optional)

**Write in the cfg as:** `condition` — a list of `{"char": <cast id>, "energy": "spent" | "tired" | "steady" | "fresh", "stress": "calm" | "tense" | "strained" | "frayed", "gap": "rested" | "awake"}` (any of the three)

Only when the story says how someone ARRIVES and the engine would not otherwise know it: she rode all
night, he has not slept in three days. The words are the engine's (`src/engine/condition.py`) — each energy
word is the line the actor is then told: *spent* "you take the shortest path", *tired* "you do what is
asked and none of the extra", *steady* "you can do the thorough version where it matters", *fresh* "you
have reserve to spend". It is applied after the time since the last scene has done its work, so it is
the state AT the opening.

**The time since their last scene — the standard, and when to write `gap`.** If the book runs `condition_flow`,
the engine measures each character's OWN gap, from the end of the last scene they were in to this one's
opening, and follows the standard day: the night hours (22:00 to 06:00) are sleep, every other hour they were
awake. So a POV cut of minutes is minutes awake, and time spent off the page in someone else's scene is lived.
No mention is the standard; anything written supersedes it (the owner, 2026-09-22): write `gap: rested` when
they rested by day, `gap: awake` when they were up all night.

If the book runs `condition_flow` (the character blueprint's Part Zero), the engine carries energy and
stress on from there, beat by beat; if it does not, the stated condition simply holds until someone
states another. A book whose world switches the `condition` system off cannot use this field — the
pre-run check (`scripts/lint_scene.py`) and the driver both refuse it. Most scenes have none.
...
The person is a character id — often one of the cast, but it can be someone absent. A bandmate
who quit by text last night is the subject of a scene she is not in. The group is a label the world note attaches
...
> `_DIM_TO_PATH` in `src/engine/state.py:53` and following, which is exactly
...
## 11. REQUIRED: when the scene opens — and, optionally, how long it runs

**Write in the cfg as:** `at` — REQUIRED on every scene since 2026-09-10 — and `lasts`, optional.

```json
"at":    {"day": 12, "time": "21:00"},
"lasts": "90m"
```
...
`at:`     day ________  time ____:____

`lasts:`  __________________________ *(optional — minutes, or 2h / 1d)*
