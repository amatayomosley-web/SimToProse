...
**A worked YES.** Premise: *a valley where every debt is carved on the debtor's door.* If the
doors cut the marks themselves, you would set `magic: true` (and leave `beings: false` — nothing
here is a being), and then owe a supernatural law such as: *the mark cannot be cut away, burned
off or left behind — a new door carries it by morning; it records the sum, never the creditor; it
cannot collect anything.* Now the world can refuse a lever, and the scene where somebody tries to
burn the door and walk away from what they owe has a rule to be refused by
(`docs/universal-law.md:58-60` works a supernatural switch through the same way).
...
**A worked pair.** Same premise — *every debt is carved on the debtor's door* — two books:

| `epistemic` | what the book becomes |
|---|---|
| `known-true` | a valley under a ledger no one can falsify. The doors really do cut themselves. |
| `known-false` | a people governed by a trick. The reeve's men cut the marks by night, and the engine will never let the doors' "power" constrain anything. |

That single word is the whole difference (`docs/universal-law.md:60` makes the same argument).
...
unscoped check made every law bear and nearly all of them deny, so every scene would have been
