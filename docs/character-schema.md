...
| Field | What | Defined in |
|---|---|---|
| `temperament` | the 7 primaries' resting levels — each `{mean, variability}` | `state-engine.md` (tier 1), `generative-model.md` |
...
| Field | What | Defined in |
|---|---|---|
| `affect` | the 7 primaries' **live** activation (current_A) | `state-engine.md` (tier 2) |
...
## Prior art
The mutable half mirrors `the-writers-desk` `CharacterState` (`character-model.md`) — reuse its STATE fields (psych zone, allostatic load, relationships, voice, interiority); this schema adds the structured **genotype · Model · drives · skills · provenance** the realism engine needs on top.
