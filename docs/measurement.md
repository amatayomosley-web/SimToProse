...
- **Correction protocol (resolves consolidation open-Q 3) — BUILT 2026-09-19:** the critic never edits. It appends a **`correction` event** referencing the bad event-id; the fold applies the inverse delta; consumers treat the referenced event as superseded. Append-only repentance — the record keeps both the error and its correction, which is itself diagnostic data (correction *rate* is detector #6: a rising rate = the recorder degrading).
  - **Status:** emitter `scripts/critic.py --correct` (`correct_run`); fold rule `src/engine/fold.py` (`superseded_ids` + the skip in `fold`); readers `Ledger.corrections_for` / `superseded_events`, `cut.py`'s corrected marker, `doctor.py`'s per-run count — detector #6's COUNT half is on the doctor's report; the RATE (corrections against a run's turns) is a reader's division, deliberately not a threshold the engine has never calibrated. **The inverse delta is the SKIP**, not a per-type inverse table: the fold is from-zero, so a superseded event never applies rather than being un-applied. `consolidation-loop.md` open-q 3 carries the full build note. **Open item 3 below (cascades) is untouched** — the additive tiers a bad beat already appraised into characters still stand.
...
3. Correction-event semantics for *cascaded* errors (a bad event already appraised into three characters) — inverse deltas compose, but order matters; design when first hit. **Still open after the 2026-09-19 build**, and now with a sharp edge: the WORLD half is done (the fold skips the superseded event), so a corrected turn leaves a snapshot that never saw the act and an appraisal tier that did. `records.WOUND_DELTA_KINDS` already carries a `correction` cause with no writer — the vocabulary slot exists, the inverse does not, and building one is this item, not the world fold's.
