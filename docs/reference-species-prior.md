...
## 2. Species prior — the resting rung per path (REWRITTEN 2026-09-10)

**You do not write a resting number any more — you write a WORD.** Where a character rests on
each path is `baseline.temperament[path].rest`: a word naming a rung — `quiet` (the path's own
name, rung 1), `low`, `raised`, `high` — capped per path at the last rung that reads as a
disposition. It is a CHARACTER DESIGN choice authored beside the voice (owner, 2026-09-10: "have
temperament be a character design question, along with their voice and other personality
options"), not a genotype cell. The species prior is **`quiet` on every path**: the
population-typical person sits at the floor of each ladder, and the draw for people nobody authors
(`scripts/make_genotype.py`, `draw_rest`) weights `quiet` at half. The `mean` beside the word is
seeded from it once (the band midpoint off the live ladder, `heritable.ensure_temperament`) and
moved after only by the arc engine.

| path | species prior (rest) | what `raised` / `high` mean here |
|---|---|---|
| STIRRING | `quiet` | always half-reaching for something / never quite at rest |
| WARINESS | `quiet` | always listening for trouble / lives braced |
| DISPLEASURE | `quiet` | a low irritation most days / easily put on edge |
| GOODWILL | `quiet` | warm before it is earned / caring is the resting state |
| DEFLATION | `quiet` | a sadness ordinary days do not lift / a weight carried unnoticed |
| DISTASTE | `quiet` | much of what people do sits badly (cap is rung 2: `low`) |
| RECEPTIVITY | `quiet` | open to being pleased / walks in already glad |
| SELF-REGARD | `quiet` | thinks well of self unprompted / expects to be noticed |
| LEVITY | `quiet` | quick to take an opening / rarely wholly in earnest (cap is rung 3: `raised`; the ninth path, 2026-09-11) |

The primitive-era table this replaced (SEEKING 0.45, FEAR 0.25, RAGE 0.20, LUST 0.20, CARE 0.40,
PANIC_GRIEF 0.25, PLAY 0.35, DISGUST 0.15 — all `[CALIBRATION]`) is in git history; its ordering
(SEEKING tonically highest, RAGE / LUST / DISGUST phasic and low) survives as the draw weights and
the rest cap, not as numbers.

**The rest words are the softest choices in this document.** There is no standardized instrument giving "resting fear" a population value the way HEXACO gives trait means, so theory constrains only the ordering, which the rest cap encodes. **If any row here is wrong it is these**, and the falsification is behavioural — a cast built on `quiet` everywhere that reads uniformly flat, or one drawn with too many `raised` that reads uniformly frantic.
