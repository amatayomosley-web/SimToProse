...
2. **Character JSON(s)** (`characters/<name>.json`) per `docs/character-schema.md`. Required by the
   engine (build_profile/assemble validate these): `fixed.name`, `fixed.genotype` (6 axes,
   values low|typical|elevated|high), `baseline.temperament` (all 7 primaries × {mean,variability}),
...
`current.affect` missing any of the 7 primaries, an affect value out of [0,1], `current.condition`
...
```bash
python scripts/direct.py --vault "<path to the book folder in your vault>" --char kestra   # a REAL book
...
is a key-gated FALLBACK, off by default (re-adding the key is a William call).
