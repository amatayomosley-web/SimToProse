...
1. ~~**The act seam**~~ — **WIRED 2026-08-24 for `direct.py`.** `--prompt-only` emits the turn messages; `--turn-json` accepts `{action, thought, tags, exit?, addressee?}` and runs validate → appraise → commit on it. A supplied turn passes the SAME faithfulness wall as a generated one — `check_name_leaks` on the supplied path, not just the generated one — because a re-entry that skips the wall is a hole in it. Verified end to end: a turn authored outside the engine committed and produced an acquisition. **`scene.py` still fuses them**: its loop picks the next speaker by salience and runs many turns per invocation, so a seam there needs a per-beat protocol rather than a flag.
