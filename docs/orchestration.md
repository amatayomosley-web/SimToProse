...
- **Director = the story-brain.** *What should happen*: PROPOSES the beats, the arc and the ending
  as candidates — the author RATIFIES (`orchestrator-design.md` §12, which superseded the
  autonomous-producer charter this line used to teach the day after it was written; corrected here
  2026-09-19) — then finds the circumstance that moves a character toward the ratified target
  (`design.md` layer 5). Creative.
...
4. PLAN      director proposes beats · arc · ending, author ratifies (`orchestrator-design.md` §12)  (destination firm once ratified; ~3-6 beats per brief, PLACE turning points — `scene-brief-blueprint.md`)
...
BEAT gate    did it land, in-character, no seam?  refusal → revise the beat, never the character
   ▼
CANON gate   continuity-critic passes → the committed events stand; the scene is marked canon (bad record → compensating event, never silent rewrite); the KEEPER runs here (`scene.py --keeper`, 2026-09-11): notices what moved and what was said, rules the tested claims established or fiction — kept sayings are facts for the next scene's actors
   ▼
...
1. ~~**The act seam**~~ — **WIRED 2026-08-24 for `direct.py`, and since for `scene.py`.** `--prompt-only` emits the turn messages; `--turn-json` accepts `{action, thought, tags, exit?, addressee?}` (`scene.py` also reads `act?`; the chair keys no law by an act and says so) - its shape checked before anything is opened and refused by name (`REPLY_*`, `src/engine/replies.py`, gate actor-reply) - and runs validate → appraise → commit on it. A supplied turn passes the SAME faithfulness wall as a generated one — `check_name_leaks` on the supplied path, not just the generated one — because a re-entry that skips the wall is a hole in it. Verified end to end: a turn authored outside the engine committed and produced an acquisition. `scene.py`'s loop picks the next speaker by salience and runs many beats per invocation, so its seam is per beat: `--prompt-only` emits the messages for the speaker salience chose, and `--turn-json` supplies the invocation's first beat; the rest act as usual.
