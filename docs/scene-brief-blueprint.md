...
**Purpose.** How big to make one sim brief: what a single run can and cannot produce, the measured beat-ceiling, and how to decompose a multi-part plot into briefs. Built from measured runs (gemma4:26b-a4b, one multi-character scene class). Pairs with `driving-the-engine.md` (the per-lever playbook); this doc is about *scene sizing*, that one is about *steering a beat*.

## What one sim run IS
A brief = **one situation** + a **cast**, each member carrying a **drive** (a standing want, blind to the outcome). Each beat the floor passes to the actor with the highest **urge** (`salience + addressed_bonus + disruption_stake − recency − inhibition`, salience pruned to 0 for a listener the emotion seat's lands_on did not name); that actor produces an emergent action + thought + self-report tags. The run ENDS — on its own, not at budget — when:
...
- **Heat extends the tail a little** (a live trigger the character could not call off pushed a 2-actor scene to 5); calm scenes lull sooner. See *Reading the stop* for why.
...
- **exit** — an **actionable** scene: the situation affords a physical act (go somewhere, strike, flee). The actor pursues the drive, walks, and the present cast drops below 2. The scene ends on a *departure*.
- **empty** — a cast of 1: one beat, no floor to pass to.

**Measured this session:** a two-actor scene whose actor *could act* on the drive ran to **5 beats** and ended when that actor left to do it (**exit**); the matched two-actor talk-only scene, with no action available, **lulled at 4**. The actionable scene ran longer because the actor had somewhere to go — a talk-scene can only decay.
...
- **lull → a settled tableau.** Everyone has said their piece. The seam you write is a **director-placed circumstance** that breaks the stillness (a letter, a fire, an arrival).
...
- **Does NOT produce: RESOLUTION or a turning-point.** Across every run this session, no decision was reached and no one was moved to the decisive act on their own. A matter pressed was put off, and put off again in the **same** words beats later — and the scene circled and lulled. Nobody *decided*, *left*, or *acted* to close the matter. (Same shape as `driving-the-engine.md` Conclusion 1: the exit/rupture is not a turn-lever.)
...
2. **Director-placed circumstance.** Something that happens *to* the cast (a letter, a fire, a death, an arrival) → write it into the *situation* of the relevant brief; do not expect the sim to generate it.
3. **Narrator-added load-bearing beat.** A turning-point, a decisive line, a loss-of-control the sim won't emerge → write it (see `driving-the-engine.md` Conclusion 4 + the loss-of-control findings).
...
## Status / confidence
Single runs, n small, temp 1.0, one model + one scene-class — **directions, not rates.** The beat-ceiling and the positioning-not-resolution finding are consistent across ~7 runs, but confirm at K≥6 (a K-seeded harness — `scripts/exp.py` was retired 2026-09-10; `bounds-experiment-design.md` specifies its successor) before a number is load-bearing. Re-measure on a model swap.
