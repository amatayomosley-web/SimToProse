...
- **`durability`** — how lasting this event-*class's* impact tends to be. A **class default carried by the event-type catalog** (`record-contract.md` — the same catalog row that carries the appraisal map): `transient | durable` (**corrected 2026-08-30** — this line read `transient | marking | reshaping`,
a vocabulary `consolidation._VALID_DURABILITY` has never accepted; an author following it produced
a beat the validator refused, and until 2026-08-30 that refusal silently discarded the entire
...
- **`resilience`** — **DERIVED at read time, never stored** (`character-schema.md` DERIVED row): composed from four already-stored fields — `effortful_control` (cut 2026-09-10; its weight is renormalised onto condition and attachment) (genotype axis) × **attachment-security** (relationship priors + presence of a current secure bond, `relationships.md`) × **current condition** (high allostatic load depletes it, `relevancy-gate.md` energy state) × **meaning-frame availability** (does the Model give this event-class a coherent place — the meaning-making term, `values-and-stakes.md`). Combination form is Class-B calibration; the fields are these four. Open-Q 1 below narrows to the numeric form only.
...
## Pricing the diff — one table, and now two receipts (2026-09-19)
The magnitude a durable diff writes is priced by **one table** (`state.price_for`, the identical chain `appraise` runs — severity × relevance × genotype gain × sensitivity × push × regard × connection, `character-model.md` law 4), then scaled by the damage step (`grw × (1 − resilience)`, the fork above). That table is keyed on the event's **dimensions** — it prices what the event **was**.

A beat can also become a durable candidate on a **reading's height alone** (`emotion-arithmetic.md` §5 step 7), and two paths, RECEPTIVITY and SELF-REGARD, have no row in that table at all — nothing in the event vocabulary means "something good reached you" or "your standing moved" (`state.py`'s own comment on `_DIM_TO_PATH`). Before gate `heights-price-arc`, a beat that qualified as durable purely on one of those readings priced to an **empty diff**: candidate, but nothing to show for it.

**The fix is a second receipt, not a new dimension.** Every path whose reading reached the durable height this beat is now *also* priced from that height, through the identical damage step the dims table uses (`height × grw × (1 − resilience)`). Where a path is reached by both routes — a severe dim and a durable reading on the same path — the two terms **sum**: two receipts of one beat, the dims table pricing what *happened*, the reading pricing what *arose*. Owner decision **D3** (2026-09-19): READINGS-ONLY for RECEPTIVITY and SELF-REGARD — no `_DIM_TO_PATH` rows are authored for them here; inventing what dimension "something good reached you" would be is the owner's mapping to make, not a pricing gate's.
...
2. **Diff healing** — ~~do durable diffs themselves slowly soften over the arc (a wound easing over years) or change only via new events?~~ **ANSWERED 2026-08-31 for WOUNDS, and not the way this leaned.** The lean was time ("very slow heal toward the pre-event baseline"). Shipped: **event-first, time-second**, in `src/engine/wound.py`. A wound's `intensity` IS the character's prediction of how bad this class of moment gets, so the learning signal is `observed - intensity` — the same law `bonds.observe` runs for an edge. Walking into the thing and finding it survivable is what eases a scar; being hurt there again is what deepens it; avoiding it changes nothing, because with no trial there is no error. Time survives as a weak second rule (`wound.erode`, retention 0.995 per declared unit — above `bond_rest._RETENTION["debt"]`, so a scar outlasts a debt) with a per-wound `permanence` floor: `permanence: 1.0` is the "some scars permanent" half of the original lean, now authorable. Deepening runs at `_A_DEEPEN` 0.30 and easing at `_A_EASE` 0.12 — the negativity-bias ordering `bonds` already uses, right here for a second reason: re-acquiring a fear after extinction is faster than the extinction was. Resilience is a GAIN on both branches, never a threshold; the 0.70 fork below is a different mechanism and `wound.py` does not read it. **Still open for TEMPERAMENT diffs** — this answers wounds only.
