...
`_KNOWN_DIMS` derives from `state._DIM_TO_PATH` rather than hand-copying it, and `validate_tags`
...
## 6. BUILT — the one registry (gate three, 2026-09-11)

Section 5's "one binding rule for every entity" is code: `connection.for_about(relationships,
held, about, path)` reads investment in a person (the edge), a concept (the wound keyed to that
concept and path — `baseline.wounds[].intensity`), a goal (`goal:<n>`, its priority) or a value
(`value:<key>`, its weight), through the one floor and the one ceiling. `connection.held_map(char)`
builds the non-person half per beat; people stay live off the edge. Investment now also stretches
the half-life (`connection.half_life_scale`, 1 + c) and presence holds an excursion in the room
(`connection.PRESENCE_HOLD`, x3). Repetition (`connection.repetition`) rides beside it. What a
feeling is ABOUT that is not a person comes from the closed concept registry (`concepts.py`),
which is how section 3's "binding for a concept" is answered: not by source variance measured in
the engine, but by the seat naming the concept and the engine matching by identity.
...
* **Whether a soured world-bond can heal, and through what.** The machinery permits it (`bond_rest.drift`
  relaxes toward the edge's rest — though a cliff lowers the rest itself, so drift never heals what was
  unforgivable); whether a life can be redeemed is a story question the author has not yet ruled on.
