...
- `TurnCommit(run_id, turn, actor, thought, action, tags, affect, condition={}, events=[],
  ...
  PATHS, each in [0,1] (the eight paths replaced the retired `PRIMARIES` on 2026-09-08; LEVITY made nine on 2026-09-11). PATHS is defined here and only here.
...
4. **Append-only.** There is NO update, NO delete on events. Corrections are new `correction`
   events, and since 2026-09-19 they FOLD: `fold.fold` collects the ids an arrived correction
   supersedes and skips them on replay (the inverse delta, because the fold is from-zero), and
   `snapshots.divergence`'s tail replay applies the same rule — it has to, or it would report its
   own arithmetic as a stale cache. Why: the log is the biography's ground truth.
...
API: `build_profile(char) -> {gains, hold, temperament, relevance_weights, model, regard, relationships}` (2026-09-10: `decay_rates`/`regulation`/`sensitivity` are retired; hold multiplies the half-life) · `appraise(affect, tags, profile)` · `decay(affect, temperament, profile)`. Pure; ValueError
...
API: `direct_affect(affect, temperament)` (retired 2026-09-08 — the rung blocks via `scripts/composer.py` replaced it) · `direct_condition(condition)` · `direct_edge(edge)` ·
...
- **Depth gates**: src/ changes need a gate in the depth-gate workspace `.depth/` (its path is
  machine-local and lives in your own agent config, not in this repo) (one gate at a
...
- **Still deliberately unbuilt** (don't "discover" these as bugs): the critic's **compensating-event
  ...
  **world/character generation tooling**; Kuzu vault graph (vault is flat
