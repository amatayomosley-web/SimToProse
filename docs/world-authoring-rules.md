...
a town's open-air lido, the swimmers who use it all year, the management that runs it — so this file
stays upstreamable to the template.
...
- **The Violation**: Writing a governing rule into `world/TheLidoRules.md` as `type: concept`
...
"tensions": [
  {"id": "winter-closure",
   "what": "the lido's new winter closure, resented by the year-round swimmers",
   "temperature": 0.15,
   "factions": ["management", "swimmers"],
   "watches": {"parties": ["management", "swimmers", "cecily"], "locations": ["poolside"]},
   "interests": {"social_violation": 0.5, "loss": 0.35},
   "cooling": "slow"}
]
```
...
- **`interests`** is WHAT IT IS ABOUT, weighted over the seven appraisal dimensions the engine
  ...
  act may heat several — a scuffle at the turnstile raising both the closure dispute and the old feud is
  two true facts, not a collision.
...
- **The Violation**: A rule that nobody swims before the lifeguard is on duty, existing only as prose
  in a concept doc. The engine cannot consult it, so no scene can be refused by it and no character can
  be caught breaking it.
- **The Valid Form**: `{"id": "no-swimming-without-a-lifeguard", "domain": "legal",
  "act": "swim-before-the-lifeguard-is-on"}` — now `laws_bearing_on()` and `verdict_for()`
...
- **The Violation**: A scene directive set at `boiler_room` while `world.locations`
  registers only the poolside and the changing rooms.
...
- **The Violation**: Authoring the swimmer who was banned last summer for climbing the diving
  board after hours as `type: concept`. He is unrecognisable to the engine and cannot carry a relationship edge.
