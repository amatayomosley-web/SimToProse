...
3. **Compensating-event correction — BUILT 2026-09-19** (designed 2026-06-10; `measurement.md` §the consistency critic). Append-only `correction` event referencing the bad event-id; the fold applies the inverse delta; downstream consumers (narration, the cut) treat corrected events as superseded. Never silent, never mutate.
   - **Emitter:** `scripts/critic.py --correct` → `correct_run`. One correction per continuity flag, appended through `world_events.append` at the run's NEXT tick, carrying `{supersedes: [event_ids], turn, issue, source}`. The ids are the flagged turn's WORLD-MOVING events, derived from `CATALOG.world_map` rather than listed. Idempotent on `(turn, issue)`; opt-in, because the flags are a strong model's reading and the owner decides. A flag on a turn that moved nothing still appends, with an empty `supersedes` — the audit trail, and the correction RATE is `measurement.md` detector #6.
   - **Fold rule:** `src/engine/fold.py` — `fold` collects the superseded ids over the whole ARRIVED log before projecting anything, then replays in log order skipping them; `project` carries an explicit no-op branch for the system types so a correction row can never fold by accident. **The inverse delta IS the skip:** the fold is from-zero (hard rule 2), every projection sets a field or folds a delta in order, and none reads its own prior value — so a superseded event is not un-applied, it simply never applies. Pinned over every type in `world_events.TYPES` by `tests/test_fold.py`.
   - **The cache reaches back with it — and the tail replay obeys the same rule.** A correction is the one event type whose effect lands BELOW its own tick, so `world_events.append` drops the snapshot cache from the earliest SUPERSEDED event's `effective_at`, not from the correction's (`_reaches_back_to`); and `snapshots.divergence` collects `fold.superseded_ids` over the tail and skips those ids while replaying, because the from-zero fold it is compared against does. **Both halves are needed and each hid the other.** Without the drop, park → `--correct` → one more turn → resume raised `LEDGER_RESUME_DIVERGENCE`. With the drop but not the skip, a snapshot cached BELOW the bad move survives, carries it forward through the tail, and diverges just the same. Skipping in the replay is exact rather than a guess *because* of the drop: a surviving cache always sits below every superseded id, so every one of them — and the correction naming them — is in the tail. Weaken the drop and the skip becomes wrong silently; they move together.
   - **Consumers:** `Ledger.corrections_for` / `Ledger.superseded_events`; `scripts/cut.py` marks a corrected turn with the critic's words; `scripts/doctor.py` reports the per-run count. `scripts/narrate.py` needs nothing — it renders from `scenes` and the EDL and reads no event rows at all.
   - **Still open:** CASCADES. A bad beat has already been appraised into characters (relationship_deltas, readings, toward, wound rows) and nothing here un-does that — `measurement.md` open item 3, "design when first hit". The prose rewrite is the author's half and stays unbuilt by choice.
