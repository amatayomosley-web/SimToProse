...
**Validated — on a superseded fixture scene, live, no scripted lines.** Three actors on genuine drives produced the conflict the scene was set up for emergently, and surfaced the scene's planned turning line **unscripted**. The conflict came from decent wants colliding: **no villain in the room** — the design's central bet, confirmed in a real run.

**The findings map straight onto this doc's own design — the reduced floor is exactly what falls short:**
- **Pure salience starves the quiet actor.** The third actor never won the floor; the two most-provoked locked it. This is *why* the documented urge model is richer than salience — **`addressed_bonus`** (the quiet one would be pulled in when spoken *about*), **`social_inhibition`**, and the **softmax temperature** (the quiet one occasionally jumps in; no monopoly) are the fix. The v1 starvation **confirms** the fuller scheduler is needed, not optional.
- **The break needs a storm-out exit + budget.** The advocate escalated but never walked out in 8 beats — the planned exit beat was arriving exactly as the budget hit. The documented **convergence exits** (concede/flip, decision-forced, **storm-out**) need wiring; lull-or-budget alone under-stops.
