...
**Status: NORMATIVE for the tier model and the collapse rule. The keeper's RULING half is BUILT
(2026-09-11, `scripts/keeper.py --rule`, `keeper.rule_scene`; wired at the canon gate by
`scripts/scene.py --keeper` beside the noticing pass): it rules established / fiction / superposed
on the scene's claims and writes verdicts through `claims.resolve`. Its third duty — writing an
adopted fact back into the bible and authoring the lore that makes it consistent — is still the
author's path.** The author approved this design in session on 2026-08-29; it is recorded
because a design that lives only in a conversation is a design that has already been lost once.

**The keeper's PRICING duty is BUILT too (2026-09-19, `scripts/keeper.py:attach_scene`, bond gate
5's third writer of a hold — `docs/bond-arithmetic.md` s3).** On an utterance `rule_scene` just
ruled ESTABLISHED, when its speaker's own extract binds them to a place or group the bible
registers, the keeper prices a SELF-SOURCED hold one rung below the word a classifier names — the
same rubric the composition pass uses at character generation, never shown the price table. It
never fires per beat (only what this pass's ruling actually kept), never on a T2 (superposed)
claim, and never re-prices a hold the speaker already carries at least as high. It never invents
the entity — the object must already be a registered `loc.`/`grp.` name, or nothing is asked about
it — and it never crosses the per-character `life`-tier cap. Absent a world to check registration
against, it fails closed and prices nothing. **The match has a producer side, not only a consumer
one:** a claim's extract can only carry a registered name as its object if the NOTICING pass was
shown those names to begin with, so `build_keeper_prompt` gained the same list, and `notice_scene`
and `canon_gate` thread `world` through to it — before this a live reply could only ever phrase an
object in prose, which the rubric can never match.

**The canon gate is DEFAULT ON since 2026-09-19 (owner decision D1), and the debt it used to leave
silent is now measured.** Until this date the gate ran only when an invocation passed `--keeper`, so
an ordinary run left every saying an actor spoke committed WITHOUT extracts — invisible to
`claims.about` and to `established()` below — and nothing said so; every live scene run on a real
book so far ran keeper-off, and its utterances sat unindexed with no line anywhere reporting it.
`src/engine/claims.py:unextracted(con, run_id)` counts exactly that gap (count, the turn span, the
speakers) and both drivers print it — as a `lore:` line — at the end of every invocation, stub or
not, gate run or skipped; `scripts/doctor.py`'s full report carries the same count per run as an
AMBER line, so the debt is visible on a database between invocations too. `--no-keeper` opts a
single invocation out; `--keeper` is kept as an accepted, now-redundant flag. See
