...
Character **C = "Ren"**, built to the schema the owner's real books use (verified against
`characters/maren-healer.json` and every book character via `src.engine.vault.load_book`):
...
- **Morbid fear of spiders:** `baseline.drives.fears_wounds[0] = {fear: "spiders — a morbid,
  ...
  the exact shape `drives-schema.md:33-43` specifies and that characters in the owner's books
  author (including the `intensity` number).
...
the schema gives it a number (`intensity`), the owner's books use it. But grep of `src/` and
...
| Character sheet expresses nuance without tags | **YES for what is read; ceiling above that** | temperament vector + genotype + 3 live traits + model weights compose (brave-Ren is genuinely brave *by numbers*) | no compound layer over the basis (cf. the source lineage `emotion-vectors.md`: contempt/disdain/scornful as neighboring *coordinates*); **no disgust axis** in `PRIMARIES` (`records.py:10`) — `social_violation → RAGE` alone (`state.py:42`), and all four RAGE directions are hot confrontation (`direction.py:33-36`), so **cold contempt is unrepresentable as state** — material any story of contempt needs; the regard map is the only contempt-shaped mechanism and it is standing disposition, not affect |

---

## 3. AUTHORED-BUT-INERT

> **RE-CHECK BANNER, 2026-09-06.** This is a DATED review and its body is left as written — a
> review edited after the fact stops being evidence of what was seen. Two of its claims are now
> known false and are recorded here rather than patched above: (a) `:271` "**no disgust axis** in
> `PRIMARIES`" and "cold contempt is unrepresentable as state" — DISGUST became the eighth
> primitive on 2026-08-22 and `SPEC-LEDGER.md:72` records the retraction with the measurement;
> (b) `:271` "no compound layer over the basis" — `compounds.py` was built the same week, and its
> naming path has since been built AND retired (`emotion-dynamics.md:616-621`). The
> AUTHORED-BUT-INERT table below has NOT been re-verified field by field; treat each row as a
> 2026-08 observation needing its own re-check before it is acted on.
...
|---|---|---|---|
| `baseline.drives.fears_wounds[*].intensity` / `.trigger` / `.avoids` / `.protects` / `.view` / `.defense` | none | some book characters carry the full schema shape with numbers; others carry `trigger` lists; Maren | the schema's numbers (`drives-schema.md:33-43`) are authored and computed-from by nothing; `trigger` lists never reach `run_gate` |
...
- **The urge model is numbers-driving-behavior, working today.** Floor selection in
  ...
  anti-monopoly term — the decorum-keeper stepping into a heated exchange is computed, not
  narrated.
...
- **"Swapping numbers moved `_order_weight` 0.300→0.517; swapping prose moved nothing":
  confirmed and located** — those were two real characters' sheets;
