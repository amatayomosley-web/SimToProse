...
## Prior art to lift
design.md already frames this as an event-sourced spine (append-only log + folded snapshot + gated canon); `world-model.md` open-question #2 flagged prior-art author-mode world-state designs as worth checking; their shape is restated below. **Adopt that shape** — the event-log + fold machinery — rather than reinvent, same as the vault lifted Scribe's knowledge graph.
