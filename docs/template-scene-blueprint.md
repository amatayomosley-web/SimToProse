...
covers **what the actor receives**. Neither is book-specific: every example here is either one of
the repo's own fixtures (`characters/*.json`, `world/ashford-slice.json`) or a scene invented on
this page (a bookbinder's workshop on the day an order is due).
...
```json
"props": ["a glue pot warming on a hotplate", "a finished album drying under the book press"]
```
...
Write props as **phrases an actor could perceive**, not as identifiers. `press_2` is a
key; *"the second book press, its handle wrapped in tape"* is a percept.

**Check before shipping:** does any actor's drive reference an object? Then that object must be in
`props` or in `situation`. A drive of *"lift the album out of the press the moment the glue has set"* against a cfg with no
props directs a character at equipment they cannot see.
...
```json
"cast": [{"id": "hester", "drive": "get the album out of the press and boxed before the customer comes to collect it"}]
```
