...
| Relevance = event dims × the character's Model weights ("where two people diverge") | `state-engine.md:40`, `values-and-stakes.md:24-33` | YES | `_DIM_VALUE_KEYS` `state.py:125-144`, `_relevance` :270-240; missing key = neutral 0.5 | BUILT-AS-SPEC'D |
| trait_sensitivity = genotype gains × HEXACO slopes | `state-engine.md:41`, `baseline-generation.md:49-59` | YES | `state.py:268-296` (`_ALLELE` :28-32; emotionality/agreeableness/extraversion slopes :169-183) | BUILT-AS-SPEC'D — 3 of 6 traits slope gains; the rest ride the prompt as context (`character-authoring-rules.md` Rule 5 documents this honestly) |
...
| Subject-regard empathy scoping (bigotry floor 0.25; affinity lifts, never lowers) | no design doc owns it — `guide-emotional-authoring.md:110-115`, `driving-the-engine.md:14` | YES | `state.py:150-158` (`_CARE_FLOOR`, `_REGARD_SCALED_DIMS`), `_regard` :248-221; runtime subject resolution `scene.py:349-390`; proven `tests/test_subject.py` | BUILT (code-first; guides document it; no `docs/` design doc is its owner) |
...
| Acquisition during play (lived / witnessed / learned; monotonic-add; dedup) | `knowledge-model.md:17,104-110` | YES | `acquisition.py` (`assess` :24, `witness_belief` :115, `reveal_name` :66, `overheard_names` :171); persisted `ledger.py` `append_acquisition` :240; resume rehydrates | BUILT-AS-SPEC'D (channels told/taught/read/deduced not distinguished; forgetting not built — declared later-layer) |
...
| Name masking + latent-leak regeneration ("recorded as-is" preserved) | `knowledge-model.md` wall; status log `driving-the-engine.md:104-111` | YES | `gate.scope_names` :543 (prompt wall), `faithfulness.py:16-35` (output detector), `direct.py` `faithful_turn` :232-232 (regenerate → reject) ; `tests/test_faithful_turn.py` | BUILT-AS-SPEC'D |
...
| mechanism | specified in | built? | where | divergence |
|---|---|---|---|---|
| Append-only log, two clocks, pure fold, snapshot-as-cache, atomic turn-commit, loud divergent resume, parking | `world-state-ledger.md:12-24`, `run-lifecycle.md:17-25` (B4, C3) | YES | `ledger.py` (`append_turn` :97, `_project` :468, `fold` :474, `resume` :478); `db.py` v7 migrate-on-connect; `tests/test_ledger.py`, `test_pipeline_e2e` | BUILT-AS-SPEC'D. Fold families grow empirically (move/harm+terminal/reveal/seize/destroy-asset/betray/bond/tension) |
