...
| Acquisition during play (lived / witnessed / learned; monotonic-add; dedup) | `knowledge-model.md:17,104-110` | YES | `acquisition.py` (`assess` :24, `witness_belief` :137, `reveal_name` :87, `overheard_names` :194); persisted `ledger.py` `append_acquisition` :270; resume rehydrates | BUILT-AS-SPEC'D (channels told/taught/read/deduced not distinguished; forgetting not built — declared later-layer) |
...
| Compensating `correction` events (append-only repentance; fold applies inverse) | `consolidation-loop.md:57`, `measurement.md:19-23` | YES | EMITTER `critic.py` (`world_moving_types` :242, `continuity_flags` :256, `correct_run` :283, `--correct`); FOLD `fold.py` (`corrections` :105, `superseded_ids` :118, skip in `fold` :140) + the cache reach-back `world_events.py` (`_reaches_back_to` :285) + the same skip in the tail replay `snapshots.py` (`divergence` :90); READERS `ledger.py` (`corrections_for` :574, `superseded_events` :577), `cut.py` (`corrected_turns` :32), `doctor.py` (`_corrections` :62); `tests/test_fold.py`, `test_critic.py`, `test_cut.py`, `test_ledger.py`, `test_doctor.py` | **BUILT 2026-09-19** — the inverse is the SKIP, not a per-type inverse table: the fold is from-zero, so a superseded event simply never applies. Pinned over every type in `world_events.TYPES`. CASCADES STILL OPEN (`measurement.md` open item 3): the additive tiers a bad beat already appraised into characters — relationship_deltas, readings, toward, wound rows — stand |
...
| mechanism | specified in | built? | where | divergence |
|---|---|---|---|---|
| Append-only log, two clocks, pure fold, snapshot-as-cache, atomic turn-commit, loud divergent resume, parking | `world-state-ledger.md:12-24`, `run-lifecycle.md:17-25` (B4, C3) | YES | `ledger.py` (`append_turn` :102, `_project` :561, `fold` :567, `corrections_for` :574, `superseded_events` :577, `resume` :580); `db.py` v7 migrate-on-connect; `tests/test_ledger.py`, `test_pipeline_e2e`, `test_fold.py` | BUILT-AS-SPEC'D. Fold families grow empirically (move/harm+terminal/reveal/seize/destroy-asset/betray/bond/tension), and since 2026-09-19 the fold also SKIPS every id an arrived `correction` supersedes (next row) |
