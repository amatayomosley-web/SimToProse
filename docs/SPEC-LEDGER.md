...
| Compensating `correction` events (append-only repentance; fold applies inverse) | `consolidation-loop.md:57`, `measurement.md:19-23` | YES | EMITTER `critic.py` (`world_moving_types` :242, `continuity_flags` :256, `correct_run` :283, `--correct`); FOLD `fold.py` (`corrections` :105, `superseded_ids` :118, skip in `fold` :140) + the cache reach-back `world_events.py` (`_reaches_back_to` :392) + the same skip in the tail replay `snapshots.py` (`divergence` :90); READERS `ledger.py` (`corrections_for` :586, `superseded_events` :589), `cut.py` (`corrected_turns` :32), `doctor.py` (`_corrections` :62); `tests/test_fold.py`, `test_critic.py`, `test_cut.py`, `test_ledger.py`, `test_doctor.py` | **BUILT 2026-09-19** — the inverse is the SKIP, not a per-type inverse table: the fold is from-zero, so a superseded event simply never applies. Pinned over every type in `world_events.TYPES`. CASCADES STILL OPEN (`measurement.md` open item 3): the additive tiers a bad beat already appraised into characters — relationship_deltas, readings, toward, wound rows — stand |
...
| mechanism | specified in | built? | where | divergence |
|---|---|---|---|---|
| Append-only log, two clocks, pure fold, snapshot-as-cache, atomic turn-commit, loud divergent resume, parking | `world-state-ledger.md:12-24`, `run-lifecycle.md:17-25` (B4, C3) | YES | `ledger.py` (`append_turn` :103, `_project` :573, `fold` :579, `corrections_for` :586, `superseded_events` :589, `resume` :592); `db.py` v7 migrate-on-connect; `tests/test_ledger.py`, `test_pipeline_e2e`, `test_fold.py` | BUILT-AS-SPEC'D. Fold families grow empirically (move/harm+terminal/reveal/seize/destroy-asset/betray/bond/tension), and since 2026-09-19 the fold also SKIPS every id an arrived `correction` supersedes (next row) |
...
**2026-09-25 — the character sheet, declared once (gate `sheet-contract`):** G2 of the contracts plan. No sheet
field was declared anywhere complete: the blueprint marked `drives.orientation` REQUIRED after the engine stopped
reading it (`scene._manner_drives` keeps a goal's text alone), the linter walked number tables without the
annotation rule the engine applies (every `_note` read as "not a number" - 19 of the 57 errors on the owner's four
books), an absent `baseline.skills` was a lint warning while both drivers index it and crash, and `fixed.position`
carried its notes into the prompt (`_strip_notes` was applied to voice and drives only - fixed here). Now
`src/engine/contracts_sheet.py` declares every field (from an inventory of every path the engine and its drivers
read, file:line each, and the paths the owner's books and the fixtures carry): its shape, whether a book must
author it (always, or while the system that owns it runs), and its status - active, retired (what replaced it and
the policy: prune, move, refuse), unread (reaches nothing), runtime (the engine writes it). An annotation (`_...`,
`note`) is ignored everywhere; a block an engine module owns is checked by that module (wounds, catalog rows,
attachments, the update rates, body, injuries). `src/engine/contracts.py` walks a file against a declaration and
REPORTS (nothing refuses until the books have migrated). The retired emotion basis is data now,
`records.RETIRED_PRIMITIVES` (the "was" column, replaced not translated), so a sheet still keyed by a primitive is
told its successor. `scripts/lint_book.py` reports the contract, grouped per declared field, and keeps only what one
sheet cannot answer (the world's ids, the other sheets) and the resting-face receipts; its numeric lists are gone.
BLUEPRINT-character carries the declarations as a table generated by `scripts/gen_contracts.py`. Measured on the
owner's books: 0 shape errors (was 19 false); 99 missing paths (11 characters on the primitive basis), 3 strength, 2
catalog; 254 retired keys; 168 unread. Suites: `tests/test_contracts_sheet.py` (49 checks), `tests/test_numeric_slots.py`
rewritten against the contract. NOT COVERED: the world note, people notes and scene file (their contracts are the
...
**2026-09-25 — the scene file, declared once (gate `scene-contract`):** G2 for the last author file. `lint_scene`
only ever saw a scene after `load_scene_cfg` had rewritten it - severity words already numbers (its own word check
could never fire), `elapsed` already refused, a bad `voice` already a SystemExit escaping its handler - and it
checked nothing of `pov`, `subject`'s shape, `voice` or `knowledge`, while it accepted a cast id from world.people
that the run refuses (a cast member must be a character). The docs' example scene files used `elapsed` (refused
since the clock) and left out `at` (required): BLUEPRINT-scene's worked example, blank form and file shape,
template-scene-blueprint's field list, guide-continuing-the-story's example. `src/engine/contracts_scene.py` declares
every key (from an inventory of every key scripts/scene.py, lint_scene, the narrator and the pinned-cfg readers
take, file:line each), each vocabulary the engine's own: the clock for `at` and `lasts`, the seven appraisal
dimensions and the severity ladder for the opening tags, the narration modes, the relation words, the condition
words. A malformed `subject` (which the loader turns into no one, silently) and a `pov` outside the cast are errors;
`opening_tags.type` / `.durability` / `.act` reach nothing; `elapsed` is retired. `lint_scene` checks the file as
written, reports the loader's own refusal after, and keeps the book checks and the craft rules. BLUEPRINT-scene
carries the declarations as a generated table (section 16b). Suite: `tests/test_contracts_scene.py` (one mutant, the
...
**2026-09-26 — the record's second wall, on every database the engine opens (gate `record-guards`):** G6 of the
contracts plan. `src/engine/schema.sql` held eleven columns only to "not empty" where the record layer refuses anything
outside a closed vocabulary - relationship_deltas.axis and rest_declared.axis (`records.RELATIONSHIP_AXES`),
relationship_deltas.ord (first or second), toward_deltas.primary_, target_binds.primary_ and wound_minted.path
(`records.PATHS`), readings.path and .rung (the ladders), readings.confidence (`readings.CONFIDENCE_WORDS`),
wound_minted.concept (`concepts.REGISTRY`), utterances.tier (`claims.TIERS`) - three deltas and a scar's intensity to no
range at all, and bible_laws.statement to NOT NULL only; and a migrated database - every book database is one - carried none of it, since
SQLite cannot ALTER a CHECK in. `records.TurnCommit.validate` never read `validation` or `wound_mints`: a mint's bad
concept, path or intensity was refused by its WOUND_* code only from inside the turn's transaction, and a missing id or a
source, text or trigger of the wrong type rolled the beat back uncoded (LEDGER_TURN_COMMIT_ROLLED_BACK). NOW: `src/engine/guards.py` builds one
BEFORE INSERT trigger per guarded table, `<table>_record_guard`, from the record layer's own constants, and `db.connect`
installs it on every open where it is missing or older (schema v34), in one IMMEDIATE transaction (a current open writes
nothing and takes no lock; the plan is read again under the lock, so two tools opening one stale book
together both open), touching only its own trigger names (compared case-blind, as SQLite does); each refuses by the
record layer's own code, a NULL included, with the ranges read from the same named bounds the validators read
(`records.DELTA_RANGE`, `INTENSITY_RANGE`). ONE WALL PER SCHEMA VERSION, HELD AT RUN TIME: each guard carries a stamp
- the version that installed it, its table's fingerprint, and the WALL, `guards.vocabulary()` of every guard: the
words, bounds and ladders AND everything in each guard's trigger but its message (the predicate, the RAISE kind, when it
fires) - and an engine meeting any trigger stamped at its
own version or a later one with another wall, or with a stamp it cannot read, refuses the book (DB_GUARD_VOCABULARY_SKEW)
instead of replacing it: other words, bounds, rungs or tests, a guard retired, a table added. The
gate's reviews showed two engines at one version, with different words, replacing each other's guard on open and
refusing each other's validated beats; then the suite's pin alone defeated by editing it; then a guard retired, or one
added on another table, passing a check of our own guards' words; then the same words read by another predicate, and
a stamp in another format, passing a fingerprint of words alone.
A change to a guarded word or bound therefore steps `db.SCHEMA_VERSION` (the suite pins `vocabulary()` to it as the
reminder); an older engine refuses a newer book (DB_SCHEMA_TOO_NEW), and a write an upgraded guard refuses under a
still-running older engine - a beat, `bond_rest.seed`, `claims.record` - is named DB_GUARD_REFUSED, not a bare
rollback. A foreign trigger holding a guard's name is refused (DB_GUARD_NAME_TAKEN), never dropped; install refuses a
connection with a transaction open (DB_TRANSACTION_OPEN). Install and `integrity` read one survey (`guards.survey`), so
`scripts/doctor.py` reports a book the engine refuses to open as red BOOK-REFUSED naming the code the open raises
(another engine's wall, a taken name, a newer book) - never amber "installs it on open" - a refused open writes
nothing (the check runs before a migration commits), and `docs/guide-operating.md`'s Failure playbook states the
owner's way out of each - for another engine's guards, `python scripts/release_guards.py <book.db>`, one command in any
shell (`db.release_guards`, so `db.py` stays the one writable opener, then `guards.release`: every stamped trigger
dropped, no row touched, a missing file refused and never created; the next open installs this engine's own) -
printed, with the book's own path, by the refusal and by the doctor alike (`guards.release_hint`).
Refusing at open rather than letting the first engine's wall stand is a stated choice (the owner, 2026-09-26): first-wins
would meet the difference later, mid-scene, as DB_GUARD_REFUSED. Triggers, not a rebuild: a rebuild copies history
through the new CHECK, and three of the 79 book databases hold rows under the emotion names retired 2026-09-08 or an older
order spelling; no row is rewritten, and `integrity` counts them as LEGACY-ROWS (amber: append-only history cannot be
repaired) and a missing or stale guard as INSERT-GUARD-MISSING (amber). A guard never refuses what the record layer
accepts: `Reading.validate` now holds confidence to the three words, and `readings.write` stores the ladder's own spelling
of a rung (every recorded reading already did). `TurnCommit.validate` checks the mints - every field `write_mints` keeps,
by `wound.check_mint` (the id is its (concept, path)'s), their WOUND_* codes, before any INSERT - `validation`'s ok / flags
/ confidence / escalate by type where given and as storable JSON, and the lands_on items. A refused commit stops the run
(no driver catches LEDGER_TURN_COMMIT_ROLLED_BACK or DB_GUARD_REFUSED); the beat rolls back whole. Evidence:
`tests/test_record_guards.py` - every near miss of every guarded word, rung and path (each case; a space, tab or
no-break space at either end; separators swapped; trailing punctuation; the full-width form; the `concept:` prefix)
through the engine's own writer is written or refused by the record layer, never by the database; every book
database copied by the backup API and migrated under HEAD and under this gate: every row identical, nine guards each,
integrity unchanged but for LEGACY-ROWS. Not done: json_valid on the JSON columns (json.dumps writes NaN); the v21/v22
not-empty CHECKs a migrated table lacks (the same mechanism could mirror them); schema.sql's own IN-list CHECKs that copy
a record-layer constant (wound_deltas.kind, rest_declared.source, claim_resolutions.verdict and eleven more) predate the
...
attach classifier's reply by named gets. A world change's payload went WHOLE into the append-only events log, and
three readers take every event's payload whatever its type (`scene_facts.payloads`, which `injuries.run_rows` reads
through; the mood replay's `subject_group`), so a keeper move carrying `injuries`, `transfers` or `told` was read as
the beat's own injury, handover or telling (a fourth, `scripts/canon_digest.py`, shows any event's `text`, `summary`,
`subject` and `durability`, and the names in its `dimensions`, against that event, and so loses a keeper's stray one
now - the third and fourth reviews). Fields of the wrong type crashed the whole pass before the scene was
parked - a turn that is a list or a map; a payload that is text, a number or true; an id that is a list or a map, or
text holding a lone surrogate, at the sqlite bind; a list or map tension id - or went in by accident: a true or false
turn as turn 1 or 0, a list of pairs as a payload, a number as an actor "5", and a false or 0 id judged as absent
but stored as the text "0" - a name the fold then read back, so a phantom agent "0" died or became a knower (the round-3
differential found it: the event judged was not the event written). Payload values of the wrong type folded
into the world: a reveal whose `to` was one name as text made each letter a knower; a numeric fact or asset, or one
holding a lone surrogate, bricked every later park or resume; a harm with terminal "false" or 1 killed, and one whose
target was only spaces killed a phantom of that name; a dimension valued true, NaN or Infinity folded as heat, and NaN reached the log as a token JSON does
not have. A claim's non-text speaker, said and extract fields and a ruling's non-text rationale were stringified into
the claims tables (a null object as "none"). A claim labelled `"type": "claim"` was refused as an unknown world
change. A reply with no readable list, a non-object entry and an entry of neither kind vanished, and a VALID list
whose quoted speech held a bracket was cut short and dropped. Every refusal built there was prose, and the noticing
pass printed only how many. NOW: a report is routed by WORLD type (`keeper._route`, the CLI's too): a type naming a
world type, give or take case and spaces, makes a world change (a mis-cased one is still refused by its type, its
`said` noted); a `said` key with any other type is the claim it is, `type` named (a declared new acceptance; all are
listed below) - unless the report carries a change's own fields (a payload, an actor, a target, a location, holding
something: a null, an empty map or list or blank text carries nothing, so a labelled claim in a shape shared with world
changes stays a claim - the fourth review): "killed" or "moved" with a victim is a world change with a wrong type,
refused by it as at HEAD, its `said` noted when it is text (the third
review found round 3 sent it to the claims, its world half named as unread and refused by nothing). With no type at
all a `said` makes a claim, as at HEAD. A payload keeps only the keys the fold reads for its type and form (`world_events.payload_keys`; a
threat's dimensions only the seven), and the rest are left out and named, never the reason a report is refused - so a
severity word in a key the type never reads no longer refuses it (at HEAD it did). An id that is not text naming
something (not storable text - `replies.text_ok` - or blank) is written as null and named, unless the fold reads it
for THIS report. That question, and whether a left-out payload key was really unread, is answered per report by
projection (`keeper._strip_check`, `world_events.projected`) onto copies of the world at the report's turn. The ids
on the report AS GIVEN: if the written payload with every junk id as given changes nothing, none is read; if it does,
the ids named are those whose removal from the report as given changes it (a harm with junk in both ids reads its
target; a betrayal reads its two together, the first named "together with" the other) - `KEEPER_REPORT_FIELD_TYPE`,
`KEEPER_REPORT_ID_EMPTY` (a harm with no target reads its actor; a target of spaces would kill a phantom); a projection
that raises on an id has read it. The payload KEY BY KEY: the written event plus one left-out key, with the ids as
written and as given; a key that changes it, or whose projection raises, is read and named - `KEEPER_TABLE_STALE`,
naming only those keys and the table that forgot them (`world_events.payload_keys`, or the seven dimensions); a read
that two left-out keys make only together is caught by projecting the report as given, whole. A RecursionError
exempts only the key that raised it: a value too deep to fold was never read, and leaving it out keeps it from
bricking every later fold (the fourth review found round 4 exempted every left-out key of the report). WHAT THE CHECK
CANNOT SEE (the third review, which built both cases): the world it projects onto is the log as it stands when the
report is judged, while the event replays into the world the log holds THEN - after a report the same pass writes at
an earlier turn, or a critic correction - so a read that fires only in such a world passed every suite and was
stripped silently. The suite therefore holds the table to the REAL fold at run time over a grid of payloads per type
(each key absent, each true or false flipped) in THREE worlds - the first with the cast alive and nothing known or
held; the second with a fact known, a victim dead, an asset held and one destroyed, an edge standing, a tension
watching only a place, the seed's tension live and the delta's gone; the third with no tension at all - under id
variants that include a stranger, a dead actor and no ids, recording every read, every absent-key probe and every
dimension read, through any helper and through `pop`, `setdefault` or a copy; the second review had found reads
behind `if "k" in payload` and behind values the fixture never took, the fourth reads gated on states the second
world lacked and taken by `pop`. A wrong table is loud where its read fires in the world a report is judged in, or in
a state the grid enumerates through the reads its recorder sees (an unbound `dict.get(payload, k)` it cannot see); one
that fires only elsewhere is not caught - the warrant test judges at check time the same way, as it did at HEAD. Where the fold reads a payload value it must be the type it reads (`validate_payload`'s TYPE half
after its blank check: storable text, true or false, a list of names, the seven dimensions to finite numbers in [0,1],
a finite heat, nothing non-finite anywhere; `WORLD_EVENT_PAYLOAD_VALUE_TYPE`), and nothing anywhere in what is written
may be nested too deep to walk (the log stores a kept map whole and every later fold parses it; the walk's own limit,
a few hundred levels that move with the caller's stack, is the bound - HEAD wrote values 400-800 deep that later folds
survived and one near 950 that bricked every fold after it, so the margin is a declared choice) - board #258 as written, since a keeper
refusal drops one report, nothing retries it, and no keeper reply had ever been recorded (0 in the vault's saved
prompts; 0 keeper calls, world events, extracts, rulings or keeper attachments in 79 book databases), where the seats'
gate refused nothing new because a seat's refusal loses its whole reading and replays 1,122 recorded replies. An
OPTIONAL value of the wrong type is left out and named instead (an extract's `object` that is not text, a ruling's
`rationale` that is not storable text, written as ""); an extract's fields need only be text, since they are stored
normalised; a subject that normalises to nothing is `CLAIM_EXTRACT_INCOMPLETE`, by name. The attach reply's refusals
of a number anywhere, of a `hold` and of a malformed `gaps` are the shared parser's own stated rules, kept (the keeper
reuses that parser whole). Every refusal opens with its code (`KEEPER_TENSION_UNKNOWN`, `KEEPER_NOT_A_WORLD_EVENT`,
`KEEPER_RULING_UNKNOWN`, `KEEPER_VERDICT_UNKNOWN`, `KEEPER_UNCODED_ERROR` for an uncoded error out of the fold, the
write - wrapped now - or the pricing, its type and message named; HEAD's words kept), every pass prints its refusals,
what its kept reports carried beyond what was written, by what became of it - read by nothing, left out of the event
(no world rule reads it), written as null or as empty (`extra`; `replies.change_extra` / `claim_extra` / `ruling_extra` /
`attach_extra`) - what of a reply it could not read and a refused change's lost claim (`notes`, on every return) - as
printable ASCII (`replies.shown` escapes control characters too; `replies.listed` quotes a name that is empty or holds
", " on the console, while the record keeps each name as it is), since the canon gate runs before the scene is parked.
`parse_rulings` counts brackets outside JSON strings (the list it reads is the first JSON list, as before), names more
JSON after it and a reply nested too deep, and no reply crashes the pass (a RecursionError included); the CLI reads its
file as strict JSON and refuses one that is not, or is too deep, by code. The keeper prompts are byte-identical to
HEAD, pinned by hash in the suite. `tensions.validate_seed` refuses a cooling that is not a word by its code (it raised
TypeError). A HEAD-vs-tree differential over 2,765 generated reports, replies and reply texts (`scratchpad/props/g5c5_diff.py`,
local; multi-report, backdated and depth cases among them), classified BY VALUE against a model of what the fold reads
written from `fold.py`'s rules rather than the keeper's code - a junk id judged read by projecting the real fold on the
report as given - with the folded world and resume compared for every same-outcome case and every refusal's CODE
checked against the gate that model says fails first (`g5c5_classify.py`), puts every difference in a declared class,
none unclassified and no code unexpected; and the final tree against the frozen tree the fourth review read, over the
same corpus, differs only in the declared naming and the labelled claims with null change fields (`g5c5_vs4.py`). "No new acceptance" means: no report HEAD judged and refused
is accepted, save a claim labelled with a non-world type (and no change field), an off-ladder word in a key the type never reads (a top-level
one, or a dimension the seven do not name), junk in an id the fold does not read for the report (now null), a ruling
whose rationale the record could not hold (HEAD's write failed; now written as ""), and a valid list HEAD's bracket
counter cut short; reports HEAD never judged because an earlier one crashed its pass are judged now. Suite:
`tests/test_keeper_replies.py`. "Recorded" means the pass's report and returned result: the keeper runs after the last
committed turn and has no row to write on - a durable home is the owner's call (a table, or - the third review's
proposal, the same decision - a reserved payload key keeping what was left out, so a later fold could still read it).
FOUND, NOT CHANGED: a report judged before one the same pass writes at an earlier turn, or before a later critic
correction, replays into a world it was not judged in (the warrant test's binding as well as the strip check's); the keeper can
mint a tension SEED (`tests/test_world_appraisal.py` pins it) while its own refusal text and `tensions.rubric` say it
never mints one; an actor id no cast holds is accepted (a move adds an agent); a harm with no target kills its ACTOR
(the fold's `victim = target or actor`), so a blank or null target does too, as at HEAD - the owner's question; a
seed's nested maps are written as the chassis validates them (only a threat's dimensions are stripped below the top
level); the mood replay (`src/engine/mood_fold.py`) reads "the last event at a turn" as the beat's own, which a keeper
world event appended later at that turn displaces - its own task; `appraiser._json_object`, which the seats share,
counts braces inside strings the way `parse_rulings` did - the seats' reader, its own task.
