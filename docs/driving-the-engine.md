...
| 2 | the wall | situation severity | soft deferral vs "Sit. We will not discuss this." vs the dehumanising form | Ilsa's exit-rate + social_violation magnitude — does the WALL force the break the drive couldn't? |
...
| 5 | regard | model.regard | math channel: appraise() at 0.05/0.2/0.6/1.0 (exact, free) · prompt channel: edge rendering A/B | CARE delta curve (math) · naming "Pell" vs "the worker" + action warmth (prompt) |
| 6 | relationship edge | debt/affinity | Ilsa→Pell debt 0.0 / 0.3 / 0.8 | advocacy intensity, naming, exit-rate |
...
| 8 | urge knobs | floor scheduler | sweep ADDRESSED_BONUS / RECENCY / FLOOR / disruption weight over the recorded BP1.3 beats — deterministic, no LLM | floor distribution + lull timing curves ("Corin takes the floor iff weight ≥ X") |
...
*(model gemma4:26b-a4b, domestic-obstruction scene class; K=6 / EXP-0 K=12; temp 0.7, seeds 0..K-1; raw in `exp-results.jsonl`)*

> **Provenance of these numbers.** The battery was run in June 2026 against the THEN-current `BP13`
> fixture, which was replaced on 2026-08-21 when the engine was separated from the book whose scene
> it had been (CLAUDE.md hard rule 1). The situations differ; the scene CLASS does not — an absent
> but salient subject, a stonewalling authority, and an advocate who cannot act directly. The rates
...
| **Urge knobs** (`scene.py`) | turn ORDER: a listener takes the floor when salience + addressed-bonus + disruption-stake clears the floor (EXP-10: Corin crossed at beat 8, urge 0.18 vs Ilsa −0.02, on her disruption-stake) | what they SAY | EXACT (deterministic arithmetic) | raise addressed-bonus / disruption weight to pull a quiet actor in |
...
1. **The EXIT/rupture is NOT a turn-level lever.** Across ~90 sampled turns and every lever, Ilsa exited **0 times**. At this contained early beat he reliably *stays* — correct, because the rupture is an **arc/accumulation** phenomenon (built over scenes, via Pell), not a knob. Stop hunting a one-turn break lever; grow it.
...
1. **Seed** — what they wake up with: the `## Beliefs` section of their `.md` (`vault.py` loads it; e.g. Pell's seven).
2. **Authored fill** — memories YOU add for the time we *skip* (this is a book tool; we don't simulate every day, yet the character lives in the gaps). To stand in a meal that develops Ilsa and Pell without running it, add a belief line to the character's `.md` Beliefs:
   `- (0.7, a quiet meal — he let me sit) Ilsa shared bread with me in the kitchen [[Ilsa Holloway]]`.
...
- **Lever:** Ilsa's scene drive.
- **A (baseline):** the passive form of the advocate's drive — wanting the absent person accounted for.
- **B (test):** the impulsive form — inaction-intolerable, wanting them brought in from the cold now.
- **Moment:** Ilsa's turn immediately after Arden stonewalls with the soft deferral line.
