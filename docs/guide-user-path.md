...
| `provider.py` | the ONE frontier-model seam (OpenRouter, pinned model, temperature 0, cached prefix, logged) — the seats run through it (2026-09-11) |
| `appraiser.py` | the two seats: the EVENT seat rates the act, the EMOTION seat reads the interior — wired into both drivers (2026-09-11) |
| `readalong.py` | read a novel under `$SWE_BOOKS/readalong/` through the emotion seat and the engine; score it as prediction (2026-09-11) |
...
| ~~`exp.py`~~ | the lever-eval harness behind `driving-the-engine.md` — retired 2026-09-10 to `staging/scripts/` |
...
- **Correct forward, never backward.** The log is append-only, enforced by SQLite triggers. A bad
  ...
  **This paragraph used to end with a promise that a run could be forked instead of rewound, and
  there is no fork.**
