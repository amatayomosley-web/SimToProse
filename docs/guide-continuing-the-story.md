...
a costume.** "Get the layout running before the model railway club's open day" is a drive; "walk out of the club when the open day goes wrong" is foreknowledge.
...
- `fixed`: `name`, `genotype` (one {hit, hold} cell per path — `heritable.py`; the six primitive-era axes are retired; where they rest is authored at `baseline.temperament[path].rest`), plus `position`/`people`.
...
**The discipline — design SUBSTRATE for the TARGET, never a label:** a character's
canon ("the politest person in any room, with the shortest fuse in it") is the **acceptance target**, not a rule. Build the person from whom the
behavior **falls out** — usually an **opposable tension** (e.g. innate DISPLEASURE in genotype/temperament vs a
formative high `conformity` in `model.schwartz` + a vault belief) — then **verify by simulation** that the target emerges.
...
- **A character's arc:** authored ONCE (its starting state). It is **run, not written** — no waypoints, no
  time-skips; the baseline drifts from consequences.
