...
- `baseline`: `temperament` (ALL 7 primaries × `{mean, variability}`), `traits`, `model`
  (`schwartz`/`moral_foundations`/`needs`/`regard`), `drives`, `skills`, `voice`.
- `current`: `affect` (7 primaries 0..1), `condition` (a dict), `active_goals`, `relationships`
...
runnable: the 7 primaries, an affect, a drive, a few beliefs). Capture depth = the future cut-space; you
...
- **A character's arc:** authored ONCE (its starting state). It is **run, not written** — no waypoints, no
  time-skips; the baseline drifts from consequences (Brakk's sheet is the worked example).
