...
| `extreme` | 0.95 | the worst of its kind a life contains | the top of the strength scale; the relationship CLIFF reads the act ladder's floor word (`treacherous`), not a strength word, since bond gate 4 (2026-09-17) |
...
- **The child** — genotype `WARINESS: {hit: high}` (`heritable.GAIN`, the adopted START), security-heavy
...
- **Ranges by row kind**, anchored to existing engine numbers, all **[CALIBRATION]** starts:
  *disposition-grade* (a standing lean) ×0.6–1.5 — the hit-cell range in `heritable.GAIN`
  is the precedent, and `make_genotype --rows` emits genotypes as exactly such
...
A recipe is `{primitive: (weight, role)}` (the retired `compounds.py`, not in this repo, lines 45-124); weights are SHAPES (similarity
is cosine, so contempt at 0.4 and 0.9 are one state at two strengths — the retired `compounds.py`, not in this repo, lines 42-44), and
the SUM is the identity dial: `blend` fills `(1 − sum)` with the character's resting self
(the retired `compounds.py`, not in this repo, lines 248-296; `emotion-recipes.md:15-25`). Today the 42 sums "span 0.60–1.10 (mean
...
1. **Choose the SUM first**, by answering the one question it encodes: *how much of the person
   ...
   **≥ 1.00** totalizing, reserve — only for states whose phenomenology IS self-loss (the sole
   ...
   entirely (the retired `compounds.py`, not in this repo, lines 283-284); spending that on mild fondness is a category error.
...
3. **Roles are part of the shape.** Same magnitudes, different role signature = a different state
   (pride vs excitement — the retired `compounds.py`, not in this repo, lines 236-241); shame vs guilt separate on roles, not
   numbers (`emotion-basis.md:84-86`). A new recipe must declare a role per primitive from the
   closed set (the retired `compounds.py`, not in this repo, lines 40).
4. **Admission gates, both deterministic:** `validate()` clean (no primitive the basis lacks —
   blocked loudly, never truncated, the retired `compounds.py`, not in this repo, lines 30-34,144-162), and `separability()` at 0.95
   finds no same-role-signature neighbour (the retired `compounds.py`, not in this repo, lines 224-245) — a duplicate shape is a
...
2. **The within-band no-op believed in** — a magnitude or row that crosses no edge, trusted to
   ...
   nothing — this repo's dominant defect class (the retired `compounds.py`, not in this repo, lines 30-31).
