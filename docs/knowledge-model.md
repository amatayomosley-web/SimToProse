...
- A vault grows ONLY by what the character **knows or learns**. Acquisition channels: **witnessed, overheard, read, told, taught, deduced** (inference — and inference can be wrong). Monotonic-add for the MVP (forgetting is a later layer). What DOES fade is a memory's retrieval strength, derived at read and never stored (`decay.calculate_effective_confidence`): by the story time since the beat that formed or last recalled it, or, for a memory the sheet carries, since the character's own first scene (gate own-timelines, 2026-09-25; it was page one, for everyone) - live since 2026-09-24 (gate memory-fades; until then both drivers handed the recall gate zero time and no memory faded in a run). A learned memory carries the turn it was learned (`created_turn`), and is filed as meaningful (`durability: durable`, gate learned-memories-durable): only a durable event becomes a memory, and a learned name is a standing fact, so none of them fades like an everyday detail.
