...
`state._DIM_TO_PATH` prices events to four decimal places against a scale nobody had defined.
...
(the 0.05 quantisation decision is recorded at `docs/emotion-dynamics.md:79-92`; this line cited
`connection-model.md`, which contains no `0.05` and no quantisation — corrected 2026-09-06) sit
between them and inherit
...
intensities, and the engine already concedes this: `_DECAY_RATE` (retired 2026-09-10 for `_HALF_LIFE` in minutes) ran from 0.72 (FEAR) to 0.90
...
* **The counters.** Four primitives — RAGE, LUST, CARE, DISGUST — have no dimension that lowers
  ...
  unbuilt.
* **The twenty prompt bands.** The 0.05 quantisation and its hysteresis are decided. ~~the phrases
  are unwritten~~ — **RETRACTED 2026-09-06: they are written.** The eight `docs/phrases-*.md` files
  hold **280 authored bands** on the 0.05 grid (care 40, disgust 60, fear 20, lust 40, panic-grief
  40, play 20, rage 20, seeking 40). What is missing is not the artifact but the WIRING: nothing
  reads those files, and `direction._PHRASES` still serves the prompt with 32 behavioural phrases,
  which makes the engine decide what the character does and leaves the actor nothing to act. The
  authored bands must be authored as FELT STATE, and the path layer is what will consume them.
...
* **Every magnitude in `_DIM_TO_PATH`.** Calibration, not definition. This document fixes what
