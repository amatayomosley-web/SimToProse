...
- **Scene goals: author OUR wants, then build the drives — never the outcome into the drive.** (the author, 2026-06-11.) A scene is set up in two passes. FIRST establish *our* wants — the author's intent, from the blueprint: what the reader must *feel*, and the beats the scene must *reach*. THEN build each actor's DRIVE **backward** from those wants — but a drive is a genuine **standing want, blind to how the scene ends**, never the outcome wearing a costume. *"Priya wants to storm off court mid-match and end her doubles partnership"* is foreknowledge — the ending smuggled into the drive. *"Priya wants to win the club doubles final"* is a drive — a want that does not know there will be a fight. The conflict, the break, the turn must **emerge** from decent drives colliding (often over something each actor weighs differently) — **verified by simulation**. If the wants don't appear, you re-tune the **drives**, never script the **lines**. This is "the simulator must be BLIND to the director's beat" (above) applied to scene setup: the *want* is the director's; the *drive* must not carry it into the actor. Validated live on a real book's scene (`multi-character.md` §v1) — decent drives produced the conflict with no villain in the room.
- **State runs whether or not the page is looking.** (the author, 2026-09-24: *"these are real people, the book is us peeking into their world. When we look doesn't determine their state, their stat runs with or without us looking."*) Every change that time makes — a mood cooling, energy draining and resting, a bond drifting toward where it rests, a scar or an injury easing, an attitude or a memory fading, a disputed fact cooling — runs on story time, the one clock (`src/engine/clock.py`), for every character, on the page or off. A scene or a chapter is where the story's time is READ, never a cause: a change of point of view that skips no time ages no one, a time skip ages everyone by its length, and the minutes inside a scene count exactly like the minutes between scenes. `docs/SPEC-LEDGER.md` records which tiers meet this and from when (gates absent-age, story-clock and after).
