...
> *Vocabulary note, 2026-09-10: a dated design record. `scripts/exp.py`, whose shape this doc specifies the battery runner from, was retired to `staging/scripts/exp.py` on 2026-09-10 (dead cast, primitive vocabulary, no test); the runner it describes is still unbuilt. `tests/test_retired_vocabulary.py` exempts it on that ground.*
...
`<book>/scenes/scene_01_cfg.json`). Per the private-content rule (`tests/test_no_private_content.py`)
this doc names no one and tells none of the scene: **the TESTED character** is the seat whose levers
an arm moves, **the PARTNER** is the other seat, and **the disposition** is the TESTED character's
authored, saturated choice that the ladder arms push against. The battery's own configs, variant
sheets, and verb lists carry the real names and the scene's content, and live in the BOOK's vault, never in this repo.
...
|---|---|---|---|
| `scene-pilot-a-1787380588` | 2 | lull (UNPERSISTED: stdout showed `max urge -0.017`) | situation text naming none of the scene's objects; drives phrased as topics |
| `scene-pilot-a-1787381616` | 8 | exit (the final action leaves the scene; the `exit` flag itself is not persisted) | the scene's objects named in the `situation` string; the PARTNER's drive rewritten from a topic to a physical goal |

Same cast, same sheets, same model, zero numbers changed. Tag types over the 8-beat run: seven
of the care/aid family and one `affront` (db, `turns.tags`). Per-beat latency from
...
two moved a scene from death-at-2 to an exit-at-8.
...
| R3 | identity numbers | system msg, **raw JSON** | `prompt.py:46` dumps `packet["stable"]` — built from `fixed`+`baseline` only (`scene.py:155 _build_stable`). Measured on one cast member: **36 decimals in one system message** (traits, model weights, drive priorities, voice.assertiveness) |
...
| R9 | world laws | **nothing at turn time** | `bible.verdict_for` / `laws_bearing_on` (`law.py:340–391`, re-exported by `bible`) are called by NO runner — only `lint_book.py` (a warning string) and the Mode-B chat hook `.claude/hooks/ground_from_book.py`. The book's authored laws bind nothing while a scene runs |
...
**Load-bearing fact 4 — the recall substrate is currently dead.** Every cast vault loads
...
(lint now warns). Most of the cast have no relationships. No belief has ever fired in this book
...
triples — the `urges  :` lines; a departure — `>> .* leaves the scene`.
...
`empty` (blank action); and two frozen verb lists per scene-class — a YIELD list and a HOLD list
(e.g. for a character authored never to swim: wades in/dives vs stays on the sand) — authored in the
battery config **in the book's vault**, frozen before the first sample, applied to the lowercased action string.
...
2. **Standard moment.** All turn-level arms use ONE frozen moment from the two-hander's scene
   class — the PARTNER's line, the TESTED character acting — stored verbatim in the vault-side battery
...
2×2: situation {without the scene's objects named, with} × PARTNER drive {topic form, physical-goal form}.
...
|---|---|---|---|
| **T1 drive phrasing** | R2 | topic form / physical-goal form / outcome-in-costume ("see that it is settled tonight") | phrasing moves CONTENT and physicality of the act (object-verbs in action), not the exit flag (June: drive never toggled exits; re-test on this fixture) → any exit-rate split ≥5/6 vs ≤1/6 |
...
| **T4 edge bands** | R5 | the TESTED character's edge to the PARTNER, trust/affinity at low/mid/high band points (variant sheets) | naming, warmth verbs, addressee move; the disposition's choice does not → the choice flips with edge warmth |
...
| **T6 identity wall** | R7 | a stranger present; TESTED-variant insight 0.50 vs 0.60 (DC 0.55) | below-DC: "person present" only — actor never names/roles the stranger (0/6); above: does. NOTE the wall gates only what event text withholds — the raw situation string is verbatim (R1), so the cue must live in the people note's identity record, not the event text → below-DC naming >0 means the identity wall leaks |
| **T7 the disposition ladder** ("cannot" #1) | all state routes vs one saturated disposition | 5 rungs stacking state toward the choice the TESTED character's authored disposition rules out: (1) a drive toward it; (2) +affect (warm: CARE/PLAY high band); (3) +edges maxed; (4) +an authored belief commanding it; (5) all four. K=6 each; the strongest rung re-run at **K=24** | the ruled-out choice stays ≤1/6 through rung 4 (identity prefix + prior wins; June's wall finding: hard pressure yields suppressed compliance, not reversal); rung 5 ≤ 12% at 95% → any rung that yields at ≥3/6 kills the bound and yields the recipe instead |
| **T7b circumstance coda** | R1 | the fait-accompli form: the moment states that the ruled-out choice has already been made for the TESTED character | the WORLD moves even though the CHOICE didn't: the actor cannot undo it in-scene; measure undo-attempt verbs (predicted ≥4/6 — the disposition reasserts as next-turn action) → a low undo-rate means circumstance quietly rewrote the disposition, which would be the single most surprising result in the battery |
...
|---|---|---|---|
| **S1 exit anatomy** | is the exit an URGE artifact or a choice? | reuse A1's with/physical cells; align each exit against the D1 replay: was the exit beat preceded by an affront-type escalation crossing the frontier? | exits cluster after affront-tagged beats; if exits appear without escalation, the exit flag is prior-driven, not dynamics-driven |
| **S2 third presence** | does cast composition alone re-route a scene? | two-hander vs +1 existing cast member, same cfg otherwise, K=6 | the added `_order_weight` stake changes floor allocation (deterministic half) AND suppresses the exit (social inhibition in the room) — beat count up, exit rate down |
...
| drive | R2 | content + physicality of the push | exit, a choice against the authored disposition | T1, T7 |
...
| edges | R5 | warmth, naming, addressee | the disposition's choice itself | T4 |
...
1. **State cannot flip a saturated disposition in one turn.** No stacking of drive/affect/edges/
   beliefs makes the TESTED character act against it in-scene. Cheapest: T7 (ladder; the K=24 rung is the claim).
...
4. **The world's laws cannot refuse anything during a run** (Mode A). T8 — the sharpest
   architecture finding if confirmed: the book's authored laws with zero runtime consumers; enforcement
...
9. **Beliefs cannot fire from prose-form authoring** (parser bound, `vault.py:22`) — already
   demonstrated by the empty vaults + every `belief_refs=[]`; T5's parseable-form conditions are
