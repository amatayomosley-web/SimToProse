...
- **Deception (hidden in WRITING mode):** secret_motivations, hidden_identity, transformation_arc, deception_flags
...
4. **One pricing table, applied at every timescale.** What an event does to a person is a property of the event and the person, not of how long the effect lasts. `state._DIM_TO_PATH` is that table: it prices all eight primaries and carries negatives. A second, thinner table for durable writes is a duplicate that will drift — and has.
...
|---|---|---|---|
| the feeling of the moment | the character's temperament (the mean seeded from the authored rest WORD, `baseline.temperament[path].rest` — a design choice beside the voice) | a half-life per path per zone, in MINUTES (`state._HALF_LIFE`), times the genotype's hold, times (1 + investment in what the path is about) and x3 while that thing is in the room (gate three, 2026-09-11) | the one clock, in minutes: the gap at a scene's opening, and `lasts / budget` per beat inside it |
| a relationship edge | the resting prior | slow | the one clock, read in days |
| a wound (engine state at `baseline.wounds`, keyed concept@PATH since 2026-09-11) | the wound's own permanence floor | very slow | the one clock, read in days |
| a temperament mean | the AUTHORED base (`_authored_mean`) | barely at all | the one clock, read in days |
| a feeling toward one person (`current.toward`, the micro half of THE THREE LAYERS) | **zero** | per primary, see below | the one clock, read in days |

**MOOD, ATTITUDE, BALANCE (2026-09-12, the redesign's gate 1 — `cairn/projects/simtoprose-emotion-redesign.md`).**
The flat float is the MOOD: fast, what everyone in the room meets. `current.toward[who][path]` is
that person's ATTITUDE: slow, what they have EARNED, a level on the path's own ladder — any rung,
the 0.25 cap is gone. What the actor plays is COMPOSED each beat by `toward.balance` and never
stored: toward the ENGAGED person, their attitude when it is above the mood; otherwise halfway down
from the mood and never below the speaker's rest — except that the person the mood is ABOUT
(`current.targets`, the path's aboutness) meets it in full. Others present may lift a path to a
damped fraction of their attitude, never lower one. So a reserved beekeeper is `quiet` at rest,
tender with the neighbour whose attitude sits at tenderness, `quiet` with a visitor — and a warm
mood raised by one person spills onto a stranger by half. Two feeds into attitude, both through
`toward.coalesce` before the commit: the EVENT route (`toward.observe`, what the person was
witnessed doing, priced through the one table) and the READING route (`toward.observe_readings`,
every reading the seat binds to a person, at the reading's vector x the connection multiplier x
the durability gate). The balance reaches the actor as words on the "Those present" line
(`actor-direction-format.md` §5).

**ONE CLOCK, SINCE 2026-09-10** (superseding "two clocks and no third" above this line in older
copies). The owner locked duration to the minute and made the declaration required: every scene
cfg says when it opens (`at`), optionally how long it runs (`lasts`), and the gap between scenes is
DERIVED, never typed (`src/engine/clock.py`). The moment's feeling is the fifth consumer of that
clock and reads it in minutes; the four slower tiers keep the per-day rates they were calibrated
in and read the same clock through `clock.elapsed_days_since`. A single conversation still does
not erode a friendship — a ninety-minute scene is a sixteenth of a day to the bond tier — and a
beat with no authored duration is zero minutes to every tier.

### Feelings toward a person fade at different rates

Ordered as `state._HALF_LIFE` (which replaced the per-beat `_DECAY_RATE` on 2026-09-10) already orders the paths, because the fast/slow ordering is a
...
**DECREASE** — three distinct ways, which is more than the design needed to specify:
  ...
  · `bond_rest.drift` relaxes an untouched edge toward its own REST over declared time — where it
    was authored, or lower after a cliff; a stranger's rest for an edge the sheet never wrote.
  · **the cliff** — the act ladder's floor word (`treacherous`) on an act of `_CLIFF_RELEVANCE` 0.60
    collapses trust to `_CLIFF_FLOOR` 0.15 outright, and moves the rest there with it (bond gate 4).
    Erosion AND rupture: a bond can fall off, not only wear down.
