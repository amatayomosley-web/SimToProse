...
How the "effective weight = baseline × state × situation × relationships" formula is actually
computed and authored. Two sources, both restated here so this section stands alone: a TTS
emotion-vector model, and the RPG stat/modifier idiom.

**The emotion-vector model, inlined.** A speech model steered by an emotion vector over a fixed
basis, mixed against the speaker's own resting voice:

> `emovec = emovec_mat + (1 − sum(weight_vector)) × speaker_baseline`
>
> When `sum < 1.0` the remainder fills with the speaker's baseline; when `sum >= 1.0` it is pure
> steering with no baseline bleed.

Two ideas in it are load-bearing here, and one is a trap:

1. **The sum is an identity dial.** How hard the moment steers and how much of the person's resting
   self survives it are *the same number*. A total activation of 0.35 leaves 65% of who they
   ordinarily are; a total of 1.0 erases it. This is the principled form of "how much of me
   survives this moment", which otherwise gets answered by an arbitrary constant.
2. **Emotions are COORDINATES, not labels.** In that basis, contempt, disdain and scorn are three
   neighbouring points — heavy on one dimension, differing in the trace amounts of two others. A
   character can sit 70% of the way toward contempt, or between contempt and scorn. **This is how
   nuance is expressed without tags:** the name is a handle on a position, not a category. A
   bounded basis plus named compound coordinates gives authorable, inspectable, continuous degrees.
3. **The trap — do not port the basis.** Those dimensions are *expressive* (how a voice sounds);
   the levers here are *motivational* (what makes a person act). They do not correspond, and the
   sum constraint is a rendering constraint — a voice must resolve to ONE voice — not a claim about
   psychology. A person genuinely can be terrified and grief-stricken at once, so the state vector
   stays unconstrained. Borrow the mixture only where this system has the same
   must-resolve-to-one-deliverable problem: the direction/prompt seam, where the actor does one thing.
