...
*(**STATUS 2026-09-06: normative for the PRIMITIVES, superseded for NAMING.** The eight primitives
and their targets stand. The compound layer is no longer how a state gets its name — see
`emotion-dynamics.md`, which replaces recipe-matching with PATHS (one axis per emotion, the name
changing at rungs along it). Read this doc for what the primitives are, not for how a feeling is
named.)*
...
- ~~**The compound layer does not exist.**~~ **RETRACTED 2026-09-06.** It was built the same week
  this line was written: `src/engine/compounds.py` holds 41 recipes with `compose`, `recognise`,
  `blend`, `validate` and `separability`, exercised by `tests/test_compounds.py`. The line was stale
  within a day and stayed up for two weeks (`SPEC-LEDGER.md:258` logged the same rot). What IS true
  now is narrower and worse: `recognise` scored by cosine, which is scale-invariant, so it named the
  same compound at every magnitude — it was wired into `direction.py`, measured wrong, and unwired
  on 2026-09-06 (`emotion-dynamics.md:616-621`). The recipes are testable; the MATCHER is retired.
...
and did not. The clause renderer (RETIRED 2026-09-08) decided what the actor was told:
...
That reading stands as written, and the salience-grouping repair is not indicated. The renderer it was measured on is RETIRED 2026-09-08.
