...
OWNS, plus the project's VOCABULARY. The exact counts of design docs, engine modules and suites live in `docs/MAP.md`, regenerated by `scripts/gen_map.py` (`--check` clean 2026-09-19 — not hand-counted here; the last hand count drifted, which is why this line no longer carries one) —
...
6. Files under **300 CODE lines** (non-blank, non-comment, non-docstring) and 800 total · stdlib only ·
   modules fail loud (ValueError/RecordError/LedgerError) — scoped to the Layer 1 engine; the RUN
   degrades-not-crashes and records the failure (the boundary is run_probe's try/except).
   The bound was ONE number on TOTAL lines until 2026-09-06, when measuring all 51 engine modules
   showed it read the wrong one: 57% of `src/engine/` is comment, docstring or blank, and the three
   files it flagged were 41-51% prose (consolidation 650 total / 272 code, state 606 / 217). It taxed
   the reasoning record, which is the thing that makes this engine auditable, and it never once
   flagged `code_families.py` — the largest module by code, at 341 — because that file sat at 499.
   A module with zero functions and zero branches is a lookup TABLE and is exempt from the code
   bound (never from the total one); the exemption is a test, not a name list. Neither number comes
   from a cliff — the code distribution runs 341/272/272/235/217/202 with no gap — so both are sited
   just above the working population and move only DOWN. Measured by `tests/test_map.py`.
...
never run is not a rule. On 2026-08-21 the sweep was refreshed to cover every private book's cast and
...
**It was not.** The default scene and a dozen fixtures had been RENAMED, not replaced — the same
scenes under new names, which a name sweep cannot see. Six reviewers reading every tracked line
found 556 such lines in 97 files, and they were rewritten as invented scenes on 2026-09-23 (gate
publish-scrub). A green name sweep says nothing about plot.
...
surnames, places and titles of the owner's private projects, in a tracked file exempt from its own sweep —
...
**Three more guards since 2026-09-23 (gate leak-guards), because the tree sweep was not enough.** The
public remote was found holding private names and renamed scenes that no tree sweep had seen: a TAG
carried an old snapshot past every branch check, and the terms list had fallen 53 names behind the books.
- **The publish gate** — `scripts/push_guard.py`, run by `.githooks/pre-push` (enable per checkout:
  `git config core.hooksPath .githooks`). It refuses every tag, every outgoing commit whose message or
  added lines carry a private term or a machine path, and every push whose tip has no REVIEW RECORD
  (`<sha>.md` naming that commit, in `$SWE_PUBLISH_REVIEWS`, default `<parent of $SWE_BOOKS>/publish-reviews`).
  Renamed plot is invisible to every pattern — phrase fingerprints of the books were measured the same
  day and missed the renamed scenes while flagging clean files — so the only check for plot is a
  reading, and the record makes that reading a condition of the push. It proves a review was recorded
  for exactly that commit, not that it was good. It fails closed: no terms list or no review folder
  refuses. `git push --no-verify` skips it, visibly. Proven through real `git push` runs in
  `tests/test_push_guard.py`.
- **The list keeps up with the books** — `test_no_private_content.py` now reads the books themselves:
  every name a private book uses must be on the terms list or on the vault's reviewed-ordinary list
  (`$SWE_NAME_REVIEW`, default `<parent of $SWE_BOOKS>/name-review.txt`). A new name in a book fails the
  suite until someone classifies it. Both lists live beside the books, never here.
...
**Engine (Layer 1): built + verified.** Spine + full pipeline, 76 test suites passing (re-measured 2026-09-06; it was 8 when this line was written) +
...
Known gaps (don't rediscover as bugs; corrected 2026-09-19 — full detail in docs/SPEC-LEDGER.md): a failed perception check yields NO belief about an act rather than a wrong one (the false-belief layer is unbuilt engine-wide); critic detects, and since 2026-09-19 can also repent (`scripts/critic.py --correct` appends a `correction` event per continuity flag — no longer detect-only); the cutting room's render/decision record — once read-only, no edit-decision-list — is now the built EDL (`src/engine/edl.py`, wired into `cut.py --edl`/`--revise`/`--show-edl` and `narrate.py`);
character generation is BUILT — genotype (scripts/make_genotype.py, seeded draw + jitter), the composition pass's script half (94-profile library, src/engine/profiles.py, scripts/composition_pass.py), and the LLM classification step (scripts/composition_pass.py build_classify_prompt/picks_from_classification/--classify) are all built (corrected 2026-09-19). See
